...
def serve_scatterplot(xc,labels=None):
    ...
    if labels is None:
        figure.add_trace(go.Scatter(mode='markers',x=xc[:,0], y=xc[:,1],
                   marker=dict(color='Gray',size=2)))
    else:
        figure.add_trace(go.Scatter(mode='markers',x=xc[labels == -1,0], y=xc[labels == -1,1],
                   marker=dict(color='Gray',size=2)))

        fig_markers = px.scatter(x=xc[labels>=0,0], y=xc[labels>=0,1],color=(labels[labels>=0]).astype(str))
...
def serve_along_optima_figure(cb,tuple_statistics, change_threshold=0.95):   
    ...
    df_grouped = df_.groupby(['threshold'])['clusterSize'].agg([tuple_statistics])
    value = df_grouped[key]

    #Rescale and find first time 90% change is detected
    x_values = df_grouped.index
    rel_value = np.asarray((value-np.min(value))/(np.max(value)-np.min(value)))
    ...
    figure.add_vline(x=x_values[idx], line_width=3, line_dash="dash", line_color="red")
...
def serve_phasespace_figure(cb,quantity='no_clusters'):
    ...
    heatmap1_data = pd.pivot_table(cb.phasespace_all, values=quantity,
                         index=['threshold'],
                         columns='sigma')
    figure.add_trace(
        go.Heatmap(z=heatmap1_data,x=heatmap1_data.columns,y=heatmap1_data.index)
    )

    # Plot line of optima 
    figure.add_trace(
        go.Scatter(x=cb.df_opt_th['sigma'], y=cb.df_opt_th['threshold'], mode='lines+markers' , name='Line of optima')                    
    )
