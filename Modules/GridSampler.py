...
class GridSampler:
    ...
    def __init__(self,XC,N_goal=10000):
        ...
        self.N_goal = N_goal;
        self.XC     = XC;
        self.TOL    = 5;    #accuracy that mean of cluster size distribution is required to be in 
        self.delta  = 0.05; #acceptable probability that mean of cluster size distribution is mis-estimated by over TOL
        self.max_clusterSize_toAnalyse = 200;
        self.minPointsPerGrid = 100;
        self.seed = 0;
        self.delta_lim = 0.05; #error for estimation of mean of cluster size
    ...
    def sampleGrid(self,algo):
        ...
        #for i,d in self.df_limits.iterrows():
            ...
            mark = (XC[:,0] >= d['xmin']) & (XC[:,0] < d['xmax']) & (XC[:,1] >= d['ymin']) & (XC[:,1] < d['ymax']);
            
            if(np.sum(mark)<self.minPointsPerGrid):
                print("Grid sample contains "+str(np.sum(mark))+" points (< "+str(self.minPointsPerGrid)+" ), therefore skipped");
                continue;
