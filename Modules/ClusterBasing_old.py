...
from FigX4_Explore2DOptimizer_withReference_Streamlined_Functions import DefineCleanedLabels_GeneralLimit, \
    GetLineOfOptima
...
class ClusterBasing:
    ...
    def __loadPoints(self, basefolder, parameters):
        ...
        if (os.path.isfile(basefolder + parameters['save_name'] + "_filtered_incell.txt")):
            XC_incell = LoadPoints(basefolder + parameters['save_name'] + "_filtered_incell.txt",
                                   datascale=parameters['datascale']);
            XC_outcell = LoadPoints(basefolder + parameters['save_name'] + "_filtered_outcell.txt",
                                    datascale=parameters['datascale']);
        else:
            XC_incell = LoadPoints(basefolder + parameters['mainfolder'] + parameters['image_filename'] + '_incell.txt',
                                   datascale=parameters['datascale']);
            XC_outcell = LoadPoints(
                basefolder + parameters['mainfolder'] + parameters['image_filename'] + '_outcell.txt',
                datascale=parameters['datascale']);
    ...
    def GetReferenceClustering(self, bestRequiredRate=1.0):
        ...
        # *********************************************
        phasespace_all_aboveT = DefineCleanedLabels_GeneralLimit(self.df_clusterSizes_all, self.phasespace_all,
        ...
        self.df_opt_th_aboveT_ncl = GetLineOfOptima(
            self.phasespace_all_aboveT[['sigma', 'threshold', 'similarityScore', 'no_clusters']], 'threshold',
            'no_clusters');

    def GetClustering(self, criterion='percent_locsIncluded'):  # 'no_clusters'

        df_opt_th_aboveT_ncl = GetLineOfOptima(self.phasespace_all_aboveT[
                                                   ['sigma', 'threshold', 'similarityScore', 'no_clusters',
                                                    'percent_locsIncluded']], 'threshold', 'no_clusters');
