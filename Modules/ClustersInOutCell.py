...
class ClustersInOutCell:
    def __init__(self, basefolder, algo="DbscanLoop", points_per_dimension=15):
        ...
        if os.path.isfile(
            basefolder + "X_incell_window.txt"
        ) and os.path.isfile(basefolder + "X_outcell_window.txt"):
            self.XC_incell = LoadPoints(basefolder + "X_incell_window.txt")
            self.XC_outcell = LoadPoints(basefolder + "X_outcell_window.txt")
        else:
            print(
                "X_incell_window or X_outcell_window in folder "
                + basefolder
                + " not found!"
            )
    ...
    def GetClusterings_InOutCell(self, skipSimilarityScore=True):
        ...
        if os.path.exists(filename + "_incell.pickle"):
            ...
            labels = FD.fit(
                self.XC_incell, skipSimilarityScore=skipSimilarityScore
            )
        ...
        if os.path.exists(filename + "_outcell.pickle"):
            ...
            labels_ref = FD_ref.fit(
                self.XC_outcell,
                self.XC_incell,
                skipSimilarityScore=skipSimilarityScore,
            )
    ...
    def GetSimilarityAlongOptima(self):
        ...
        if os.path.exists(filename + ".pickle"):
            ...
            print(
                "Loaded clusterDataWithSimilarityScores results from "
                + filename
                + ".pickle"
            )
        else:

            self.df_opt_th = GetLineOfOptima(
                self.phasespace_all[["sigma", "threshold", "no_clusters"]],
                "threshold",
                "no_clusters",
            )
            phasespace_sub = self.phasespace_all.loc[self.df_opt_th["idx"], :]
            df_clusters_opt_th_in = getClusterSizesAll(
                self.XC_incell, phasespace_sub
            )

            print(
                "Computing similarity score along line of optima for in cell"
            )
            cli_similarityScore_in, similarityScore_in = getSimilarityScore(
                self.XC_incell, phasespace_sub, df_clusters_opt_th_in
            )

            print(
                "Computing similarity score along line of optima for out cell"
            )
            ...
            df_clusters_opt_th_out = getClusterSizesAll(
                self.XC_outcell, phasespace_out
            )
            cli_similarityScore_out, similarityScore_out = getSimilarityScore(
                self.XC_outcell, phasespace_out, df_clusters_opt_th_out
            )
            ...
            self.df_clusters_opt_th = df_clusters_opt_th_in.append(
                df_clusters_opt_th_out, ignore_index=True
            )

            with open(filename + ".pickle", "wb") as handle:
                data = {
                    "df_opt_th": self.df_opt_th,
                    "df_clusters_opt_th": self.df_clusters_opt_th,
                }
                pickle.dump(data, handle, protocol=pickle.HIGHEST_PROTOCOL)
            print(
                "Computed and saved clusterDataWithSimilarityScores results in "
                + filename
                + ".pickle"
            )

        self.df_opt_th["threshold"] = (self.df_opt_th["threshold"]).astype(int)
        self.df_clusters_opt_th["threshold"] = (
            self.df_clusters_opt_th["threshold"]
        ).astype(int)

        self.df_opt_th["similarityScore"] = (
            self.df_opt_th["similarityScore"]
        ).astype(int)
        self.df_clusters_opt_th["similarityScore"] = (
            self.df_clusters_opt_th["similarityScore"]
        ).astype(int)

    def GetReferenceClustering(
        self,
        bestRequiredRate=1.0,
        computeSimilarityScores=False,
        generalLimit=True,
    ):
        ...
        # *********************************************
        phasespace_all_aboveT = DefineCleanedLabels(
        ...
        self.df_opt_th_aboveT_ncl = GetLineOfOptima(
            self.phasespace_all_aboveT[
                ["sigma", "threshold", "no_clusters", "percent_locsIncluded"]
            ],
            "threshold",
            "no_clusters",
        )
    ...
    def GetClustering(self, criterion="percent_locsIncluded"):  # 'no_clusters'
        ...
        i_choose = self.df_opt_th_aboveT_ncl.loc[
            self.df_opt_th_aboveT_ncl[criterion].argmax(), "idx"
        ]
    ...
    def get_info_phasespace_approx(this, sigma, threshold):
        ...
        idx = np.argmax(
            (this.phasespace_all["sigma"] == sigma)
    ...
    def plot_phasespace(
        ...
        df1 = this.df_clusterSizes_all[
            (this.df_clusterSizes_all[criterion] > min_criterion)
            & (this.df_clusterSizes_all["type"] == type)
        ]
        ...
        heatmap1_data = pd.pivot_table(
            df1, values="value", index=["threshold"], columns="sigma"
        )
        ...
        sigma_idx, th_idx = this.__get_indices_in_phasespace(
            this.df_opt_th["sigma"], this.df_opt_th["threshold"]
        )
        ax.plot(sigma_idx + 0.5, th_idx + 0.5, "b")

    def plot_phasespace_in_vs_outcell(
        this, value="no_clusters", criterion="clusterSize", min_criterion=0
    ):
        fig, axs = plt.subplots(1, 2, figsize=(12, 6))

        this.plot_phasespace(
            axs[0],
            value=value,
            type="incell",
            criterion=criterion,
        ...
        plt.tight_layout()
        plt.savefig(
            f"{this.save_name}_{value}_for_{criterion}_greaterthan_{min_criterion}.pdf",
            bbox_inches="tight",
        )
        plt.close("all")
    ...
    def plot_clustersizes_along_optima(this, min_clustersize=30):
        ...
        df_in = df_clusters_opt_th[
            (df_clusters_opt_th["type"] == "incell")
            & (df_clusters_opt_th["clusterSize"] >= min_clustersize)
        ]
        df_out = df_clusters_opt_th[
            (df_clusters_opt_th["type"] == "outcell")
            & (df_clusters_opt_th["clusterSize"] >= min_clustersize)
        ]
        ...
        plt.tight_layout()
        plt.savefig(
            f"{this.save_name}_ClusterSize_ResultsAlongOptima_minClustersize{min_clustersize}.pdf",
            bbox_inches="tight",
        )
    ...
    def plot_statistics(this):
        ...
        df_grouped = this.df_clusters_opt_th.groupby(["threshold", "type"])[
            "clusterSize"
        ].agg(
            [
                ("median", np.median),
        ...
        for key, ax in zip(df_grouped.keys(), axs.flatten()):
            sns.lineplot(
                data=df_grouped, x="threshold", y=key, hue="type", ax=ax
            )
            ax.set_ylabel(key)

        plt.tight_layout()
        plt.savefig(
            f"{this.save_name}_Statistics_alongLineOfOptima.pdf",
            bbox_inches="tight",
        )
    ...
    def plot_gif_along_optima(
        this, type_, criterion="clusterSize", min_criterion=0
    ):
        ...
        for idx, row in this.df_opt_th.iterrows():
            ...
            mark = (
                (df_clusters["index"] == idx)
                & (df_clusters["type"] == type_)
                & (df_clusters[criterion] >= min_criterion)
            )

            this.plot_scatter_selection(
                XC,
                this.phasespace_all.loc[idx, labels_name],
                df_clusters.loc[mark, :],
                ax,
                scalebar=True,
            )
            ...
            ax.text(
                ...
                + " , r = "
                + str(np.round(row["sigma"], 3)),
                ha="center",
                va="center",
            )

            plt.savefig(
                png_folder + "goThroughOptima_" + str(count) + ".png",
                bbox_inches="tight",
            )
            plt.close("all")
            count += 1
    ...
    def plot_overview_scatterplots(this):
        ...
        plt.savefig(
            this.save_name + "_localizations_incell_vs_outcell.pdf",
            bbox_inches="tight",
        )
        plt.close("all")

    def plot_similarity_results_along_optima(
        this, criterion="clusterSize", min_criterion=0
    ):

        df_in = this.df_clusters_opt_th[
            (this.df_clusters_opt_th["type"] == "incell")
            & (this.df_clusters_opt_th[criterion] >= min_criterion)
        ]
        df_out = this.df_clusters_opt_th[
            (this.df_clusters_opt_th["type"] == "outcell")
            & (this.df_clusters_opt_th[criterion] >= min_criterion)
        ]
        ...
        plt.tight_layout()
        plt.savefig(
            f"{this.save_name}_SimilarityResultsAlongOptima_{criterion}_greaterThan_{min_criterion}.pdf",
            bbox_inches="tight",
        )

    def plot_included_localizations(
        this, criterion="clusterSize", min_criterion=0
    ):
