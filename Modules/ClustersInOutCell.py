...
class ClustersInOutCell:
    ...
    def __init__(self, basefolder, algo='DbscanLoop', points_per_dimension=15):
        ...
        # Load Parameter file
        parameters = {'algo': algo, 'points_per_dimension': points_per_dimension}
        ...
        if os.path.isfile(basefolder + "X_incell_window.txt") and os.path.isfile(basefolder + "X_outcell_window.txt"):
    ...
    def GetSimilarityAlongOptima(self):
        ...
        if (os.path.exists(filename + '.pickle')):
            ...
            self.df_opt_th = GetLineOfOptima(self.phasespace_all[['sigma', 'threshold', 'no_clusters']], 'threshold',
                                             'no_clusters');
    ...
    def GetReferenceClustering(self, bestRequiredRate=1.0, computeSimilarityScores=False, generalLimit=True):
        ...
        # *********************************************
        phasespace_all_aboveT = DefineCleanedLabels(self.df_clusterSizes_all, self.phasespace_all,
                                                    criterion='clusterSize', bestRequiredRate=bestRequiredRate,
        ...
        if (computeSimilarityScores == True):
            cli_similarityScore, similarityScore = getSimilarityScore(self.XC_incell, phasespace_all_aboveT,
                                                                      clusterInfo_aboveT);
        ...
        self.df_opt_th_aboveT_ncl = GetLineOfOptima(
            self.phasespace_all_aboveT[['sigma', 'threshold', 'no_clusters', 'percent_locsIncluded']], 'threshold',
            'no_clusters');
    ...
    def plot_phasespace(this, ax, value="no_clusters", type="incell", criterion="clusterSize", min_criterion=0):
        ...
        df1 = this.df_clusterSizes_all[
            (this.df_clusterSizes_all[criterion] > min_criterion) & (this.df_clusterSizes_all['type'] == type)]
    ...
    def plot_clustersizes_along_optima(this, min_clustersize=30):
        ...
        df_in = df_clusters_opt_th[
            (df_clusters_opt_th['type'] == 'incell') & (df_clusters_opt_th['clusterSize'] >= min_clustersize)]
        df_out = df_clusters_opt_th[
            (df_clusters_opt_th['type'] == 'outcell') & (df_clusters_opt_th['clusterSize'] >= min_clustersize)]
        ...
        plt.tight_layout()
        plt.savefig(f"{this.save_name}_ClusterSize_ResultsAlongOptima_minClustersize{min_clustersize}.pdf",
                    bbox_inches="tight");
    ...
    def plot_gif_along_optima(this, type_, criterion='clusterSize', min_criterion=0):
        ...
        for idx, row in this.df_opt_th.iterrows():
            ...
            this.plot_scatter_selection(XC, this.phasespace_all.loc[idx, labels_name], df_clusters.loc[mark, :], ax,
                                        scalebar=True);
        ...
        filename = this.save_name + "_video_" + type_ + "_clusterings_min" + criterion + "_" + str(
            min_criterion) + ".gif"
    ...
    def plot_similarity_results_along_optima(this, criterion='clusterSize', min_criterion=0):
        ...
        df_in = this.df_clusters_opt_th[
            (this.df_clusters_opt_th['type'] == 'incell') & (this.df_clusters_opt_th[criterion] >= min_criterion)];
        df_out = this.df_clusters_opt_th[
            (this.df_clusters_opt_th['type'] == 'outcell') & (this.df_clusters_opt_th[criterion] >= min_criterion)];
        ...
        plt.tight_layout()
        plt.savefig(f"{this.save_name}_SimilarityResultsAlongOptima_{criterion}_greaterThan_{min_criterion}.pdf",
                    bbox_inches="tight");
