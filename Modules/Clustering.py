...
class Clustering:
    ...
    def fit(self, algo, params= None):
        ...
        if algo == "OPTICS":
            ...
            clustering_optics = OPTICS(min_samples=threshold, xi=xi, max_eps=eps_max, metric=euclidean,
                                       cluster_method='xi').fit(XC); # min_cluster_size=.05
    ...
    def plotScatter(self, filename):
        ...
        fig, ax = plt.subplots();
        mark = (labels == -1);
        sns.scatterplot(x=XC[mark, 0], y=XC[mark, 1], color='grey',   s=1,alpha=0.2);
        mark = (labels >= 0);
        sns.scatterplot(x=XC[mark, 0], y=XC[mark, 1], hue=labels[mark], palette='bright',
                        s=1, style=-1 * correct_detected[mark], legend=False);
        ax.set_aspect('equal');
