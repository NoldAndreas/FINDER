...
def GetOptimalT(
    ...
    for t in np.unique(df_incell[criterion]):
        no_clusters_over_t_outcell = np.sum(df_outcell[criterion] > t) / len(
            df_outcell[criterion]
        )
        no_clusters_over_t_incell = np.sum(df_incell[criterion] > t) / len(
            df_incell[criterion]
        )
        if (
            np.sum(df_outcell[criterion] > t)
            + np.sum(df_incell[criterion] > t)
            > min_noClustersToAnalyse
        ):
            incell_vs_outcell_probability.append(
                no_clusters_over_t_incell
                / (no_clusters_over_t_outcell + no_clusters_over_t_incell)
            )
            ts.append(t)
    ...
    return (
        T,
        incell_vs_outcell_probability_max,
        no_clusters_incell,
        no_clusters_outcell,
    )


def DefineCleanedLabels(
    df_clusterSizes_all,
    phasespace_all,
    criterion,
    bestRequiredRate,
    ...
    df_outcell = df_clusterSizes_all[
        (df_clusterSizes_all["type"] == "outcell")
    ]

    if generalLimit:
        T_, s_vs_n_, nocl_incell_, nocl_outcell_ = GetOptimalT(
            df_incell, df_outcell, criterion, bestRequiredRate=bestRequiredRate
        )
        print("T= " + str(T_))
    ...
    for idx, row in phasespace_all.iterrows():
        ...
        df_incell = df_clusterSizes_all[
            (df_clusterSizes_all["index"] == idx)
    ...
    phasespace_all_aboveT[
        "percent_locsIncluded_ref"
    ] = percent_locsIncluded_aboveT_ref
...
def GetOverlay(XC_incell, XC_outcell):
    ...
    if ("overlay_outcell" in parameters.keys()) and (
        parameters["overlay_outcell"] == 1
    ):
        density_ration_inoutcell = GetDensity(XC_incell) / GetDensity(
            XC_outcell
        )
        ...
        xis = np.linspace(
            np.min(XC_outcell[:, 0]), np.max(XC_outcell[:, 0]), n_x + 1
        )
        yis = np.linspace(
            np.min(XC_outcell[:, 1]), np.max(XC_outcell[:, 1]), n_y + 1
        )
        for i, xl in enumerate(xis[:-1]):
            ...
            for i, yl in enumerate(yis[:-1]):
                ...
                mark = (
                    mark & (XC_outcell[:, 1] >= yl) & (XC_outcell[:, 1] <= yr)
                )
                XC_paste = XC_outcell[mark, :] - [xl, yl]
                if np.random.rand() > 0.5:
                    XC_paste = XC_paste[:, [1, 0]]
                XC_outcell_overlay = np.concatenate(
                    (XC_outcell_overlay, XC_paste)
                )
            # break;
        #    break;
        ...
        print(
            "Ratio of localization densities: "
            + str(GetDensity(XC_incell) / GetDensity(XC_outcell))
        )
    else:
...
def PlotScatter(XC_, labels=[], ax=[], scalebar=False):
    ...
    mark_ = labels == -1
    ax.scatter(x=XC_[mark_, 0], y=XC_[mark_, 1], s=0.4, c="grey", alpha=0.1)
    ...
    if scalebar == True:
        ...
        xy = np.min(XC_, axis=0)
        delta_xy = np.max(XC_, axis=0) - np.min(XC_, axis=0)
        x = xy[0] + delta_xy[0] / 10
        y = xy[1] - delta_xy[1] / 10

        d = 0.5 * 1 / 0.158
        # d
        ax.plot([x, x + d], [y, y], "k", linewidth=2)
        ax.text(
            x + d / 2, y + delta_xy[1] / 40, "500nm", ha="center", va="center"
        )
...
def GetLineOfOptima(df, x_selector, y_selector, no_bins=0):
    ...
    if no_bins == 0:
        bins = np.asarray(
            [np.min(x_sel) - 1]
            + list((x_sel_sort[:-1] + x_sel_sort[1:]) / 2)
            + [np.max(x_sel) + 1]
        )
        no_bins = len(bins) - 1
    else:
        bins = np.linspace(
            0.99 * np.min(x_sel), np.max(x_sel) * 1.01, no_bins + 1
        )
...
def GetClusterDistribution(labels):
    label_vs_cl_size = np.array(
        [[l, np.sum(labels == l)] for l in np.unique(labels) if (l >= 0)]
    )
...
def AssembleStatistics(df_clusterSizes_):
    ...
    for t in np.unique(df_clusterSizes_["threshold"]):
        ...
        d_ = df_clusterSizes_.loc[
            df_clusterSizes_["threshold"] == t, "clusterSize"
        ]
        ...
        v1 = np.sum(dv_[(dv_.index < t + 1)])
        # /np.sum(dv_[(dv_.index < t+3)]);
        dict1["firstBin"] = v1
...
def AnalyseClusterSizes(df_clusterSizes, df_clusterSizes_ref, filename):
    ...
    df_stats_per_th = pd.concat(
        [df_stats_per_th_signal, df_stats_per_th_noise]
    )

    fig, axs = plt.subplots(3, 2, figsize=(12, 12))
    sns.lineplot(
        data=df_stats_per_th, x="threshold", y="cv", hue="type", ax=axs[0, 0]
    )
    sns.lineplot(
        data=df_stats_per_th, x="threshold", y="fano", hue="type", ax=axs[0, 1]
    )
    sns.lineplot(
        data=df_stats_per_th,
        x="threshold",
        y="skewness",
        hue="type",
        ax=axs[1, 0],
...
def PlotDistribution(df, df_ref, filename):
    ...
    for i, df_ in enumerate([df, df_ref]):
        ...
        sns.boxplot(
            data=df_, y="clusterSize", x="threshold", ax=ax, color="lightgrey"
        )
        ax.set_ylim(0, 80)
        ax.plot(
            np.arange(len(np.unique(df_["threshold"]))),
            np.unique(df_["threshold"]),
        )
