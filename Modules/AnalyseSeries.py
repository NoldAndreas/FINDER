...
from PlotScatter import PlotScatter, my_pal
...
def AnalyseSeries_PlotCases(filename):
    ...
    for data in all_data:
        ...
        filename_ = (
            ...
            + data.algo
            + "_Ncluster_"
            + str(data.Geometry.N_clusters)
            + "_Nnoise_"
            + str(data.Geometry.N_Noise)
            + "_seed_"
            + str(data.Geometry.seed)
            + ".pdf"
        )
...
def AnalyseSeries(df, params, filename):
    ...
    fig, axs = plt.subplots(
        nrows=3, ncols=2, gridspec_kw=gs_kw, figsize=(12, 12)
    )
    # gridspec_kw=gs_kw
    ...
    PlotScatter(
        all_data[0].Geometry.labels_groundtruth,
        all_data[0].Geometry.XC,
        ax=axs[0, 0],
    )
    PlotScatter(
        all_data[-1].Geometry.labels_groundtruth,
        all_data[-1].Geometry.XC,
        ax=axs[0, 1],
    )
