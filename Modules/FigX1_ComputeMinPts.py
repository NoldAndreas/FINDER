...
def PlotScatter(info):
    ...
    ax.set_aspect('equal');
    plt.savefig(outputfolder+"results"+str(info['threshold'])+".pdf",bbox_inches="tight");
...
def PlotScatterColorSize(info,size):
    ...
    ax = sns.scatterplot(x=XC[mark,0],y=XC[mark,1],hue=size[mark],hue_norm=norm,palette='vlag',linewidth=0,
                    s=2,ax=ax);
    ...
    ax.set_aspect('equal');
    plt.savefig(outputfolder+"results"+str(info['threshold'])+"_size.pdf",bbox_inches="tight");
...
def GetDensities(info,n):
    ...
    plt.savefig(outputfolder+"results"+str(info['threshold'])+"_densityClusters_vs_all.pdf",bbox_inches="tight");
#************************************************
#************************************************
...
if(False):
    ...
    with open(outputfolder+'temp.pickle','ab') as handle:
        pickle.dump({'labels':labels,'threshold':'CAML_87B144'}, handle,protocol=pickle.HIGHEST_PROTOCOL)    
elif(False):
    ...
    for threshold in thresholds:
        ...
        with open(outputfolder+'temp.pickle','ab') as handle:
            pickle.dump({'labels':labels,'threshold':threshold}, handle,protocol=pickle.HIGHEST_PROTOCOL)     
else:
