...
class CellSegmentation:
    ...
    def load_parameters(self):
        ...
        parameterfile = self.__get_inout_folder(
            inout="Input", filename="PARAMETERS_splitInOutCell.json"
        )
    ...
    def save_parameters(self):
        ...
        with open(
            self.__get_inout_folder(filename="PARAMETERS_splitInOutCell.json"),
            "w",
        ) as json_file:
    ...
    def get_image_from_localizations(self, xc, n_x, name):
        ...
        if self.output_mode >= 1:
            np.savetxt(
                self.__get_inout_folder(filename=name + "_H.txt"), h, fmt="%f"
            )
    ...
    def segmentation(self):
        ...
        h, x, y = self.get_image_from_localizations(
            xc, parameters["N_x"], "heatmap_XC"
        )
        ...
        if parameters["quantile_of_nonzero"]:
            cutoff_h = np.quantile(
                h[h > 0], parameters["intensity_quantile_cutoff"]
            )
        else:
