    ...
    for i, ps in PS.iterrows():
        ...
        for j in PS.index[PS.index <= i]:#np.arange(i+1):
            ...
            if(not (i==j)):
                ...
                s_ij = getSimilarityScore_ij(i,j,PS,clusterInfo);#centers,radii);
                ...
                similarityScoreMatrix[ps_index==j,ps_index==i] = score; #/Normalize here?  eg  /np.int(np.max(PS.loc[j,"labels"]) + 1)
                similarityScoreMatrix[ps_index==i,ps_index==j] = score; #/Normalize here?  eg  /np.int(np.max(PS.loc[i,"labels"]) + 1)
            else:
                # we do not include similarity with self 
                None
                #0.5, because 
                #cli_similarityScore[mark_i] += 1
                #cli_similarityScore[mark_j] += 1

                #number_labels_unique       = len([l for l in np.unique(ps['labels']) if (l != -1)]);
                #similarityScoreMatrix[ps_index==i,ps_index==j] = number_labels_unique;#np.max(ps["labels"]) + 1;
