...
def clusterEvaluation(labels, labels_groundTruth, min_overlap_per_ref):
    ...
    clusters_ref = [
        np.where(labels_groundTruth == i)[0] for i in np.arange(no_clusters_gT)
    ]
...
def analyseRatios(cl_, ratios):
    ...
    for i, ratio_ in enumerate(ratios):
        clusters_correct = clusterEvaluation(
            labels, labels_groundtruth, ratio_
        )
...
fig, axs = plt.subplots(nrows=3, ncols=6, gridspec_kw=gs_kw, figsize=(14, 8))
# gridspec_kw=gs_kw
...
for i in np.arange(6):
    ...
    for cl_ in all_data:
        ...
        for ii, tp in enumerate(true_pos_):
            ...
            if x_name == "Delta_ratio":
                data_eval["Delta_ratio"].append(
                    cl_.Geometry.parameters["Delta_ratio"]
                )
            elif x_name == "noise_ratio":
    ...
    sns.lineplot(
        data=df[mark], x="ratio", y="true_pos", hue="algo", ax=axs[0, i]
    )
    sns.lineplot(
        data=df[mark], x="ratio", y="false_pos", hue="algo", ax=axs[1, i]
    )
