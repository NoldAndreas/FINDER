...
for library_type in ["Clusters_Neuron", "Clusters_DNA_1mers"]:
    ...
    G = Geometry_Grid(
        basefolder, library_type, n_side=0, Delta_ratio=1, noise_ratio=0
    )
    ...
    ax = axs[0]
    sns.histplot(sizes, bins=20, ax=axs[0])
    # ,norm_hist=True);
    ...
    sns.scatterplot(
        x=XC[:, 0], y=XC[:, 1], color="grey", alpha=alpha, ax=axs[1]
    )
    ...
    print(
        "Larges cluster diameter: "
        + str(G.GetTypicalDiameter_of_templateClusters())
    )

    plt.savefig(
        basefolder + "/FigS1_Library_" + library_type + ".pdf",
        bbox_inches="tight",
    )
    print(
        "Figure saved in: "
