...
def PlotDistribution(labels,ax,col):
    ...
    print("Noise localizations: "+str(np.sum(labels==-1))+" or "+str(np.round(100*np.sum(labels==-1)/len(labels),2))+" %");
    print("Clusters with size >= "+str(edges_max_clusterSizeDist)+": "+str(np.sum((cluster_sizes>edges_max_clusterSizeDist))));
    print("Number of localzations in clusters with size >= "+str(edges_max_clusterSizeDist)+' : '+str(locs_in_large_clusters)+
...
def PlotScatter(labels,XC,markPlot,ax=[],ax_dist=[],col='grey',showScaleBar=False,showBorder=False,highlights=[]):
...
#    sns.scatterplot(x=XC[mark,0],y=XC[mark,1],linewidth=0,color='grey',alpha=0.2,ax=ax,legend=False);
 ...
 #   ax.scatter(x=XC[mark,0],y=XC[mark,1],marker='o',s=1,c=cols,alpha=0.1,edgecolor=None,linewidth=0);    
    sns.scatterplot(x=XCplot[mark,0],y=XCplot[mark,1],hue=labelsPlot[mark],palette='deep',linewidth=0,
...
if(computeClustering):
    ...
    np.savetxt(results_folder+case+'_labels_FINDER.txt',labels_FINDER,fmt="%.0f", header='FINDER time: '+str(time.time()-t1)+' seconds');            
...
if(plot10PtDistance):
   ...
   # ax.plot(r,h_ripley);  
    ...
    ax.axhline(sigma_selected_FINDER,c=my_pal['FINDER_1D_loop'],linestyle='dashed');
    ...
    ax.axvline(sigma_selected_FINDER,c=my_pal['FINDER_1D_loop'],linestyle='dashed')
...
if(plotMainAnalysis):
    ...
    PlotScatter(labels_FINDER,XC,markPlot,ax=axs[0,0],ax_dist=axs[1,0],col=my_pal['FINDER_1D_loop'],showScaleBar=True,highlights=highlights);  
    axs[0,0].set_title('FINDER \n (minPts = 10, r = '+str(np.round(sigma_selected_FINDER,1))+'nm)');   
    
    PlotScatter(labels_CAML_07VEJJ,XC,markPlot,ax=axs[0,1],ax_dist=axs[1,1],col=my_pal['CAML_07VEJJ'],highlights=highlights);  
    axs[0,1].set_title('CAML (07VEJJ)\n ');   
    axs[1,1].set_ylabel('');
    axs[1,1].set_yticklabels(["",""]);    
    
    PlotScatter(labels_CAML_87B144,XC,markPlot,ax=axs[0,2],ax_dist=axs[1,2],col=my_pal['CAML_87B144'],highlights=highlights);  
