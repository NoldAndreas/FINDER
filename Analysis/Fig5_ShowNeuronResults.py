...
def PlotDistribution(labels, ax, col):
    ...
    locs_in_large_clusters = np.sum(
        cluster_sizes[cluster_sizes > edges_max_clusterSizeDist]
    )
    ...
    print(
        ...
        + str(np.sum(labels == -1))
        + " or "
        + str(np.round(100 * np.sum(labels == -1) / len(labels), 2))
        + " %"
    )
    print(
        "Clusters with size >= "
        + str(edges_max_clusterSizeDist)
        + ": "
        + str(np.sum((cluster_sizes > edges_max_clusterSizeDist)))
    )
    print(
        "Number of localzations in clusters with size >= "
        + str(edges_max_clusterSizeDist)
        + " : "
        + str(locs_in_large_clusters)
        + " or "
        + str(100 * locs_in_large_clusters / len(labels))
        + " %"
    )
...
def PlotScatter(
    ...
    mark = labelsPlot == -1
    ax.scatter(
        x=XCplot[mark, 0], y=XCplot[mark, 1], s=0.4, c="grey", alpha=0.1
    )
    ...
    if showScaleBar:
        ...
        ax.annotate(str_, (x_0 + dx / 2, y_0 + dy / 3), ha="center")
        # fontsize='normal'
    else:
    ...
    for i_, highlight in enumerate(highlights):
        circle_ = plt.Circle(
            highlight["xy"], highlight["R"], color="r", fill=False
        )
        ax.add_patch(circle_)
        pos_ = np.asarray(highlight["xy"]) + highlight["R"] * 0.8 * np.asarray(
            [1, 1]
        )
        ax.annotate(
            labs[i_],
...
def PlotImageNoNoise(XC, labels, name):
    ...
    ax.plot([x_0, x_0 + dx], [y_0, y_0], "k")
    ax.annotate(str_, (x_0 + dx / 2, y_0 + 1000), ha="center")
    # fontsize='normal
    ...
    plt.savefig(
        results_folder + "/" + case + "_overview_" + name + "_noNoise.png",
        dpi=300,
        bbox_inches="tight",
    )
...
if case == "neuron":
    ...
    highlights.append(
        {"xy": [6070 + 158 * 210, 580 + 158 * 30 - 500], "R": 600}
    )
    highlights.append(
        {"xy": [8200 + 158 * 210, 2000 + 158 * 30 - 500], "R": 800}
    )
else:
    lims_x = [-np.Inf, np.Inf]
    lims_y = [-np.Inf, np.Inf]
...
if computeClustering:
    ...
    np.savetxt(
        results_folder + "/" + case + "_labels_DBSCAN.txt",
        labels_DBSCAN,
        fmt="%.0f",
    ...
    t1 = time.time()
    labels_CAML_07VEJJ = Clustering_CAML(
        "CAML_07VEJJ", XC, datafolder=basefolder
    )
    print("Compute time CAML_07VEJJ: " + str(time.time() - t1) + " seconds")
    np.savetxt(
        results_folder + "/" + case + "_labels_CAML_07VEJJ.txt",
        labels_CAML_07VEJJ,
        fmt="%.0f",
    ...
    t1 = time.time()
    labels_CAML_87B144 = Clustering_CAML(
        "CAML_87B144", XC, datafolder=basefolder
    )
    print("Compute time CAML_87B144: " + str(time.time() - t1) + " seconds")
    np.savetxt(
        results_folder + "/" + case + "_labels_CAML_87B144.txt",
        labels_CAML_87B144,
        fmt="%.0f",
    ...
    np.savetxt(
        results_folder + "/" + case + "_labels_FINDER.txt",
        labels_FINDER,
        fmt="%.0f",
    ...
    parameters_selected_Finder = [
        sigma_selected_FINDER,
        threshold_selected_FINDER,
    ]

    np.savetxt(
        results_folder + "/" + case + "_FINDER_optimalSigma.txt",
        parameters_selected_Finder,
        fmt="%f",
    ...
    labels_all = np.asarray(
        [labels_CAML_87B144, labels_DBSCAN, labels_FINDER, labels_CAML_07VEJJ]
    ).T
    np.savetxt(
        results_folder + "/" + case + "_labels.txt",
        labels_all,
        fmt="%.0f %.0f %.0f %.0f",
    ...
    labels_FINDER = np.loadtxt(
        results_folder + "/" + "protein_ttx_1hr_2_labels_FINDER.txt"
    )  # labels_all[:,2];
    labels_CAML_07VEJJ = labels_all[:, 3]

    params_FINDER = np.loadtxt(
        results_folder + "/" + case + "_FINDER_optimalSigma.txt"
    )
...
if plotFullImage:
    ...
    plt.savefig(
        results_folder + "/" + case + "_overview.png",
        dpi=300,
        bbox_inches="tight",
    )
...
if plot10PtDistance:
    ...
    ax.axhline(
        sigma_selected_FINDER, c=my_pal["FINDER_loop"], linestyle="dashed"
    )
...
if False:
    ...
    gs_kw = dict(
        width_ratios=[1, 1, 1], height_ratios=[4, 1], hspace=-0.5, wspace=0.1
    )
    ...
    for i, sigma in enumerate(sigmas):
        ...
        ax = axs[0, i]
        PlotScatter(
            labels_, XC, ax=ax, ax_dist=axs[1, i], showScaleBar=(i == 0)
        )
        ax.set_title(
            "DBSCAN (noisefree)\n minPts = 10, r = "
    ...
    plt.savefig(
        results_folder + "/" + case + "_DBSCAN_examples.pdf",
        bbox_inches="tight",
    )
    print(
        "File saved in : "
...
if plotMainAnalysis:
    # Step 3b: Plot
    gs_kw = dict(
        width_ratios=[1, 1, 1], height_ratios=[3, 1], hspace=0.0, wspace=0.1
    )
    ...
    PlotScatter(
        ...
        ax=axs[0, 0],
        ax_dist=axs[1, 0],
        col=my_pal["FINDER_loop"],
        showScaleBar=True,
        highlights=highlights,
    )
    axs[0, 0].set_title(
        "FINDER \n (eps ="
        + str(np.round(sigma_selected_FINDER, 2))
        + "nm"
        + ", minPts = "
        + str(np.round(threshold_selected_FINDER, 1))
        + ")"
    )
    ...
    plt.savefig(
        results_folder + "/" + case + "_results.pdf", bbox_inches="tight"
    )
