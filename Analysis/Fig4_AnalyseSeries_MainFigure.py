...
my_pal = {
    "CAML_07VEJJ": "#eabe8e",
    "CAML_87B144": "#d67d1d",  #'FINDER_1D_loop':'#701ac0',\
    #'FINDER_1D':'#af6eeb', \
    "FINDER_loop": "#701ac0",
    "FINDER": "#af6eeb",
    "dbscan": "dimgrey",
...
def PlotScatter(labels, XC, ax=[], optionScaleBarText=False):
    ...
    if optionScaleBarText:
        ax.annotate(
            "$200nm$", (x_0 + 100, y_0 + 25), fontsize="large", ha="center"
        )
...
def AnalyseSeries(df, params, filename, axs, optionScaleBarText):
    ...
    if x_name == "noise_ratio":
        ...
        x_to_display = np.asarray(df[x_name])[-1]
        # We select last instance, i.e. case with highest perturbation
        data_to_display = all_data[-1]
    mark = df[x_name] == x_to_display
    df = df[mark]

    PlotScatter(
        data_to_display.Geometry.labels_groundtruth,
        data_to_display.Geometry.XC,
        ax=axs[0],
        optionScaleBarText=optionScaleBarText,
    )
...
dict_ylabel = {
    "true_positives_ratio": "True positives (ratio)",
    "false_positives_ratio": "False positives (ratio)",
    "compute_time": "Computation time [seconds]",
}

dict_xlabel = {
    "noise_ratio": "Noise vs cluster localizations",
...
gs_kw = dict(
    ...
    height_ratios=[4, 4, 4, 0.5],
)
fig, axs = plt.subplots(
    nrows=4, ncols=len(filenamesList), gridspec_kw=gs_kw, figsize=(16, 9)
)
# gridspec_kw=gs_kw
...
for i, filename in enumerate(filenamesList):
    ...
    boxplot_return = AnalyseSeries(
        df, params, basefolder + filename[:-4], axs[:, i], optionScaleBarText
    )
