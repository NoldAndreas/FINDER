...
OUTPUTFOLDER = get_datafolder(
    os.path.join("Data_AnalysisOrganized", "Fig3_3mers", "Output")
)
...
def define_window(xc_incell, xc_outcell, n_select, outputfolder):
    ...
    # Select windows
    xc_incell_w, xy_center_incell, r_incell = select_points(
        xc_incell, n_select
    )
    xc_outcell_w, xy_center_outcell, r_outcell = select_points(
        xc_outcell, n_select
    )

    # save points
    np.savetxt(
        outputfolder_window + "X_incell_window.txt", xc_incell_w, fmt="%f"
    )
    np.savetxt(
        outputfolder_window + "X_outcell_window.txt", xc_outcell_w, fmt="%f"
    )

    # save PARAMETERFILE
    parameters_window = {
        "xy_center_incell": (np.float64(xy_center_incell)).tolist(),
...
OUTPUTFOLDER_WINDOW = define_window(
    XC_INCELL, XC_OUTCELL, N_SELECT, OUTPUTFOLDER
)

CB = ClustersInOutCell(
    OUTPUTFOLDER_WINDOW, points_per_dimension=points_per_dimension
)
