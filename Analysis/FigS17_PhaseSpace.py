...
for case in ["neuron", "ttx"]:
    if do_computation == True:
        if case == "neuron":
            XC = np.loadtxt(
                data_folder
                + "/TemplateClusters/NeuronData/dendrite_example_Cell1_GluA2_40I_ROI1_1_MMStack_Pos0.ome_locs_render_driftCorr_filter_render_pix.6fr20_picked2_picked3.txt"
            )
        elif case == "ribosome":
            XS = np.loadtxt(
                data_folder + "/TemplateClusters/RibosomeData/40s.txt"
            )
            mark = (
                (XS[:, 0] > 360)
                * (XS[:, 0] < 380)
        ...
        if case == "neuron":
            pkl_path = os.path.join(
                data_folder, "phase_spaces", "neuron_phasespace.pkl"
            )
            ...
            neuron_path = os.path.join(
                data_folder, "TemplateClusters", "NeuronData", neuron_name
            )
        ...
        if case == "ttx":
            pkl_path = os.path.join(
                data_folder, "phase_spaces", "protein_ttx_1hr_2_phasespace.pkl"
            )
            if not os.path.isfile(pkl_path):
                print(
                    "The file'protein_ttx_1hr_2_phasespace.pkl' was not uploaded in this directory because it is too large."
                )
            ...
            filename = os.path.join(
                data_folder,
                "TemplateClusters",
                "ProteinData_ttx_1hr_2",
                ttx_name,
            )
            if not os.path.isfile(filename):
                print(
                    "The file for 'ProteinData_ttx_1hr_2' was not uploaded in this directory because it is too large."
                )
                print("Please contact us if you need to use it")
    ...
    plt.savefig(
        data_folder + "/Results/FigS17_18_a_" + case + "_Phasediagram.pdf",
        bbox_inches="tight",
    )
    print(
        "File saved in : "
        + data_folder
        + "/Results/FigS17_18_a_"
        + case
        + "_Phasediagram.pdf"
    )
    ...
    for ii, si in enumerate(sigmas_idx):
        for jj, mi in enumerate(minPts_idx):
            ...
            mark = labels == -1
            sns.scatterplot(
                x=XC[mark, 0], y=XC[mark, 1], color="grey", alpha=0.2, ax=ax
            )
            mark = labels >= 0
            sns.scatterplot(
                x=XC[mark, 0],
    ...
    plt.savefig(
        data_folder + "/FigS17_18_b_" + case + "_ClusteringResults.pdf",
        bbox_inches="tight",
    )
    print(
        "File saved in : "
