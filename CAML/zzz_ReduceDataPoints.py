...
if __name__ == "__main__":
    ...
    if reduction_method in ["1"]:
        ...
        default_outfolder = os.path.join(
            inputpath, "Reduced_" + str(max_points_per_set) + "_Pts_Max"
        )
        ...
        fraction_points_per_set = fn_etc.askforinput(
            ...
            defaultval="0.5",
            isvalid=lambda v: v.replace(".", "").isdigit()
            and float(v) > 0
            and float(v) < 1,
        )
        ...
        default_outfolder = os.path.join(
            inputpath, "Reduced_" + str(fraction_points_per_set) + "x_Pts"
        )
    ...
    for fileIdx in range(starting_index, finishing_index):
        ...
        if fraction_points_per_set > 0:
            max_points_per_set = int(
                fraction_points_per_set * TotalPointsThisImage
            )

        if TotalPointsThisImage <= max_points_per_set:
            print(
                "This file has the same or fewer points than the specified maximum ("
                + str(max_points_per_set)
                + ") points. It will be copied to the output folder as-is."
            )
            datatable_reduced = datatable
            reduced_output_fname = os.path.join(
                s1_prep_outputpath,
                output_prefix + "_copied" + ps["InputFileExt"],
            )
        else:
            print(
                "Choosing "
                + str(max_points_per_set)
                + " points for the reduced dataset ("
                + str(
                    round((max_points_per_set / TotalPointsThisImage) * 100, 2)
                )
                + "% of original total points)"
            )
            # keep a random subset of the points up to the maximum number of points specified
            keepers_idx = np.random.choice(
                datatable.shape[0], max_points_per_set, replace=False
            )
            datatable_reduced = datatable[keepers_idx]
            reduced_output_fname = os.path.join(
                s1_prep_outputpath,
                output_prefix + "_reduced" + ps["InputFileExt"],
            )

        # save the reduced datatable to the output folder
        print("Saving reduced dataset...", end="", flush=True)
        np.savetxt(
            reduced_output_fname,
            datatable_reduced,
            delimiter=ps["InputFileDelimiter"],
            header=ps["TableHeaders"],
            fmt="%s",
        )
        print("Done")
