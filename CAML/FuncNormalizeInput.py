...
def normalize_dists(X_distances_raw, total_X_distances, PreparationType):
    ...
    if X_distances_raw.shape[0] > 5 * chunk_width:
        ...
        print(
            "Large matrix - using MemMap on disk (may be slower to process)."
        )
    ...
    if ProcAsChunks:
        ...
        if os.path.exists(temp_data_fname):
            os.unlink(
                temp_data_fname
            )  # Check for existing temp file and delete it if it exists
    ...
    if PreparationType == "Norm-Self":
        ...
        if ProcAsChunks:
            for chunk in chunk_idx:
                X_data_normalized[chunk : chunk + chunk_width, 1:] = np.diff(
                    X_distances_raw[chunk : chunk + chunk_width, :], axis=1
                )
        else:
        ...
        if ProcAsChunks:
            for chunk in chunk_idx:
                X_data_normalized[chunk : chunk + chunk_width, :] = (
                    X_data_normalized[chunk : chunk + chunk_width]
                    - X_train_min[chunk : chunk + chunk_width, None]
                ) / X_data_range[chunk : chunk + chunk_width, None]
        else:
            X_data_normalized = (
                X_data_normalized - X_train_min[:, None]
            ) / X_data_range[:, None]
        ...
        X_data_concat = np.concatenate(
            (
                X_distances_raw[:, 0].reshape(total_X_distances, 1),
                X_distance_differences,
            ),
            axis=1,
        )
        ...
        # Add back the first column (distance from self to 1st neighbour)
        X_data_normalized = np.concatenate(
            (
                X_distances_raw[:, 0].reshape(total_X_distances, 1),
                X_distance_differences,
            ),
        ...
        X_data_normalized = X_data_normalized / np.min(
            X_data_normalized[np.nonzero(X_data_normalized)], axis=0
        )  # TODO
        ...
        X_dists_normd_rescaled = np.array(
            X_dists_normalized * 100000, np.int32
        )
        ...
        X_data_normalized = np.concatenate(
            (
                X_dists_normd_rescaled[:, 0].reshape(total_X_distances, 1),
                X_dists_normd_scaled_diffs,
            ),
            axis=1,
        )
        ...
        # Add back the first column (distance from self to 1st neighbour)
        X_dists_diffs_addback = np.concatenate(
            (
                X_distances_raw[:, 0].reshape(total_X_distances, 1),
                X_distance_differences,
            ),
        ...
        X_dists_diffs_normalized = preprocessing.normalize(
            X_dists_diffs_addback
        )

        # Rescale the normalised values
        X_data_normalized = np.array(
            X_dists_diffs_normalized * 100000, np.int32
        )
        ...
        # Add back the first column (distance from self to 1st neighbour)
        X_dists_diffs_normalized = np.concatenate(
            (
                X_distances_raw[:, 0].reshape(total_X_distances, 1),
                X_distance_differences,
            ),
    ...
    ##        SumNNdists = np.sum(X_distances_raw[ps['ClosesestFriend']:sumNNs,:])
        ...
        raise ValueError(
            "Unsure how to prepare the input data. Check PreparationType setting..."
        )
