...
def MakeCellOutline(ImageSize, Create_cSMAC, dname, fname, SavePreview):
    ...
    cell_vert_radius = cell_radius + cell_rad_var * np.random.random(
        len(cell_vert_angle)
    )
    ...
    cell_bounds_x = np.concatenate(
        (cell_bounds_x, cell_bounds_x[0, None]), axis=0
    )
    cell_bounds_y = np.concatenate(
        (cell_bounds_y, cell_bounds_y[0, None]), axis=0
    )
    ...
    x_interp, y_interp = interpolate.splev(
        np.linspace(0, 1, TotalInterpPts), tck
    )

    cell_bounds = np.concatenate(
        (x_interp[:, None], y_interp[:, None]), axis=1
    )
    ...
    if Create_cSMAC:
        ...
        csmac_vert_angle = np.linspace(
            0, 2 * np.pi, csmac_verts, endpoint=False
        )
        csmac_vert_radius = csmac_radius + csmac_rad_var * np.random.random(
            len(csmac_vert_angle)
        )

        csmac_bounds_x = cell_centreX + csmac_vert_radius * np.cos(
            csmac_vert_angle
        )
        csmac_bounds_y = cell_centreY + csmac_vert_radius * np.sin(
            csmac_vert_angle
        )

        # close the shape by copying the first five entries to the end
        csmac_bounds_x = np.concatenate(
            (csmac_bounds_x, csmac_bounds_x[0, None]), axis=0
        )
        csmac_bounds_y = np.concatenate(
            (csmac_bounds_y, csmac_bounds_y[0, None]), axis=0
        )

        # interpolate a smooth boundary
        cSMACTotalInterpPts = int(InterpOversample * csmac_verts)
        csmac_tck, u = interpolate.splprep(
            [csmac_bounds_x, csmac_bounds_y], s=0.0
        )
        x_csmac_interp, y_csmac_interp = interpolate.splev(
            np.linspace(0, 1, cSMACTotalInterpPts), csmac_tck
        )

        csmac_bounds = np.concatenate(
            (x_csmac_interp[:, None], y_csmac_interp[:, None]), axis=1
        )
    else:
    ...
    if SavePreview:
        ...
        plt.savefig(
            os.path.join(dname, fname + " - cell outline.png"),
            dpi=300,
            bbox_inches=0,
            facecolor="none",
            edgecolor="none",
    ...
    pickle.dump(
        (cell_bounds, csmac_bounds),
        open(os.path.join(dname, fname + " - cell outline.pkl"), "wb"),
        protocol=4,
    )
...
def SprinklePoints(
    ...
    if doImages:
        ...
        plt.scatter(
            shrunk_cell_boundary[:, 0],
            shrunk_cell_boundary[:, 1],
            s=2,
            color="grey",
        )
        ax.set_xlim(0, ImageSize[0])
        ...
        if UsecSMAC:
            plt.scatter(
                csmac_bounds[:, 0], csmac_bounds[:, 1], s=1, color="red"
            )
    ...
    if ptsperclu > 0 and cluperum2 > 0 and percentptsinclu > 0:
        ...
        #            Cluster_Seeds[1:NonAssocClusterCount,:] = tmp_NonAssocSeeds # replace the existing supplied seeds with the random ones.
            ...
            if UsecSMAC:
                Clusters_Required = int(
                    np.floor(((SimuCellArea - cSMACArea) / 1e6) * cluperum2)
                )
            else:
                Clusters_Required = int(
                    np.floor((SimuCellArea / 1e6) * cluperum2)
                )
            ...
            while GoodFirstSeed == False:
                FirstClusterSeed_x = (
                    np.random.randint(
                        cluster_x_min, high=cluster_x_max, size=1
                    )
                    * coord_res
                )
                FirstClusterSeed_y = (
                    np.random.randint(
                        cluster_y_min, high=cluster_y_max, size=1
                    )
                    * coord_res
                )
                if three_dee:
                    FirstClusterSeed_z = (
                        np.random.randint(
                            cluster_z_min, high=cluster_z_max, size=1
                        )
                        * coord_res
                    )
                else:
                    FirstClusterSeed_z = np.array([cluster_z_max])
                FirstClusterSeed = np.concatenate(
                    (
                        FirstClusterSeed_x[None, :],
                        FirstClusterSeed_y[None, :],
                        FirstClusterSeed_z[None, :],
                    ),
                    axis=1,
                )
                GoodFirstSeed = seed_boundary.contains_points(
                    FirstClusterSeed[:, :2]
                )[0]
                if GoodFirstSeed and UsecSMAC:
                    FirstIncSMAC = csmac_boundary.contains_points(
                        FirstClusterSeed[:, :2]
                    )[0]
                    DiceRoll = np.random.rand(1)[0]
                    if (
                        DepleteClustersFromcSMAC
                        and FirstIncSMAC
                        and DiceRoll < DepleteClustersFraction
                    ):
                        GoodFirstSeed = False
            ...
            while GoodSeeds < Clusters_Required:
                ...
                while ProbationaryCluster and AllocationAttempt < 10:
                    ...
                    ProposedClusterSeed_x = (
                        np.random.randint(
                            cluster_x_min, high=cluster_x_max, size=1
                        )
                        * coord_res
                    )
                    ProposedClusterSeed_y = (
                        np.random.randint(
                            cluster_y_min, high=cluster_y_max, size=1
                        )
                        * coord_res
                    ...
                    ProposedClusterSeed = np.concatenate(
                        (
                            ProposedClusterSeed_x[None, :],
                            ProposedClusterSeed_y[None, :],
                            ProposedClusterSeed_z[None, :],
                        ),
                        axis=1,
                    )

                    # check if it has fallen inside the cell shape
                    ProposedInside = seed_boundary.contains_points(
                        ProposedClusterSeed[:, :2]
                    )[0]

                    # check if we are in the cSMAC
                    if ProposedInside and UsecSMAC:
                        ProposedIncSMAC = csmac_boundary.contains_points(
                            ProposedClusterSeed[:, :2]
                        )[0]
                        DiceRoll = np.random.rand(1)[0]
                        if (
                    ...
                    if (
                        ProposedInside
                    ):  # check we are not too close to any other seed
                        # get distances to other seeds
                        otherseeds = SeedBank[:GoodSeeds, :]
                        seed_dists = distance.cdist(
                            ProposedClusterSeed, otherseeds
                        )

                        if (
                            np.min(seed_dists) > 2.2 * rad_max
                        ):  # seed must be at least 4xRadius away plus a small buffer to force clear segregation of finished clusters
                            SeedBank[
                                ProposedSeedID - 1, :
                            ] = ProposedClusterSeed
            ...
            Cluster_Seeds = SeedBank[
                :Clusters_Required, :
            ]  # copy the valid seeds
        ...
        TotalInClusterPoints = int(
            np.ceil(ptsperclu * Clusters_Required)
        )  # our in-cluster points are clusters * points-per-cluster.
        TotalEvents = int(
            np.ceil((TotalInClusterPoints / (percentptsinclu / 100)))
        )  # if x percent of points are in-clusters then 1/x points are there in total.
        TotalExClusterPoints = (
            TotalEvents - TotalInClusterPoints
        )  # our ex-cluster points are therefore the difference.

        NCPointsList = np.zeros(
            (1, 3)
        )  # init using empty gives bizarre results so we init the array with a single entry and delete it later
        ...
        if TotalExClusterPoints > 0:
            ...
            while NCPointsRemaining != 0:
                ...
                if NCPointsRemaining > 0:
                    ...
                    Generating_NCPoints = int(
                        NCPointsRemaining * OvercompensationFactor
                    )

                    NCPointsList_x = (
                        np.random.randint(
                            noncluster_x_min,
                            high=noncluster_x_max,
                            size=(Generating_NCPoints, 1),
                        )
                        * coord_res
                    )
                    NCPointsList_y = (
                    ...
                    if three_dee:
                        NCPointsList_z = (
                            np.random.randint(
                                noncluster_z_min,
                                high=noncluster_z_max,
                                size=(Generating_NCPoints, 1),
                            )
                            * coord_res
                        )
                    else:
                        NCPointsList_z = np.full(
                            (Generating_NCPoints, 1), noncluster_z_max
                        )
                    NCPtsProposed = np.concatenate(
                        (NCPointsList_x, NCPointsList_y, NCPointsList_z),
                        axis=1,
                    )

                    # Find events within the cell RoI
                    NCPoints_In_Cell = cell_boundary.contains_points(
                        NCPtsProposed[:, :2]
                    )
                    ...
                    unique_NCpts = np.logical_not(
                        np.isin(NCPtsProposed, NCPointsList).all(axis=1)
                    )
                    ...
                    if np.sum(unique_NCpts) < NCPtsProposed.shape[0]:
                        print(
                            "\n\t***\t"
                            + str(
                                NCPointsList.shape[0] - unique_NCpts.shape[0]
                            )
                            + " Proposed NC points with duplicate xy coordinates\n"
                        )

                    # deplete points in the SMAC regions, as required
                    if UsecSMAC and NCPtsProposed.shape[0] > 0:
                        DiceRoll = (
                            np.random.rand(NCPtsProposed.shape[0], 1)
                            > DepleteNCPointsFraction
                        )
                        if DepleteNCPointsFromcSMAC:
                            KeepNCPoints_SafeZone = np.invert(
                                csmac_boundary.contains_points(
                                    NCPtsProposed[:, :2]
                                )
                            )  # True for points outside cSMAC
                        else:
                            KeepNCPoints_SafeZone = (
                                csmac_boundary.contains_points(
                                    NCPtsProposed[:, :2]
                                )
                            )  # True for points inside the cSMAC
                        KeepNCPoints = np.array(
                            [
                                x | y | z[0]
                                for (x, y, z) in zip(
                                    KeepNCPoints_SafeZone, DiceRoll
                                )
                            ]
                        )  # True for points in the safe zone OR points surviving dice-roll
                    ...
                    if NCPtsProposed.shape[0] > 0:
                        NCPointsInAnyCluster = np.zeros(
                            (NCPtsProposed.shape[0], Cluster_Seeds.shape[0]),
                            dtype=bool,
                        )
                        for pidx, polygon in enumerate(Cluster_Patches):
                            poly_coords = np.array(polygon.exterior.xy).T
                            p = Path(poly_coords)
                            NCPointsInAnyCluster[:, pidx] = p.contains_points(
                                NCPtsProposed[:, :2]
                            )

                        KeptPoints = np.invert(
                            np.any(NCPointsInAnyCluster, axis=1)
                        )
                        NCPtsProposed = NCPtsProposed[KeptPoints, :]
                        NCPointsList = np.concatenate(
                            (NCPointsList, NCPtsProposed)
                        )

                        # remove duplicate xy points in NCPointsList
                        _, uniq_idx = np.unique(
                            NCPointsList, axis=0, return_index=True
                        )
                        if uniq_idx.shape[0] < NCPointsList.shape[0]:
                            ...
                            print(
                                "\n\t***\t3 - "
                                + str(
                                    NCPointsList.shape[0] - uniq_idx.shape[0]
                                )
                                + " NC points with duplicate xy coordinates\n"
                            )

                        NCPointsRemaining = int(
                            TotalExClusterPoints - NCPointsList.shape[0]
                        )  # re-test for sufficient number of events

                    else:
                        print(
                            "managed to remove all generated points. this may become a problem!"
                        )

                    NCPtsProposed = None
                    OvercompensationFactor = (
                        2 * OvercompensationFactor
                    )  # bump up the overcompensation for the next round lest we get stuck in a situation generating only a few points which don't land where we want them

                    if OvercompensationFactor > 100:
                        raise ValueError(
                            "Taking to long to position NC Points in this clustered image. Please check your clustering parameters."
                        )
                    ...
                    if uniq_idx.shape[0] < NCPointsList.shape[0]:
                        ...
                        print(
                            "\n\t***\t4A - "
                            + str(NCPointsList.shape[0] - uniq_idx.shape[0])
                            + " NC points with duplicate xy coordinates\n"
                        )

                    # random permutation of the NCPointsList indices, minus 1 to take account of the init row, then plus one to avoid index=0
                    RandomKeeperIdx = (
                        np.random.permutation(NCPointsList.shape[0] - 1) + 1
                    )
                    RandomKeeperIdx = np.concatenate(
                        ([0], np.sort(RandomKeeperIdx[:TotalExClusterPoints]))
                    )
                    ...
                    NCPointsRemaining = int(
                        TotalExClusterPoints - RandomKeeperIdx.shape[0] + 1
                    )  # re-test for sufficient number of events to exit the while-loop
        ...
        NCPointsList = np.concatenate(
            (NCPointsList, np.zeros((NCPointsList.shape[0], 2))), axis=1
        )

        if doImages:
            plt.scatter(
                NCPointsList[:, 0], NCPointsList[:, 1], s=2, color="cyan"
            )

        # Add events around the cluster seeds
        ClusteredPointsList = np.zeros(
            (TotalInClusterPoints, 5)
        )  # init an empty array: [ x, y, z, clusID, Label ] We will ignore this first row later.
        InsertionPoint = 0
        for clusterID in range(Clusters_Required):

            tmp_pts_to_generate = ptsperclu
            ClusteredPointsList_final = np.zeros(
                (1, 5)
            )  # x, y, z, cluster UID, Label

            while tmp_pts_to_generate > 0:

                ClusteredPointsList_tmp = np.zeros(
                    (tmp_pts_to_generate, 5)
                )  # x, y, z, cluster UID, Label
                ...
                if DistributionType == "Uniform":
                    ...
                    min_dist = int(
                        np.floor(((rad_min + Edge_fuzzing) ** 2 / coord_res))
                    )
                    max_dist = int(
                        np.floor(((rad_max - Edge_fuzzing) ** 2 / coord_res))
                    )
                    CluPtAngles_phi = (
                        2 * np.pi * np.random.rand(tmp_pts_to_generate, 1)
                    )  # phi
                    CluPtAngles_theta = np.arccos(
                        2 * np.random.rand(tmp_pts_to_generate, 1) - 1
                    )  # theta
                    CluPtRadii = np.sqrt(
                        np.random.randint(
                            min_dist,
                            high=max_dist,
                            size=(tmp_pts_to_generate, 1),
                        )
                        * coord_res
                    )  # radii, range rmin to rmax

                    ClusteredPointsList_tmp[:, 0, None] = np.array(
                        (
                            CluPtRadii
                            * np.cos(CluPtAngles_phi)
                            * np.sin(CluPtAngles_theta)
                        )
                        + Cluster_Seeds[clusterID, 0]
                    )
                    ClusteredPointsList_tmp[:, 1, None] = np.array(
                        (
                            CluPtRadii
                            * np.sin(CluPtAngles_phi)
                            * np.sin(CluPtAngles_theta)
                        )
                        + Cluster_Seeds[clusterID, 1]
                    )
                    ClusteredPointsList_tmp[:, 2, None] = np.array(
                        (CluPtRadii * np.cos(CluPtAngles_theta))
                        + Cluster_Seeds[clusterID, 2]
                    )

                    if Edge_fuzz_ratio > 0:
                        # fuzzle the points to blur out hard-edge clusters
                        fuzzy_x_offsets = np.random.normal(
                            0, Edge_fuzzing, tmp_pts_to_generate
                        )
                        fuzzy_y_offsets = np.random.normal(
                            0, Edge_fuzzing, tmp_pts_to_generate
                        )
                        fuzzy_z_offsets = np.random.normal(
                            0, Edge_fuzzing, tmp_pts_to_generate
                        )
                        ClusteredPointsList_tmp[:, 0, None] = (
                            ClusteredPointsList_tmp[:, 0, None]
                            + fuzzy_x_offsets[:, None]
                        )
                        ClusteredPointsList_tmp[:, 1, None] = (
                            ClusteredPointsList_tmp[:, 1, None]
                            + fuzzy_y_offsets[:, None]
                        )
                        ClusteredPointsList_tmp[:, 2, None] = (
                            ClusteredPointsList_tmp[:, 2, None]
                            + fuzzy_z_offsets[:, None]
                        )
                    ...
                    sigma = np.array(
                        [0.5 * rad_max, 0.5 * rad_max, 0.5 * rad_max]
                    )  # decrease/increase the multiplier to generate tighter/looser clustering about the cluster seed
                    covariance = np.diag(sigma**2)
                    xyz_gaussian = np.random.multivariate_normal(
                        mean, covariance, ptsperclu
                    )
                    ClusteredPointsList_tmp[:, 0, None] = xyz_gaussian[
                        :, 0, None
                    ]
                    ClusteredPointsList_tmp[:, 1, None] = xyz_gaussian[
                        :, 1, None
                    ]
                    ClusteredPointsList_tmp[:, 2, None] = xyz_gaussian[
                        :, 2, None
                    ]

                ClusteredPointsList_tmp[:, 3] = clusterID + 1
                ClusteredPointsList_tmp[
                    :, 4
                ] = 1  # the label for clustered points

                if Uncertainty_fwhm > 0:
                    # add localization uncertainty to each point
                    locprec_x_offsets = np.random.normal(
                        0, Uncertainty_fwhm, tmp_pts_to_generate
                    )
                    locprec_y_offsets = np.random.normal(
                        0, Uncertainty_fwhm, tmp_pts_to_generate
                    )
                    locprec_z_offsets = np.random.normal(
                        0, Uncertainty_fwhm, tmp_pts_to_generate
                    )
                    # Todo: combine and save these uncertainty values for each point
                    ClusteredPointsList_tmp[:, 0, None] = (
                        ClusteredPointsList_tmp[:, 0, None]
                        + locprec_x_offsets[:, None]
                    )
                    ClusteredPointsList_tmp[:, 1, None] = (
                        ClusteredPointsList_tmp[:, 1, None]
                        + locprec_y_offsets[:, None]
                    )
                    ClusteredPointsList_tmp[:, 2, None] = (
                        ClusteredPointsList_tmp[:, 2, None]
                        + locprec_z_offsets[:, None]
                    )

                # remove duplicate xy points in ClusteredPointsList_tmp
                _, uniq_idx = np.unique(
                    ClusteredPointsList_tmp, axis=0, return_index=True
                )
                if uniq_idx.shape[0] < ClusteredPointsList_tmp.shape[0]:
                    uniq_idx = np.sort(uniq_idx)
                    ClusteredPointsList_tmp = ClusteredPointsList_tmp[
                        uniq_idx, :
                    ]
                ...
                unique_CPts = np.logical_not(
                    np.isin(
                        ClusteredPointsList_tmp, ClusteredPointsList_final
                    ).all(axis=1)
                )
                ClusteredPointsList_tmp = ClusteredPointsList_tmp[
                    unique_CPts, :
                ...
                ClusteredPointsList_final = np.concatenate(
                    (
                        ClusteredPointsList_final,
                        ClusteredPointsList_tmp[uniq_idx, :],
                    ),
                    axis=0,
                )
            ...
            ClusteredPointsList[
                InsertionPoint : InsertionPoint + ptsperclu, :
            ] = ClusteredPointsList_final[
                1:, :
            ]  # ignoring the first row which is all zeros.
            InsertionPoint += ptsperclu

        if doImages:
            plt.scatter(
                ClusteredPointsList[:, 0],
                ClusteredPointsList[:, 1],
                s=2,
                color="red",
            )
        ...
        if ClusteredPointsList.shape[0] > 0:
            _, uniq_idx = np.unique(
                ClusteredPointsList[:, :2], axis=0, return_index=True
            )
            if uniq_idx.shape[0] < ClusteredPointsList.shape[0]:
                print(
                    "\t***\t"
                    + str(ClusteredPointsList.shape[0] - uniq_idx.shape[0])
                    + " Clustered points with duplicate xy coordinates"
                )

        if NCPointsList.shape[0] > 0:
            _, uniq_idx = np.unique(
                NCPointsList[:, :2], axis=0, return_index=True
            )
            if uniq_idx.shape[0] < NCPointsList.shape[0]:
                print(
                    "\t***\t"
        ...
        if PointList.shape[0] > 0:
            _, uniq_idx = np.unique(
                PointList[:, :2], axis=0, return_index=True
            )
            if uniq_idx.shape[0] < PointList.shape[0]:
                print(
                    "\t***\t"
                    + str(PointList.shape[0] - uniq_idx.shape[0])
                    + " overall points with duplicate xy coordinates"
                )
        else:
            raise ValueError(
                "Somehow failed to get this far without generating any points!"
            )
        ...
        Cluster_Seeds = np.zeros(
            (1, 3)
        )  # function will return an empty list of seeds
        ...
        while NCPointsRemaining != 0:
            if NCPointsRemaining > 0:
                ...
                Generating_NCPoints = int(
                    NCPointsRemaining * OvercompensationFactor
                )  # over-estimate so that we don't go through this process too many times
                ...
                NCPtsProposed = np.concatenate(
                    (NCPointsList_x, NCPointsList_y, NCPointsList_z), axis=1
                )
                ...
                if NCPtsProposed.shape[0] > 0:
                    ...
                    _, uniq_idx = np.unique(
                        NCPtsProposed, axis=0, return_index=True
                    )
                ...
                NCPointsRemaining = int(
                    TotalNonClusterPoints - PointList.shape[0]
                )  # re-test for sufficient number of events
                ...
                RandomKeeperIdx = (
                    np.random.permutation(PointList.shape[0] - 1) + 1
                )
                RandomKeeperIdx = np.concatenate(
                    (
                        [0],
                        np.sort(RandomKeeperIdx[: TotalNonClusterPoints - 1]),
                    )
                )
        ...
        PointList = np.concatenate(
            (PointList, np.zeros((PointList.shape[0], 2))), axis=1
        )  # Add clusterUID and Label values (both are 0 for NC points)
        ...
        if PointList.shape[0] > 0:
            ...
            if uniq_idx.shape[0] < PointList.shape[0]:
                print(
                    "\t***\t"
                    + str(PointList.shape[0] - uniq_idx.shape[0])
                    + " CSR points with duplicate xy coordinates"
                )
        else:
            raise ValueError(
                "Somehow failed to get this far without generating any points!"
