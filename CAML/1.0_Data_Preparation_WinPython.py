...
This script is for Windows users:
    ...
    If you are using Spyder (or any IPython based IDE) and you wish to
    use parallel processing then you need to use this script for Stage 1.

    You need to ensure that you have set the correct working directory by
    right-clicking the tab with this script's name and selecting 'Set console
    working directory'.

    The select all text in this script (Ctrl A) and press F9 to run it.
...
# Image Saving
# ========================================================================
SaveImages = True  # Save images (scatterplots) of the data?
...
SimuCell_SaveImagesFor = (
    "partial"  # 'all'     Save all repeats in a scenario group
)
...
doAppendIsoGrid = (
    False  # This isn't really finished yet and should be left as false.
)
...
# removed this next line for Windows
# if __name__ == '__main__':

# added 'while True:' to preserve the original indent layout
while True:
    ...
    while not good_json:
        ...
        if "input_PrepJSON" in locals():
            default_json_file = (
                input_PrepJSON  # from a previous run of this script
            )
    ...
    if SaveImages and (
        SimuCell_SaveImagesFor == "all" or SimuCell_SaveImagesFor == "partial"
    ):
        cmap2ch = fn_etc.load_colormap(
            "./cmaps/two_channel_bluyel.txt", flip=False
        )
    ...
    inputpath = fn_etc.askforinput(
        message="Enter the path of the folder containing "
        + ps["InputFileExt"]
        + " data tables",
    ...
    if ask_prepType in ["1"]:
        if ps["ClusMembershipIDCol"] == False or ps["LabelIDCol"] == False:
            fn_etc.err_msg(
                "You have not specified ClusMembershipIDCol or LabelIDCol in ProcSettings. Can\t train without knowing where the labels are!"
            )
            raise ValueError(
                "A value for ClusMembershipIDCol and LabelIDCol in ProcSettings is required when training. Please specify one."
            )
        else:
            prepType = "training"
    ...
    default_outfolder = os.path.join(
        inputpath,
        "1_prep_dNN("
        + str(ps["ClosestFriend"])
        + "-"
        + str(ps["FurthestFriend"])
        + ")_"
        + prepType,
    )
    ...
    if starting_index != 0:
        reset_starting_index = fn_etc.askforinput(
            message="Current Index is set to "
            + str(starting_index)
            + ", i.e. begin with File "
            + str(starting_index + 1)
            + ". Do you want to reset it to zero? (Y or N)",
            errormessage="Type Y or N",
            defaultval="y",
            isvalid=lambda v: v.lower() in ["y", "n", "yes", "no"],
        )
        ...
        if reset_starting_index.lower() in ["y", "yes"]:
            starting_index = 0
            print(
                "Current index has been reset to zero. Processing will begin from the first file in the list."
            )
        else:
            print(
                "Keeping the current index. Processing will begin with File "
                + str(starting_index + 1)
                + " in the list."
            )
    ...
    if finishing_index != 0:
        reset_finishing_index = fn_etc.askforinput(
            ...
            + " is done. Do you want to reset it and process all files? (Y or N)",
            errormessage="Type Y or N",
            defaultval="y",
            isvalid=lambda v: v.lower() in ["y", "n", "yes", "no"],
        )
        ...
        if reset_finishing_index.lower() in ["y", "yes"]:
            ...
            print(
                "Finishing Index has been reset and all files in the folder will be processed."
            )
        else:
            print(
                "Keeping the current index. Processing will end once File "
                + str(finishing_index)
                + " is done."
            )
    else:
    ...
    with open(
        os.path.join(s1_prep_outputpath, "PrepLog.txt"), "a+", newline=""
    ) as prep_log:
    ...
    for fileIdx in range(starting_index, finishing_index):
        ...
        fn_etc.info_msg(
            str(fileIdx + 1) + " of " + str(total_files) + "\t" + current_file
        )
        print("Loading data...", end="", flush=True)
        datatable = np.genfromtxt(
            os.path.join(inputpath, current_file),
        ...
        if "DataScale" in ps:
            datatable[:, ps["xCol"]] = (
                ps["DataScale"] * datatable[:, ps["xCol"]]
            )
            datatable[:, ps["yCol"]] = (
                ps["DataScale"] * datatable[:, ps["yCol"]]
            )

        if ps["three_dee"]:
            datatable[:, ps["zCol"]] = (
                ps["DataScale"] * datatable[:, ps["zCol"]]
            )
        ...
        with open(os.path.join(inputpath, current_file), "r") as f:
            ...
            if ps["InputFileDelimiter"] != "\t":
                ps["TableHeaders"] = ps["TableHeaders"].replace(
        ...
        if TotalPointsThisImage < ps["FurthestFriend"] - ps["ClosestFriend"]:
            fn_etc.warn_msg(
                ...
                + " points. Minimum of "
                + str(ps["FurthestFriend"] - ps["ClosestFriend"] + 1)
                + " points is required, according to the supplied JSON file. Missing neighbours will be padded with nans!"
            )
        ...
        if doSetUpAxes == True:
            ...
            ps["xMin"] = (
                np.floor(xmin / ps["AutoAxesNearest"]) * ps["AutoAxesNearest"]
            )
            ps["xMax"] = (
                np.ceil(xmax / ps["AutoAxesNearest"]) * ps["AutoAxesNearest"]
            )
            ps["yMin"] = (
                np.floor(ymin / ps["AutoAxesNearest"]) * ps["AutoAxesNearest"]
            )
            ps["yMax"] = (
                np.ceil(ymax / ps["AutoAxesNearest"]) * ps["AutoAxesNearest"]
            )
            if ps["three_dee"]:
                ps["zMin"] = (
                    np.floor(zmin / ps["AutoAxesNearest"])
                    * ps["AutoAxesNearest"]
                )
                ps["zMax"] = (
                    np.ceil(zmax / ps["AutoAxesNearest"])
                    * ps["AutoAxesNearest"]
                )
                ps["ImageSize"] = [
                    ps["xMax"] - ps["xMin"],
                ...
                print(
                    "AutoAxes: \t Set image boundaries to [ "
                    + str(ps["xMin"])
                    + ", "
                    + str(ps["xMax"])
                    + ", "
                ...
                ps["ImageSize"] = [
                    (ps["xMax"] - ps["xMin"], ps["yMax"] - ps["yMin"])
                ]
                print(
                    "AutoAxes: \t Set image boundaries to [ "
                    + str(ps["xMin"])
        ...
        if doSanityCheck:
            ...
            if np.isnan(datatable[:, ps["xCol"]]).any():
                ...
                raise ValueError(
                    "Coordinate xy data must be real numbers, not NaNs."
                )
            ...
            xy_range = [
                np.floor(min(datatable[:, ps["xCol"]])),
            ...
            if ps["three_dee"]:
                ...
                if np.isnan(datatable[:, ps["zCol"]]).any():
                    fn_etc.err_msg(
                        "NaNs found in your input data z-coord column."
                    )
                    raise ValueError(
                        "Coordinate z data must be real numbers, not NaNs."
                    )

                z_range = [
                    np.floor(min(datatable[:, ps["zCol"]])),
                    np.ceil(max(datatable[:, ps["zCol"]])),
                ]
                print(
                    "Data ranges:\t"
                    + str(xy_range[1] - xy_range[0])
                    + "(x) "
                    + str(xy_range[3] - xy_range[2])
                    + "(y) "
                    + str(z_range[1] - z_range[0])
                    + "(z)"
                )
            else:
                print(
            ...
            print(
                "SanityCheck:\tChecking the lateral (xy) coordinate data are sensible ... ",
                end="",
                flush=True,
            )
            try:
                aspect_ratio = (xy_range[1] - xy_range[0]) / (
                    xy_range[3] - xy_range[2]
                )
            ...
            if (
                aspect_ratio < Sane_min_xy_ratio
                or aspect_ratio > Sane_max_xy_ratio
            ):
                fn_etc.err_msg(
                    "x-range is "
                    + str(round(aspect_ratio, 5))
                    + " times the y-range!\n\t\tAcceptable limits for the x-range are "
                    + str(Sane_min_xy_ratio)
                    + " to "
                    + str(Sane_max_xy_ratio)
                    + " times the y-range.\n\t\tCheck z-column input or adjust the 'Sanity Check' variables."
                )
                raise ValueError(
                    "Large difference between x and y data ... are the correct columns given?\nRemember to use zero indexing when specifying your columns!\nIf columns are correct then inspect the values used when performing the 'Sanity Check'."
                )
            ...
            if ps["three_dee"] and np.isnan(datatable[:, ps["zCol"]]).any():
                print(
                    "SanityCheck: \t Checking the axial (z) data are sensible ... ",
                    end="",
                    flush=True,
                )

                try:
                    zx_aspect_ratio = (z_range[1] - z_range[0]) / (
                        xy_range[1] - xy_range[0]
                    )
                except ZeroDivisionError:
                    zx_aspect_ratio = 0
                try:
                    zy_aspect_ratio = (z_range[1] - z_range[0]) / (
                        xy_range[3] - xy_range[2]
                    )
                except ZeroDivisionError:
                    zy_aspect_ratio = 0
                ...
                if zx_aspect_ratio > Sane_max_zx_ratio:
                    fn_etc.err_msg(
                        "z-range is "
                        + str(round(zx_aspect_ratio, 5))
                        + " times the x-range - check z-column input!\n\t\tAcceptable maximum for the z-range is "
                        + str(Sane_max_zx_ratio)
                        + " times the x-range.\n\t\tCheck z-column input or adjust the 'Sanity Check' variables."
                    )
                    raise ValueError(
                        "Large difference in scale between x and z data ... are the correct columns given?\nRemember to use zero indexing when specifying your columns!\nIf columns are correct then inspect the values used when performing the 'Sanity Check'."
                    )
                elif zy_aspect_ratio > Sane_max_zx_ratio:
                    fn_etc.err_msg(
                        "z-range is "
                        + str(round(zy_aspect_ratio, 5))
                        + " times the y-range - check z-column input!\n\t\tAcceptable maximum for the z-range is "
                        + str(Sane_max_zy_ratio)
                        + " times the y-range.\n\t\tCheck z-column input or adjust the 'Sanity Check' variables."
                    )
                    raise ValueError(
                        "Large difference in scale between y and z data ... are the correct columns given?\nRemember to use zero indexing when specifying your columns!\nIf columns are correct then inspect the values used when performing the 'Sanity Check'."
                    )
                else:
        ...
        if doFilterDuplicates:
            if ps["three_dee"]:
                data_coordsonly = np.concatenate(
                    (
                        datatable[:, ps["xCol"], None],
                        datatable[:, ps["yCol"], None],
                        datatable[:, ps["zCol"], None],
                    ),
                    axis=1,
                )
            else:
            ...
            _, uniq_idx = np.unique(
                data_coordsonly, axis=0, return_index=True
            )  # get the unique rows
            ...
            if uniq_idx.shape[0] < datatable.shape[0]:
                ...
                fn_etc.info_msg(
                    "Checked for duplicate points and removed "
                    + str(DuplicatePointsRemoved)
                    + " identical points."
                )
            else:
        ...
        if SaveImages:
            ...
            if (
                "SaveImagesForRepeat" in ps
                and SimuCell_SaveImagesFor.lower() == "partial"
            ):
                ...
                SimuCell_test = output_prefix.split("_")[
                    -1
                ]  # the last element should be the repeat number
                SimuCell_Ints = ""
                SimuCell_Chars = ""
                ...
                if not all(
                    (
                        int(SimuCell_Ints) == ps["SaveImagesForRepeat"],
                        SimuCell_Chars == "cellID()",
                    )
                ):
                    ps[
                        "SaveMyImages"
                    ] = False  # this file isn't flagged for image-saving
        ...
        if ps["SaveMyImages"]:
            ...
            for ExportImage in ExportImageFormat:
                ...
                preview_fname = os.path.join(
                    s1_prep_outputpath,
                    output_prefix + "_preview." + ExportImage,
                )
        ...
        if doAppendIsoGrid:
            ...
            iso_grid = fn_isogrid.iso_spots(
                boundary=[ps["xMin"], ps["xMax"], ps["yMin"], ps["yMax"]],
                spacing=50,
            )
            iso_appendage = np.empty(
                (
                    iso_grid.shape[0],
                    datatable.shape[1],
                )
            ...
            TotalPointsThisImage = datatable.shape[
                0
            ]  # update this from before
            ...
            if doFilterDuplicates:
                ...
                if uniq_idx.shape[0] < datatable.shape[0]:
                    ...
                    if not doAddUID:
                        doAddUID = True
                    fn_etc.info_msg(
                        "Re-screened for duplicate xy points: removed "
                        + str(oldTotalPoints - TotalPointsThisImage)
                        + " identical points."
                    )
                else:
                    print(
                        "Re-screened for duplicate xy points: none were found"
                    )
                del data_xy
        ...
        if not "LabelIDCol" in ps and prepType == "training":
            ...
            datatable = np.insert(
                datatable, ps["LabelIDCol"], ClassLabels_tmp, axis=1
            )
            ps["TableHeaders"] = ps["TableHeaders"] + "\tClass Label"
            print(
                "LabelIDCol: \t Added Class Labels to column "
                + str(ps["LabelIDCol"])
            )
        ...
        GuessProcTime = (
            (ProcTimeEst_A * TotalPointsThisImage * TotalPointsThisImage)
            + (ProcTimeEst_B * TotalPointsThisImage)
            + ProcTimeEst_C
        )
        if GuessProcTime > 30:
            ...
            print(
                "Total of "
                + str(TotalPointsThisImage)
                + " points is expected to take "
                + str(FancyProcTime)
                + " to calculate distances."
            )  # + GuessFinishTime.strftime('%H:%M:%S on %d %B'))
        else:
            print("Total of " + str(TotalPointsThisImage) + " points.")
        free_space = fn_etc.get_free_space(s1_prep_outputpath)
        GuessMemoryRequirements = (
            (
                (
                    TotalPointsThisImage
                    * (ps["FurthestFriend"] - ps["ClosestFriend"] + 1)
                    * (datatable.shape[1] + 1)
                )
                * len(ps["ImageSize"])
                * 64
            ...
            + (datatable.size * 64)
        ) / 8  # float64 bytes required
        print(
            fn_etc.convert_size(GuessMemoryRequirements)
            + " is required to store this image's xy data and distance measurements ("
            + fn_etc.convert_size(free_space)
            + " available space)"
        )
        if GuessMemoryRequirements > free_space:
            ...
            print(
                "\x1b[1;33;40m"
                + "Free up at least "
                + fn_etc.convert_size(GuessMemoryRequirements - free_space)
                + " to continue processing this image.\nBe aware that subsequent images in the queue may also require additional storage space."
                + "\x1b[0m",
                flush=True,
            )
            ask_prepType = fn_etc.askforinput(
        ...
        datatable_mmap_fname = os.path.join(
            s1_prep_outputpath, datatable_mmap_partname
        )
        ...
        dists_mmap_fname = os.path.join(
            s1_prep_outputpath, dists_mmap_partname
        )
        ...
        if ps["three_dee"]:
            Parallel(n_jobs=total_cpus, verbose=3)(
                delayed(fn_distcalc_3D.dnns_v3)(
                    datatable_mmapped, datatable_mmapped, ps, dists_mmapped, i
                )
                for i in range(TotalPointsThisImage)
            )
        else:
            Parallel(n_jobs=total_cpus, verbose=3)(
                delayed(fn_distcalc.dnns_v3)(
                    datatable_mmapped, datatable_mmapped, ps, dists_mmapped, i
                )
                for i in range(TotalPointsThisImage)
            )
        ...
        print(
            ...
            + " seconds ("
            + str(round(TotalPointsThisImage / elapsed_time, 2))
            + " points per second.)"
        )
        ...
        if prepType == "training":
            ...
            cluster_IDs_only = dists_mmapped[
                :, :, ps["ClusMembershipIDCol"] + 1
            ]
            target_vectors = np.zeros(
                (cluster_IDs_only.shape[0], 1), dtype="int"
            )
            ...
            print(
                "Image has "
                + str(ClusteredPointsThisImage)
                + " clustered points (from "
                + str(total_clusters_this_image)
                + " clusters) and "
                + str(TotalPointsThisImage - ClusteredPointsThisImage)
                + " non-clustered points."
            )
            ...
            with open(
                os.path.join(s1_prep_outputpath, "PrepLog.txt"),
                "a+",
                newline="",
            ) as prep_log:
                writer = csv.writer(prep_log, delimiter="\t")
            ...
            # dump target vectors
            target_vectors_mmap_partname = (
                output_prefix + "_TargetVectors.MemMap"
            )
            ps["target_vectors_mmap_fname"] = target_vectors_mmap_partname
            ps["VectorsDumpShape"] = target_vectors.shape
            target_vectors_mmap_fname = os.path.join(
                s1_prep_outputpath, target_vectors_mmap_partname
            )
            ...
            # dump target binary
            target_binary_mmap_partname = (
                output_prefix + "_TargetBinary.MemMap"
            )
            ps["target_binary_mmap_fname"] = target_binary_mmap_partname
            ps["BinaryDumpShape"] = target_binary.shape
            target_binary_mmap_fname = os.path.join(
                s1_prep_outputpath, target_binary_mmap_partname
            )
            ...
            # dump target labels
            target_labels_mmap_partname = (
                output_prefix + "_TargetLabels.MemMap"
            )
            ps["target_labels_mmap_fname"] = target_labels_mmap_partname
            ps["LabelsDumpShape"] = target_labels.shape
            target_labels_mmap_fname = os.path.join(
                s1_prep_outputpath, target_labels_mmap_partname
            )
        ...
        print(
            ...
            + ps["FilePrefix"]
            + " at "
            + datetime.datetime.now().strftime("%H:%M:%S on %d %B")
            + "\n"
        )
    ###
    #####
    ...
    if EmailUponCompletion:
        print(
            fn_etc.send_jobdone_email(
                "Data Preparation on folder " + s1_prep_outputpath,
                " - Data Prep done",
            )
        )
    print(
