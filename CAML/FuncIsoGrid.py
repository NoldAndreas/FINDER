...
def iso_spots(boundary=[0, 10000, 0, 10000], spacing=100):
    ...
    gridA_xv, gridA_yv = np.meshgrid(
        gridA_x, gridA_y, sparse=False, indexing="ij"
    )
    gridA_yv[1::2] += 0.5 * spacing  # or [::2] to modify the even indices

    grid_out = np.concatenate(
        (
            gridA_xv.reshape(gridA_xv.size, 1),
            gridA_yv.reshape(gridA_yv.size, 1),
        ),
        axis=1,
    )
    ...
    x_within = np.logical_and(
        grid_out[:, 0] > boundary[0], grid_out[:, 0] < boundary[1]
    )
    y_within = np.logical_and(
        grid_out[:, 1] > boundary[2], grid_out[:, 1] < boundary[3]
    )
