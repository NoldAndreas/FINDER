...
if __name__ == '__main__':
    ...
    if total_files_train > 0:
        ...
        if total_files_train != np.shape(PrepLog_imported)[0]:
            ...
            if total_files_train == np.shape(data_files_train)[0] == np.shape(json_files_train)[0] == np.shape(target_vectors_files_train)[0] == np.shape(target_binary_files_train)[0]:
                ...
                print(' JSON describes ' + str(np.shape(json_files_train)[0]) + ' files to process. All sets must match for training to proceed.')
                print(' ├─Dists files:\t\t' + str(total_files_train))
                print(' ├─Data files:\t\t' + str(np.shape(data_files_train)[0]))
                print(' ├─TargetBinary files:\t' + str(np.shape(target_binary_files_train)[0]))
                print(' └─TargetLabels files:\t' + str(np.shape(target_labels_files_train)[0]))
                print(' └─TargetVectors files:\t' + str(np.shape(target_vectors_files_train)[0]))
                
                raise ValueError(ErrorMessage)

    else:
        fn_etc.err_msg('No files to process!')
        raise ValueError('No files to process! Check you have given the correct folder...')
    ...
    shape_tracker = 0   # to enable growing of our mmap array as new data comes in.
...
#            print('Guessing mmapsize')
            points_pool_fname = os.path.join(s2_trnvaltest_outputpath, 'train_pool.MemMap')
            points_pool_mmap = np.memmap(points_pool_fname, 
                              dtype='float64', 
                              shape=((3, ps_current['FurthestFriend'])),
                              mode='w+')
    
        fn_etc.info_msg(str(fileIdx + 1) + '/' + str(total_files_train) + ' - ' + ps_current['FilePrefix'])
    
        # load distances from memmap file
        import_dists_f = os.path.join(inputpath_train, ps_current['dists_mmap_fname'])
        ...
        import_vectors_f = os.path.join(inputpath_train, ps_current['target_vectors_mmap_fname'])
        ...
        import_binary_f = os.path.join(inputpath_train, ps_current['target_binary_mmap_fname'])
        ...
        import_labels_f = os.path.join(inputpath_train, ps_current['target_labels_mmap_fname'])
        if os.path.isfile(import_labels_f):
            target_labels = np.memmap(import_labels_f, dtype='int', shape=tuple(ps_current['LabelsDumpShape']), mode='r')

        # load the original coordinate data from mmap
        import_datatable_f = os.path.join(inputpath_train, ps_current['datatable_mmap_fname'])
        if os.path.isfile(import_datatable_f):
            target_datatable = np.memmap(import_datatable_f, dtype='float64', shape=tuple(ps_current['DataDumpShape']), mode='r')
        ...
        if TotalPointsThisImage == PrepLog_imported['Total Points'][fileIdx] and total_clustered_points_this_file == PrepLog_imported['Clustered Points'][fileIdx]:        
        ...
        training_indices = np.empty((0,), dtype=int) # initialize the training indices with a dummy row
        for label in range(total_labels):
            ...
            if label_indices.shape[0] > 0:
                ...
                label_counters[label] = label_counters[label] + np.shape(label_indices)[0]
            
            print('\t- Harvested: \t' + str(np.shape(label_indices)[0]) + '\t ' + label_names[label] + ' points.')
        ...
        new_mmap = np.memmap(points_pool_fname,
                             mode='r+',
                             dtype='float64',
                             shape=(shape_tracker + total_retained_points, ps_current['FurthestFriend'], total_features))
        
        # put the new points into the larger pool
        if feature_type == 'dists':

            new_mmap[shape_tracker:shape_tracker + total_retained_points,:] = Dists_all_New[training_indices,:,0,None]
            ...
            training_NN_x = Dists_all_New[training_indices,:,ps_current['xCol'] + 1]
            training_NN_y = Dists_all_New[training_indices,:,ps_current['yCol'] + 1]
            
            # append the origin-point x and y coords at the end of the NN list (it will be removed later)
            training_NN_x = np.concatenate((training_NN_x, training_origin_pts[:,0, None]), axis=1)
            training_NN_y = np.concatenate((training_NN_y, training_origin_pts[:,1, None]), axis=1)

            # subtract the xy origin coords from each and normalize (check the appended origin should all be zero)
            training_NN_xcoords_norm = training_NN_x - training_origin_pts[:,0,None]
            training_NN_ycoords_norm = training_NN_y - training_origin_pts[:,1,None]
            ...
            training_NN_xcoords_norm = ((training_NN_xcoords_norm - row_x_mins[:,None]) / row_x_ranges[:,None])
            training_NN_xcoords_norm = training_NN_xcoords_norm - training_NN_xcoords_norm[:,-1, None]  # recenter about the origin point x-coord
            ...
            training_NN_ycoords_norm = ((training_NN_ycoords_norm - row_y_mins[:,None]) / row_y_ranges[:,None])
            training_NN_ycoords_norm = training_NN_ycoords_norm - training_NN_ycoords_norm[:,-1, None] # recenter about the origin point y-coord
...
#            plt.show()
            ...
            new_mmap[shape_tracker:shape_tracker + total_retained_points,:,0] = training_NN_xcoords_norm[:,:-1]
            new_mmap[shape_tracker:shape_tracker + total_retained_points,:,1] = training_NN_ycoords_norm[:,:-1]
            
            if ps_current['three_dee']:
                # do the above for the z data
                training_NN_z = Dists_all_New[training_indices,:,ps_current['zCol'] + 1]
                training_NN_z = np.concatenate((training_origin_pts[:,2, None], training_NN_z), axis=1)
                training_NN_zcoords_norm = training_NN_z - training_origin_pts[:,2,None]
                ...
                training_NN_zcoords_norm = ((training_NN_zcoords_norm - row_z_mins[:,None]) / row_z_ranges[:,None])
                training_NN_zcoords_norm = training_NN_zcoords_norm - training_NN_zcoords_norm[:,-1, None] # recenter about the origin point z-coord
                
                new_mmap[shape_tracker:shape_tracker + total_retained_points,:,1] = training_NN_zcoords_norm[:,:-1]
        else:
            raise ValueError('Can only work with feature_type set to \'dists\' or \'coords\'')
        ...
        vectors_pooled = np.vstack((vectors_pooled, target_vectors[training_indices]))               
        binaries_pooled = np.vstack((binaries_pooled, target_binary[training_indices]))
        labels_pooled = np.vstack((labels_pooled, target_labels[training_indices]))
        ...
        _ = gc.collect()
        print('\t- Added points to pool. ' + str(shape_tracker) + ' points pooled in total.')
        # fn_etc.complete_msg
    ...
    pool_all_labels_indices = [] # this list will hold a list of point-indices for each label
    for label in range(total_labels):
        print('\tShuffling pooled points for ' + label_names[label] + '... ', end='')
...
#              )
    ...
    for label in range(total_labels):
        ...
        if label == total_labels - 1:
            ...
            training_reqd_counts.append(total_training_count - sum(training_reqd_counts[:total_labels]))
            validation_reqd_counts.append(total_validation_count - sum(validation_reqd_counts[:total_labels]))
            testing_reqd_counts.append(total_testing_count - sum(testing_reqd_counts[:total_labels]))
            ...
            training_reqd_counts.append(int(np.floor(total_training_count * this_label_training_ratio)))
            validation_reqd_counts.append(int(np.floor(total_validation_count * this_label_validation_ratio)))
            testing_reqd_counts.append(int(np.floor(total_testing_count * this_label_testing_ratio)))
        
        total_reqd_counts.append(training_reqd_counts[label] + testing_reqd_counts[label] + validation_reqd_counts[label])
    ...
    print('\t--------------------\t------------\t------------\t------------')
    for label in range(total_labels):
        print('\t' + label_names[label] + '\t\t' + str(total_reqd_counts[label]) + '\t\t' + str(len(pool_all_labels_indices[label])) + '\t\t', end='', flush=True)
    ...
    print('\tTotal\t\t\t' + str(total_reqd_points) + '\t\t' + str(total_avail_points) + '\t\t' + fn_etc.fancy('  NO  ','white','red') if total_avail_points < total_reqd_points else fn_etc.fancy('  OK  ','white','green'))
    
    if total_avail_points < total_reqd_points:
        ErrorMessage = 'Insufficient points!\r\n' + \
                       'Training, validation, and testing requires ' + str(total_reqd_points) + ' points but \r\n' + \
                       'we only have ' + str(total_avail_points) + ' points pooled from \r\n' + \
                       'all files in ' + inputpath_train + '.\r\n' + \
...
#        print('\x1b[36;1m\t' + 'OK!' + '\x1b[0m', flush=True)
    ...
    for label in range(total_labels):
        print('\tShuffling and drawing \''+label_names[label]+'\' points for the training, validation, and testing pools...', end='')
        training_startIdx = 0
        validation_startIdx = training_reqd_counts[label]
        testing_startIdx = training_reqd_counts[label] + validation_reqd_counts[label]

        training_rnd_idx = np.hstack((training_rnd_idx, pool_all_labels_indices[label][training_startIdx:training_reqd_counts[label]]))
        validation_rnd_idx = np.hstack((validation_rnd_idx, pool_all_labels_indices[label][validation_startIdx:validation_startIdx+validation_reqd_counts[label]]))
        testing_rnd_idx = np.hstack((testing_rnd_idx, pool_all_labels_indices[label][testing_startIdx:testing_startIdx+testing_reqd_counts[label]]))
    ...
    TrainValTest_data_basename = time.strftime('%Y%m%d-%H%M') + '_nn' + str(ps_current['ClosestFriend']) + '-' + str(ps_current['FurthestFriend']) + '_' + str(total_labels) + 'labels_Trn_Val_Tes.pkl'
    TrainValTest_data_fname = os.path.join(s2_trnvaltest_outputpath, TrainValTest_data_basename)
    print('\tSaving training, validation, and testing data to:\n\t\t' + TrainValTest_data_fname + ' ... ')
    ...
    pickle_file = open(TrainValTest_data_fname, 'wb' ) # open the file for writing
    pickle.dump(save_order, pickle_file, protocol=4) # save the save_order first
    for item in save_order:
        pickle.dump(locals()[item], pickle_file, protocol=4) # save each item in save_order to the pickle file
    ...
    fn_etc.ok_msg('Finished preparing training, validation, and testing datasets.')
