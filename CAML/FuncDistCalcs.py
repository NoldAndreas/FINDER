...
def dnns_v3(inputdata1, inputdata2, ps, output_filename, i):
    ...
    #        TestRegion = inputdata2
        ...
        TestRegionSize = np.ceil(
            np.sqrt(
                maxNeighbours
                / (
                    inputdata2.shape[0]
                    / (0.3 * ps["ImageSize"][0] * ps["ImageSize"][1])
                )
            )
        )
    #        UseableRegion = False
    ...
    while not UseableRegion:
        TestRegBounds = [
            np.floor(inputdata1[0, ps["xCol"]] - TestRegionSize),
        ...
        TestRegion = RegionCropper(
            inputdata2, TestRegBounds, [ps["xCol"], ps["yCol"]]
        )
    ...
    dists = cdist(
        inputdata1[:, [ps["xCol"], ps["yCol"]]],
        TestRegion[:, [ps["xCol"], ps["yCol"]]],
    ).T
    ...
    if padNeighbours > 0:
        ...
        dists = np.concatenate(
            (dists, np.full((padNeighbours, dists.shape[1]), np.nan)), axis=0
        )

    # export this point's results for insertion into the main output
    output_filename[i, :, :] = dists[
        ps["ClosestFriend"] : ps["FurthestFriend"] + 1, :
    ]
