...
def send_jobdone_email(
    strJobID, strSubject=" Processing Complete", extraInfo=None
):
    """
    ...
    if os.path.isfile(email_secrets_json):
        ...
        if not all(
            ...
            return warn_msg(
                "Email notifier: settings file found but does not contain valid settings!\r\n"
                + "Email notifications cannot be sent."
            )
    else:
        return warn_msg(
            "Email notifier: Settings file ('email_secrets.json') could not be found!\r\n"
            + "Email notifications cannot be sent."
        )
    ...
    message["From"] = (
        email_secrets["sender_name"]
        + "<"
        + email_secrets["sender_email_addr"]
        + ">"
    )
    message["To"] = (
        email_secrets["recipient_name"]
        + "<"
        + email_secrets["recipient_email_addr"]
        + ">"
    )

    try:
        smtpObj = smtplib.SMTP(
            email_secrets["sender_host_name"],
            email_secrets["sender_host_port"],
        )
        smtpObj.connect(
            email_secrets["sender_host_name"],
            email_secrets["sender_host_port"],
        ...
        smtpObj.ehlo()
        smtpObj.login(
            email_secrets["sender_login"], email_secrets["sender_passwd"]
        )
        smtpObj.sendmail(
            email_secrets["sender_email_addr"],
            email_secrets["recipient_email_addr"],
        ...
        return warn_msg(
            "Error: Notification message could not be sent\r\n"
...
def askforinput(message, errormessage, defaultval, isvalid):
    ...
    while not validresult:
        ...
        if defaultval is not None:
            result = input(
                fancy(str(message), "cyan", "darkgrey")
                + fancy(
                    " [ Default is " + defaultval + " ]", "white", "darkgrey"
                )
                + ": "
            )
        else:
        ...
        if not result and defaultval is not None:
            result = defaultval
            print(
                fancy("Using default:", "black", "green") + " " + result + "\n"
            )
            validresult = True
            ...
            if result == "exit!":
                raise ValueError(
                    "You entered 'exit!' so the script has stopped running."
                )
            ...
            # input was supplied and it was good
                ...
                print(
                    fancy("Acknowledged:", "black", "green")
                    + " "
                    + result
                    + "\n"
                )
...
def ok_msg(msg_str):
    print(
        "\x1b[1;30;42m" + "     OK!    " + "\x1b[0m" + "  " + msg_str,
        flush=True,
    )
...
def info_msg(msg_str):
    print(
        "\x1b[1;37;44m" + "    INFO    " + "\x1b[0m" + "  " + msg_str,
        flush=True,
    )
...
def warn_msg(msg_str):
    print(
        "\x1b[1;30;43m" + "  WARNING!  " + "\x1b[0m" + "  " + msg_str,
        flush=True,
    )
...
def make_image(inputdata, outputname, ps_input, colormapID):
    ...
    data_x_width = max(inputdata[:, ps_input["xCol"]]) - min(
        inputdata[:, ps_input["xCol"]]
    )
    data_y_width = max(inputdata[:, ps_input["yCol"]]) - min(
    ...
    if (field_x_width / data_x_width) > 5 or (
        field_y_width / data_y_width
    ) > 5:
        suggest_x_min = (
            np.floor(
                np.min(inputdata[:, ps_input["xCol"]])
                / ps_input["AutoAxesNearest"]
            )
            * ps_input["AutoAxesNearest"]
        )
        suggest_x_max = (
            np.ceil(
                np.max(inputdata[:, ps_input["xCol"]])
                / ps_input["AutoAxesNearest"]
            )
            * ps_input["AutoAxesNearest"]
        )
        suggest_y_min = (
            np.floor(
                np.min(inputdata[:, ps_input["yCol"]])
                / ps_input["AutoAxesNearest"]
            )
            * ps_input["AutoAxesNearest"]
        )
        suggest_y_max = (
            np.ceil(
                np.max(inputdata[:, ps_input["yCol"]])
                / ps_input["AutoAxesNearest"]
            )
            * ps_input["AutoAxesNearest"]
        )
        ...
        paperSize_y = (
            (suggest_y_max - suggest_y_min) / (suggest_x_max - suggest_x_min)
        ) * paperSize_x
    else:
        paperSize_y = (
            (ps_input["yMax"] - ps_input["yMin"])
            / (ps_input["xMax"] - ps_input["xMin"])
        ) * paperSize_x
    ...
    if "ClusMembershipIDCol" in ps_input:
        if (
            ps_input["ClusMembershipIDCol"] >= 0
            and type(ps_input["ClusMembershipIDCol"]) == int
        ):
            ColorByCluster = True
    ...
    if ColorByCluster:
        ...
        if "PointsMinValue" in ps_input:
            ...
            PointsMinValue = inputdata[
                :, ps_input["ClusMembershipIDCol"]
            ].min()

        if "PointsMaxValue" in ps_input:
            PointsMaxValue = ps_input["PointsMaxValue"]
        else:
            PointsMaxValue = inputdata[
                :, ps_input["ClusMembershipIDCol"]
            ].max()
            if PointsMaxValue == PointsMinValue:
                PointsMaxValue = PointsMinValue + 1

        MinValData = inputdata[
            np.where(
                (
                    inputdata[:, ps_input["ClusMembershipIDCol"]]
                    == PointsMinValue
                )
            ),
            :,
...
def load_colormap(cmapfile, flip=False):
    ...
    if os.path.isfile(cmapfile):
        ...
        try:
            ...
            print(
                "Unable to interpret your custom colormap file! Reverting to 'viridis'"
            )
            return "viridis"
        ...
        if cmapfile in builtins:
            ...
            print(
                "Specified colormap is unavailable (not a built-in or the file does not exist)\r\n"
                + "You gave: '"
                + cmapfile
                + "'\r\n"
...
def plot_confusion_matrix(
    ...
    plt.imshow(
        cm, interpolation="nearest", cmap=cmap, vmin=color_min, vmax=color_max
    )
