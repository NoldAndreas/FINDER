...
DoImage_ScoresHistogram = (
    False  # Distribution of clustering probabilities for each image
)
...
Image_ScoresLabels_Folder = (
    "Scores and Labels"  # Folder name for the above images
)
...
if True:  # __name__ == '__main__':
    ...
    if "model_fname" in locals():
        default_model_file = (
            model_fname  # model_fname exists from previous script
        )
    else:
    ...
    model_name = os.path.splitext(
        os.path.basename(os.path.normpath(model_fname))
    )[0]
    ...
    outputpath_novel = os.path.abspath(
        os.path.join(inputpath_novel, "..", "4_evaluated_by_" + ModelShortName)
    )

    print("Checkpoint 2")
    ## get a list of the files to process from the given folder
    dist_files_novel = natsorted(
        [i for i in os.listdir(inputpath_novel) if "Dists.MemMap" in i]
    )
    data_files_novel = natsorted(
        [i for i in os.listdir(inputpath_novel) if "Data.MemMap" in i]
    )
    json_files_novel = natsorted(
        [i for i in os.listdir(inputpath_novel) if ".json" in i]
    )

    if np.shape(dist_files_novel)[0] > 0:
        if (
            np.shape(dist_files_novel)[0]
            == np.shape(data_files_novel)[0]
            == np.shape(json_files_novel)[0]
        ):
            total_files_novel = np.shape(dist_files_novel)[0]
            print(
                "Found " + str(total_files_novel) + " datasets to work with..."
            )
        else:
            ...
            if go_with == 0 or go_with == 1:
                ...
                raise ValueError(
                    "Files are mismatched! Each dataset requires a -Data.MemMap, -Dists.MemMap, and .json file to proceed."
                )
            elif go_with == 2:
                # json_files is shortest
                # this should always be the case as it is generated after the other two files!
                fn_etc.info_msg(
                    "Processing partial list of files from selected folder -- rerun this script when additional file-sets are added."
                )
                dist_files_novel = dist_files_novel[
                    : np.shape(json_files_novel)[0]
                ]
                data_files_novel = data_files_novel[
                    : np.shape(json_files_novel)[0]
                ]
                total_files_novel = np.shape(dist_files_novel)[0]
                print(
                    "Found "
                    + str(total_files_novel)
                    + " datasets to work with..."
                )
    else:
        raise ValueError(
            "No files to process! Check you have a) given the correct folder and b) this folder contains files prepared by the 'Data preparation' processing step."
        )
    ...
    if os.path.exists(model_fname):
        ...
        print(
            "Loaded Model "
            + model_ID
            + "\t(You can safely ignore warnings above about 'No training configuration found')"
        )
        model_config = model.get_config()
        print(model)
        print(model_config)
        RequiredInputSize = model_config["layers"][0]["config"][
            "batch_input_shape"
        ][1]
        ...
        if ModelLabelsTotal == 1:
            ModelLabelsTotal = 2
        print(
            "This model expects input from "
            + str(RequiredInputSize)
            + " near-neighbours."
        )
        ...
        try:
            InputFolder_Check_A = inputpath_novel.split("_")[
                -2
            ]  # the second-last element should have the NN count
            InputFolder_Check_B = InputFolder_Check_A.split("-")[
                -1
            ]  # Get the last digit
        except:
            InputFolder_Check_A = "not a folder made by CAML"
            InputFolder_Check_B = str(
                RequiredInputSize
            )  # if we can't extract the information we need
        InputFolder_Ints = ""
        InputFolder_Chars = ""
        ...
        if (
            not int(InputFolder_Ints) == RequiredInputSize
            and InputFolder_Chars == "dNN(-)"
        ):
            raise ValueError(
                "Your input data uses "
                + InputFolder_Ints
                + " near-neighbour values but Model "
                + model_ID
                + " is expecting "
                + str(RequiredInputSize)
                + " near-neighbour values.\r\nPlease use a model which matches the input data size."
            )
    ...
    if starting_index != 0:
        keep_starting_index = fn_etc.askforinput(
            message="Staring Index is set to "
            + str(starting_index)
            + " (begin with File "
            + str(starting_index + 1)
            + "). Is this correct? Y to carry on or N to start with the first file.",
        ...
        if keep_starting_index.lower() in ["y", "yes"]:
            print(
                "Keeping the current start file. Processing will start with File "
                + str(starting_index + 1)
            )
        else:
            print(
                "Starting point has been reset. Processing will begin with the first file in the list."
            )
            starting_index = 0
    ...
    if finishing_index != 0:
        keep_finishing_index = fn_etc.askforinput(
            message="Finishing Index is set to "
            + str(finishing_index)
            + ", i.e. end processing after File "
            + str(finishing_index - 1)
            + " is done. Is this correct? Y to carry on or N to end with the last file.",
            errormessage="Type Y or N",
            defaultval="y",
            isvalid=lambda v: v.lower() in ["y", "n", "yes", "no"],
        )
        ...
        if keep_finishing_index.lower() in ["y", "yes"]:
            print(
                "Keeping the current end file. Processing will end once File "
                + str(finishing_index)
                + " is done."
            )
        else:
            ...
            print(
                "Finishing point has been reset. Processing will end with the last file in the list."
            )
    else:
        finishing_index = total_files_novel
    ...
    if DoEmailNotifications == "ask":
        ...
        if choose_email_notification.lower() in ["y", "yes"]:
            ...
            print(
                "Notification emails will be sent (if email configuration is correctly set up)."
            )
        else:
    ...
    if proceed_with_processing.lower() in ["p"]:
        ...
        raise ValueError(
            "Not an error, you just decided not to proceed and that's OK! :)"
        )
    ...
    if DoImage_ScoresHistogram and not os.path.exists(
        os.path.join(outputpath_novel, Image_ScoresHistogram_Folder)
    ):
        os.makedirs(
            os.path.join(outputpath_novel, Image_ScoresHistogram_Folder)
        )
    ...
    if DoImage_InputValues and not os.path.exists(
        os.path.join(outputpath_novel, Image_InputValues_Folder)
    ):
    ...
    with open(
        os.path.join(outputpath_novel, ModelLog_fname), "a+", newline=""
    ) as model_log:
        ...
        writer.writerow(
            [
                ...
                "Cluster Segmentation Time (s)",
                "Cluster Shape Fitting Time (s)",
            ]
        )
    ...
    for fileIdx in range(starting_index, finishing_index):
        ...
        if ps_current["SaveMyImages"] and fileIdx == starting_index:
            if DoImage_ByModelScores or DoImage_ByLabels:
                if not os.path.exists(
                    os.path.join(outputpath_novel, Image_ScoresLabels_Folder)
                ):
                    os.makedirs(
                        os.path.join(
                            outputpath_novel, Image_ScoresLabels_Folder
                        )
                    )

        datatable_called_fname = os.path.join(
            outputpath_novel,
            ps_current["FilePrefix"]
            + "_DataCalled"
            + ps_current["InputFileExt"],
        )

        fn_etc.progress_msg(
            str(fileIdx + 1) + " of " + str(finishing_index),
            ps_current["FilePrefix"],
        )
        ...
        if os.path.exists(datatable_called_fname):
            ...
            import_dists_f = os.path.join(
                inputpath_novel, ps_current["dists_mmap_fname"]
            )
            ...
            if ps_current["ClusMembershipIDCol"]:
                import_vectors_f = os.path.join(
                    inputpath_novel, ps_current["target_vectors_mmap_fname"]
                )
                ...
                import_binary_f = os.path.join(
                    inputpath_novel, ps_current["target_binary_mmap_fname"]
                )
            ...
            X_novel_distances_raw = np.array(
                Dists_all_New[:, :, 0], dtype="float32"
            )  # convert from float64 to float32 to ease memory requirements
            X_novel = fn_normalize.normalize_dists(
                X_novel_distances_raw,
                TotalPointsThisImage,
                DistancesProcessedAs,
            )
            if ModelLabelsTotal == 2:
                X_novel = X_novel.reshape(
                    (X_novel.shape[0], X_novel.shape[1], 1)
                )  # Reshape the data to be repeats/measures/features for LSTM
            ...
            novel_probabilities = model.predict(
                X_novel, batch_size=64, verbose=1
            )  # probability assessment of event being in cluster(~1) or not (~0)
            if ModelLabelsTotal == 2:
                novel_predictions = [
                    float(np.round(x - (pred_threshold - 0.5)))
                    for x in novel_probabilities
                ]  # convert probability into boolean by pessimistic threshold ... must be >pred_threshold to qualify as 1
            else:
                novel_predictions = novel_probabilities.argmax(
                    axis=-1
                )  # convert model's probabilities to labels
            ...
            pickle.dump(
                (novel_probabilities, novel_predictions),
                open(
                    os.path.join(
                        outputpath_novel,
                        ps_current["FilePrefix"]
            ...
            import_datatable_f = os.path.join(
                inputpath_novel, ps_current["datatable_mmap_fname"]
            )
            ...
            datatable_called = np.concatenate(
                (
                    datatable_mmapped,
                    novel_probabilities,
                    np.array(novel_predictions).reshape(
                        TotalPointsThisImage, 1
                    ),
                ),
                axis=1,
            )
            ps_current["TableHeaders"] = (
                ps_current["TableHeaders"]
                + ps_current["InputFileDelimiter"]
                + "score"
                + ps_current["InputFileDelimiter"]
                + "label ("
                + model_ID
            ...
            if ModelLabelsTotal == 3:
                ...
                PercentClustered = (
                    (TotalType1Points + TotalType2Points)
                    / TotalPointsThisImage
                ) * 100
            else:
                PercentClustered = (
                    (TotalType1Points) / TotalPointsThisImage
                ) * 100
            ...
            print(
                ...
                + str(fileIdx + 1)
                + " has "
                + str(round(PercentClustered, 2))
                + " percent of points in clusters"
            )
            if ModelLabelsTotal == 3:
                print(
                    "\t"
                    + str(
                        round(TotalType1Points / TotalPointsThisImage * 100, 2)
                    )
                    + " percent of points are Type 1"
                )
                print(
                    "\t"
                    + str(
                        round(TotalType2Points / TotalPointsThisImage * 100, 2)
                    )
                    + " percent of points are Type 2"
                )
            ...
            if DoImage_ScoresHistogram:
                ...
                if ModelLabelsTotal == 2:
                    sns_plot = sns.distplot(
                        novel_probabilities, kde=False, ax=sns_ax
                    )
                    sns_plot.set(
                        xlabel="'In Cluster' Probability",
                    ...
                    sns_plot = sns.distplot(
                        ...
                        label=["Type 0", "Type 1", "Type 2"],
                    )
                    sns_plot.set(
                        xlabel="Point Type Probability",
                        ylabel="Frequency (Points)",
                        xlim=[0, 1],
                    )
                ...
                for ExportImage in ExportImageFormat:
                    sns_plot_fname = os.path.join(
                        outputpath_novel,
                        Image_ScoresHistogram_Folder,
                        ps_current["FilePrefix"]
                        + " - Histogram."
                        + ExportImage,
                    )
            ...
            if ps_current["AutoAxes"] == True:
                ps_current["xMin"] = (
                    np.floor(
                        np.min(datatable_mmapped[:, ps_current["xCol"]])
                        / ps_current["AutoAxesNearest"]
                    )
                    * ps_current["AutoAxesNearest"]
                )
                ps_current["xMax"] = (
                    np.ceil(
                        np.max(datatable_mmapped[:, ps_current["xCol"]])
                        / ps_current["AutoAxesNearest"]
                    )
                    * ps_current["AutoAxesNearest"]
                )
                ps_current["yMin"] = (
                    np.floor(
                        np.min(datatable_mmapped[:, ps_current["yCol"]])
                        / ps_current["AutoAxesNearest"]
                    )
                    * ps_current["AutoAxesNearest"]
                )
                ps_current["yMax"] = (
                    np.ceil(
                        np.max(datatable_mmapped[:, ps_current["yCol"]])
                        / ps_current["AutoAxesNearest"]
                    )
                    * ps_current["AutoAxesNearest"]
                )
            ...
            if ps_current["SaveMyImages"]:
                ...
                if DoImage_ByModelScores or DoImage_ByLabels:
                    ...
                    if ps_temp["ImageSize"][0] / 50 < 300:
                        ...
                        ps_temp["PointSize"] = (
                            round(ps_temp["ImageSize"][0] / 5000) + 1
                        )
                ...
                if DoImage_ByModelScores:
                    ...
                    print(
                        "Saving image of points coloured by model's score... ",
                        end="",
                        flush=True,
                    )
                    for ExportImage in ExportImageFormat:
                ...
                if DoImage_ByLabels:
                    ...
                    print(
                        "Saving image of points colours by classification label... ",
                        end="",
                        flush=True,
                    )
                    for ExportImage in ExportImageFormat:
                ...
                if DoImage_InputValues:
                    ...
                    clustered_indices = np.where(
                        np.logical_and(
                            datatable_called[:, labels_col] > 0,
                            datatable_called[:, scores_col]
                            > inputplot_clus_minscore,
                            datatable_called[:, scores_col]
                            <= inputplot_clus_maxscore,
                        )
                    )[0]
                    nonclustered_indices = np.where(
                        np.logical_and(
                            datatable_called[:, labels_col] == 0,
                            datatable_called[:, scores_col]
                            >= inputplot_nonclus_minscore,
                            datatable_called[:, scores_col]
                            < inputplot_nonclus_maxscore,
                        )
                    )[0]
                    ...
                    colormin = np.floor(
                        np.min(
                            (
                                X_novel[clustered_indices, :, 0].min(),
                                X_novel[nonclustered_indices, :, 0].min(),
                            )
                        )
                    )
                    colormax = np.ceil(
                        np.max(
                            (
                                X_novel[clustered_indices, :, 0].max(),
                                X_novel[nonclustered_indices, :, 0].max(),
                            )
                        )
                    )
                    ...
                    if GroundTruthCol != None:
                        fig_dd, (
                            ...
                            1, 4, gridspec_kw={"width_ratios": [30, 1, 30, 1]}
                        )
                    else:
                        fig_dd, (ax_nclu, ax_clus) = plt.subplots(
                            1, 2, gridspec_kw={"width_ratios": [1, 1]}
                        )

                    # plot the nonclustered points
                    nclu_distdiffsmat = ax_nclu.matshow(
                        np.array(
                            X_novel[nonclustered_indices, :, 0],
                            dtype="float32",
                        ),
                        cmap=cmap,
                        vmin=colormin,
                    ...
                    ax_nclu.tick_params(
                        color=plotfgcolor, labelcolor=plotfgcolor
                    )
                    ...
                    # add axis labels
                    ax_nclu.set_title(
                        "100 Random Non-clustered Points (scores < "
                        + str(inputplot_nonclus_maxscore)
                        + ")",
                        color=plotfgcolor,
                        y=1.1,
                    )
                    ax_nclu.set_xlabel(
                        "nth Nearest Neighbour", color=plotfgcolor
                    )
                    ax_nclu.xaxis.set_label_position("top")
                    ax_nclu.set_ylabel("Point #", color=plotfgcolor)

                    # add nclu colorbar
                    ax_nclu_cbar = fig_dd.colorbar(
                        nclu_distdiffsmat, fraction=0.046, pad=0.04, ax=ax_nclu
                    )
                    ax_nclu_cbar.set_label(
                        colour_axis_label, color=plotfgcolor
                    )  # set colorbar label plus label color
                    ax_nclu_cbar.ax.yaxis.set_tick_params(
                        color=plotfgcolor
                    )  # set colorbar tick color
                    ax_nclu_cbar.outline.set_edgecolor(
                        plotfgcolor
                    )  # set colorbar edgecolor
                    plt.setp(
                        plt.getp(ax_nclu_cbar.ax.axes, "yticklabels"),
                        color=plotfgcolor,
                    )

                    if GroundTruthCol != None:
                        # plot the nonclustered points' ground truth
                        nclu_grndtruthsmat = ax_nclu_gt.matshow(
                            np.array(
                                ground_truth[nonclustered_indices, None],
                                dtype="float32",
                            ),
                            cmap=cmap2ch,
                            vmin=0,
                        ...
                        ax_gtnclu_cbar = fig_dd.colorbar(
                            nclu_grndtruthsmat,
                            fraction=0.46,
                            pad=0.15,
                            ax=ax_nclu_gt,
                    ...
                    clus_distdiffsmat = ax_clus.matshow(
                        np.array(
                            X_novel[clustered_indices, :, 0], dtype="float32"
                        ),
                        cmap=cmap,
                        vmin=colormin,
                        vmax=colormax,
                    ...
                    # add axis labels
                    ax_clus.set_title(
                        "100 Random Clustered Points (scores > "
                        + str(inputplot_clus_minscore)
                        + ")",
                        color=plotfgcolor,
                        y=1.1,
                    )
                    ...
                    # Add colourbar
                    ax_clus_cbar = fig_dd.colorbar(
                        clus_distdiffsmat, fraction=0.046, pad=0.04, ax=ax_clus
                    )
                    ax_clus_cbar.set_label(
                        colour_axis_label, color=plotfgcolor
                    )  # set colorbar label plus label color
                    ax_clus_cbar.ax.yaxis.set_tick_params(
                        color=plotfgcolor
                    )  # set colorbar tick color
                    ax_clus_cbar.outline.set_edgecolor(
                        plotfgcolor
                    )  # set colorbar edgecolor
                    plt.setp(
                        plt.getp(ax_clus_cbar.ax.axes, "yticklabels"),
                        color=plotfgcolor,
                    )

                    if GroundTruthCol != None:
                        # plot the nonclustered points' ground truth
                        clus_grndtruthsmat = ax_clus_gt.matshow(
                            np.array(
                                ground_truth[clustered_indices, None],
                                dtype="float32",
                            ),
                            cmap=cmap2ch,
                            vmin=colormin,
                        ...
                        ax_gtclus_cbar = fig_dd.colorbar(
                            clus_grndtruthsmat,
                            fraction=0.46,
                            pad=0.15,
                            ax=ax_clus_gt,
                    ...
                    for ExportImage in ExportImageFormat:
                        DistancePlots_fname = os.path.join(
                            outputpath_novel,
                            Image_InputValues_Folder,
                            ps_current["FilePrefix"]
                            + "Differences in Distances - Some Random Points."
                            + ExportImage,
                        )
                        fig_dd.savefig(
                            DistancePlots_fname,
            ...
            WorkList_onlyclustered = (
                datatable_mmapped[
                    np.array(novel_predictions) == 1, ps_current["UIDCol"]
                ].astype(int)
                - 1
            )
            # add the clustering scores alongside the indices
            WorkList_probs_onlyclustered = novel_probabilities[
                WorkList_onlyclustered, :
            ]
            WorkList = np.concatenate(
                [
                    WorkList_onlyclustered.reshape(
                        [WorkList_onlyclustered.size, 1]
                    ),
                    WorkList_probs_onlyclustered,
                ],
                axis=1,
            )
            ...
            if (
                ModelLabelsTotal == 2
                and TotalType1Points > minimum_points_for_cluster
            ):
                ...
                if PercentClustered > 95:
                    fn_etc.progress_msg(
                        "***",
                        "High percentage of clustered points: Performing additional cluster segmentation...",
                    )

                print(
                    "Segmenting "
                    + str(TotalType1Points)
                    + " clustered points into like-clusters: ",
                    end="",
                    flush=True,
                )
                ...
                UpdateProgress_UIDs = list(
                    WorkList[:: int(WorkList.shape[0] / BarLength + 1)]
                )
                UpdateProgress_UIDs.pop(0)  # remove the first index
                UpdateProgress_UIDs.append(
                    WorkList[-1]
                )  # add the last index to the list
                HeartBeat_UIDs = list(
                    set(list(WorkList[::25])).difference(UpdateProgress_UIDs)
                )
                ...
                for CurrentPoint_UID in WorkList:
                    ...
                    CurrentPoint_Label = novel_predictions[
                        CurrentPoint_UID
                    ]  # ML Label (C or NC)

                    # Get list of CurrentPoint's NN-UIDs, subtract one to get their indices
                    CurrPts_NNUIDs = (
                        Dists_all_New[CurrentPoint_UID, :, -1].astype(int) - 1
                    )
                    ...
                    if nn_ends_IDx >= minimum_points_for_cluster:
                        ...
                        if PercentClustered > 95:
                            ...
                            CurrentDists = np.diff(
                                Dists_all_New[
                                    CurrentPoint_UID, :nn_ends_IDx, 0
                                ]
                            )  # / Dists_all_New[CurrentPoint_UID,0,0]
                            CurrDistsMinusMedian = np.abs(
                                CurrentDists - np.median(CurrentDists)
                            )
                            CDMedianDev = np.median(CurrDistsMinusMedian)
                            CurrDistsScore = (
                                CurrDistsMinusMedian / CDMedianDev
                                if CDMedianDev
                                else 0.0
                            )
                            OutlierPoints = np.array(
                                np.where(CurrDistsScore > 5 * CDMedianDev)
                            )
                            ...
                            if OutlierDistance > minimum_points_for_cluster:
                                ...
                                nn_ends_IDx = (
                                    OutlierPoints[0, 0] + 1
                                )  # we stop this cluster growing at the first (closest NN) outlier (plus 1 to include the furthest point before the jump)

                        # Get the current cluster IDs; excluding -1 IDs
                        Existing_ClusIDs = novel_clusterclub[
                            CurrPts_NNUIDs[0:nn_ends_IDx]
                        ]
                        Potential_ClusIDs = Existing_ClusIDs[
                            Existing_ClusIDs > 0,
                        ]

                        if Potential_ClusIDs.size > 0:
                            # our nearest also-clustered neighbours have usable cluster IDs
                            ImpendingAssignmentID = np.min(
                                (
                                    np.min(Potential_ClusIDs),
                                    novel_clusterclub[CurrentPoint_UID],
                                )
                            )
                            novel_clusterclub[
                                CurrentPoint_UID
                            ] = ImpendingAssignmentID
                            novel_clusterclub[
                                CurrPts_NNUIDs[0:nn_ends_IDx]
                            ] = ImpendingAssignmentID
                        else:
                            ...
                            novel_clusterclub[
                                CurrentPoint_UID
                            ] = -1  # assign NC membership
                            refined_cluster_call[
                                CurrentPoint_UID
                            ] = 0  # update label to NC
                    else:
                        novel_clusterclub[
                            CurrentPoint_UID
                        ] = (
                            -1
                        )  # demote this point as it doesn't have enough friends to bother with
                        refined_cluster_call[
                            CurrentPoint_UID
                        ] = 0  # also update the label to reflect demotion
                    ...
                    if CurrentPoint_UID in UpdateProgress_UIDs:
                        ...
                        CompletedStr = CompletedBlock * CompletedLength
                        print(
                            DeleteStr + CompletedStr + RemainingStr,
                            end="",
                            flush=True,
                        )
                    ...
                    if CurrentPoint_UID in HeartBeat_UIDs:
                        ...
                        print(
                            DeleteStr + CompletedStr + Flipper + RemainingStr,
                            end="",
                            flush=True,
                        )
                ...
                UniqueClusIDs = UniqueClusIDs[
                    UniqueClusIDs != -1
                ]  # exlude ID = -1 (nonclustered)

                total_clusters = np.shape(UniqueClusIDs)[0]

                # done processing - finish the progress bar
                print(
                    " Done (found " + str(total_clusters) + " clusters)",
                    flush=True,
                )
                ...
                if not ps_current["three_dee"]:
                    demoted_points = (
                        list()
                    )  # clustered points to be remarked as not-clustered
                    demoted_clusters = list()  # clusterIDs to delete
                    reassigned_points = (
                        list()
                    )  # clustered points which have been given new clusterIDs
                    cluclub_out = (
                        list()
                    )  # cluster info: ClusterID, OutlineCoords, Area, Population (of points)
                    ...
                    if BufferEps == "nnd":
                        ...
                        NNDs = (
                            list()
                        )  # no sense passing a pile of unused data to each worker
                    ...
                    _ = Parallel(
                        n_jobs=total_cpus, verbose=3, require="sharedmem"
                    )(
                        delayed(fn_cluclu.cluster_shaper)(
                            i,
                            novel_clusterclub,
                            UniqueClusIDs,
                            datatable_mmapped,
                    ...
                    for i in demoted_clusters:
                        refined_cluster_call[
                            np.where(novel_clusterclub == i)
                        ] = 0  # first flip the status of points with demoted clusters
                        novel_clusterclub[
                            np.where(novel_clusterclub == i)
                        ] = -1  # then reassign the NC cluster-ID
                    ...
                    for reassign_task in reassigned_points:
                        novel_clusterclub[
                            ...
                            0, 1
                        ]  # subtract 1 to convert pointUID to index (zero base indexing)

                    # update cluster status of demoted points from C to NC
                    for demoted_pt in demoted_points:
                        refined_cluster_call[
                            demoted_pt - 1
                        ] = 0  # update classification (1 --> 0 for demoted points)
                        novel_clusterclub[
                            demoted_pt - 1
                        ] = (
                            -1
                        )  # update clusterIDs (OriginalID --> -1 for demoted points)

                    # scan for duplicates in the list of cluster UIDs-- there shouldn't be any!
                    dups = [
                        x for x in cluclub_cIDs if cluclub_cIDs.count(x) > 1
                    ]
                    assert len(dups) == 0
                    ...
                    fn_etc.info_msg(
                        "Cluster shape forming is not yet supported for 3D images."
                    )

                ReassignedClusIDs = np.array(
                    range(1, np.shape(UniqueClusIDs)[0] + 1), dtype="int"
                )
                ...
                novel_clusterclub_harmonised = np.zeros(
                    novel_clusterclub.shape
                ).astype(int)
                for idx, NewClusterID in enumerate(ReassignedClusIDs):
                    novel_clusterclub_harmonised[
                        np.where(novel_clusterclub == UniqueClusIDs[idx])
                    ] = NewClusterID
                ...
                novel_clusterclub[
                    novel_clusterclub == -1
                ] = 0  # relabel NC points as 0 (rather than -1) for ease of plotting

                UniqueClusIDs = np.unique(novel_clusterclub)
                UniqueClusIDs = UniqueClusIDs[
                    UniqueClusIDs != 0
                ]  # exlude ID = 0 (nonclustered)

                if not ps_current["three_dee"]:
                    # Re-assess the total clusters
                    total_clusters = np.shape(UniqueClusIDs)[0]
                    TotalClusteredPoints = sum(
                        [idx > 0 for idx in refined_cluster_call]
                    )
                    PercentClustered = (
                        TotalClusteredPoints / TotalPointsThisImage
                    ) * 100
                    print(
                        "File "
                        + str(fileIdx + 1)
                        + " has "
                        + str(round(PercentClustered, 2))
                        + " percent of all points in "
                        + str(total_clusters)
                        + " clusters (after refinement)"
                    )
                ...
                fn_etc.info_msg(
                    "Cluster formation for model labels > 2 is not yet supported (you have "
                    + str(ModelLabelsTotal)
                    + " labels)."
                )
                total_clusters = -1
                ...
                fn_etc.info_msg(
                    "This image has "
                    + str(TotalType1Points)
                    + " clustered points and a minimum of "
                    + str(minimum_points_for_cluster)
                    + " clustered points are required to form a cluster. Clustering images will not be saved for this image."
                )
                total_clusters = 0
            ...
            with open(
                os.path.join(outputpath_novel, ModelLog_fname),
                "a+",
                newline="",
            ) as model_log:
                writer = csv.writer(model_log, delimiter="\t")
                writer.writerow(
                    [
                        ...
                        str(TotalPointsThisImage),
                        str(round(modeleval_time, 2)),
                        str(round(segmentation_time, 2)),
                        str(round(geometryfit_time, 2)),
                    ]
                )
            ...
            print(
                "Saving new data table with cluster information...",
                end="",
                flush=True,
            )
            if refined_points_exist:
                datatable_clusIDs = np.concatenate(
                    (
                        datatable_called,
            ...
            np.savetxt(
                datatable_called_fname,
                datatable_clusIDs,
                delimiter=ps_current["InputFileDelimiter"],
                fmt="%10.5f",
                header=ps_current["TableHeaders"],
                comments="",
            )
            print("Done.")
            ...
            if (
                ...
                data_essentials = np.concatenate(
                    (
                        datatable_clusIDs[:, ps_current["xCol"], None],
                        datatable_clusIDs[:, ps_current["yCol"], None],
                        datatable_clusIDs[:, clusID_col, None],
                    ),
                    axis=1,
                )

                cluster_patchlist_fname = os.path.join(
                    outputpath_novel,
                    ps_current["FilePrefix"] + "_ClusterShapes.pkl",
                )
                pickle.dump(
                    (
                        cluclub_path,
                    ...
                    open(cluster_patchlist_fname, "wb"),
                    protocol=4,
                )
                print("Done.")
                ...
                if ps_current["SaveMyImages"] and not ps_current["three_dee"]:
                    ...
                    vals = np.linspace(
                        0, 1, UniqueClusIDs.shape[0]
                    )  # include NC points this time
                    np.random.shuffle(vals)
                    jumble_cmap = plt.cm.colors.ListedColormap(cmap(vals))
                    jumble_cmap.set_under(
                        color="grey"
                    )  # colours outside the scale will be this color.

                    print(
                        "Saving image of points coloured by cluster ID...",
                        end="",
                        flush=True,
                    )
                    ps_temp = ps_current
                    ...
                    ps_temp["PointsMinValue"] = np.min(
                        UniqueClusIDs
                    )  # minimum needs to be wrt to the cluster IDs so that NC points become out of range and are coloured as 'under'.
                    ...
                    print(
                        "Saving image of points with cluster outlines...",
                        end="",
                        flush=True,
                    )
                    ...
                    for patch_idx, patch_p in enumerate(cluclub_path):
                        patch = patches.PathPatch(
                            patch_p,
                            facecolor=jumble_cmap(patch_idx),
                            edgecolor=None,
                            alpha=0.5,
                            lw=0.25,
                            zorder=-1,
                    ...
                    plt.scatter(
                        datatable_mmapped[
                            novel_clusterclub > 0, ps_current["xCol"]
                        ],
                        datatable_mmapped[
                            novel_clusterclub > 0, ps_current["yCol"]
                        ],
                        s=0.5,
                        c="k",
                        zorder=2,
                    ...
                    for ExportImage in ExportImageFormat:
                        cluster_outlines_fname = os.path.join(
                            outputpath_novel,
                            Image_ScoresLabels_Folder,
                            ps_current["FilePrefix"]
                            + " - Cluster Outlines."
                    ...
                    if DoImage_ByLabels and refined_points_exist:
                        ...
                        ps_temp["PointsMaxValue"] = ModelLabelsTotal - 1
                        print(
                            "Saving image of points coloured by updated classification label... ",
                            end="",
                            flush=True,
                        )
                        for ExportImage in ExportImageFormat:
    ...
    if DoImage_ScoresHistogram:
        print(
            "Histogram of model-score distribution were saved to folder '"
            + Image_ScoresHistogram_Folder
            + "'"
        )
    if DoImage_ByModelScores or DoImage_ByLabels:
        print(
            "Scatterplots for model scores and/or labels were saved to folder '"
            + Image_ScoresLabels_Folder
            + "'"
        )
    if DoImage_InputValues:
        print(
            "Input-value heatmaps were saved to folder '"
            + Image_InputValues_Folder
            + "'"
        )
