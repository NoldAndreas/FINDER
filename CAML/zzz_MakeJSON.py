...
if __name__ == '__main__':
    ...
    while not good_json:
        ...
        TakenColumns = [] # a list to hold columns which have already been given by the user
        ...
        InputFileExt = fn_etc.askforinput(
            ...
            isvalid = lambda v : (set(v) <= set(string.ascii_letters + string.digits + '.')) and v[0] == '.')
        ...
        InputFileDelimiter = fn_etc.askforinput(
            ...
            isvalid = lambda v : set(v) <= set(string.ascii_letters + ' !\"#$%&\'()*+,-./:;<=>?@[\]^_`{|}~'))
        
        if InputFileDelimiter == '\\t': InputFileDelimiter = '\t' # fix problem with escaped slashes on the tab delimiter case
        ...
        yCol = fn_etc.askforinput(
            ...
            errormessage= 'An integer greater than zero is required. You cannot specify the same column for multiple purposes (columns used: ' + str(TakenColumns) + ')',
            defaultval= default_yCol,
            isvalid = lambda v: v.isdigit() and int(v) >= 0 and int(v) not in TakenColumns)
        ...
        ClusMembershipIDCol = fn_etc.askforinput(
            ...
            errormessage= 'An integer greater than zero is required or the word None. You cannot specify the same column for multiple purposes (columns used: ' + str(TakenColumns) + ')',
            defaultval = default_ClusMembershipIDCol,
            isvalid = lambda v: (v.isdigit() and int(v) > 0 and int(v) not in TakenColumns) or v.lower() == 'none')
        ...
        if AutoAxes:
            ...
            xMax = fn_etc.askforinput(
                ...
                errormessage= 'An integer is required and it must be greater than the x-minimum of ' + str(xMin),
            ...
            yMax = fn_etc.askforinput(
                ...
                errormessage= 'An integer is required and it must be greater than the y-minimum of ' + str(yMin),
            ...
            if ThreeDeeData:
                ...
                zMax = fn_etc.askforinput(
                    ...
                    errormessage= 'An integer is required and it must be greater than the z-minimum of ' + str(yMin),
                    defaultval= default_yMax,
...
#            ImageSize = int(ImageSize)
        ...
        FurthestFriend = fn_etc.askforinput(
                ...
                errormessage= 'An integer is required and must be greater than ' + str(ClosestFriend) + ' (Closest Near-Neighbour from which distances are measured)',
        ...
        if verify_good_json.lower() in ['y','yes']:
            ...
            print('Settings rejected. Please give new settings or Ctrl X to cancel now.')
    ...
    JSON_out_file = fn_etc.askforinput(
        ...
        isvalid = lambda v : set(v) <= set(string.ascii_letters + string.digits + ' ._-'))
