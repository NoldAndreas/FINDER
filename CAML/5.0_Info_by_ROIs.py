...
AreaVsPointsPlot_folder = (
    "Area_v_Points"  # Folder name for above images to be saved.
)
...
area_factor = (
    1e6  # Areas will be in your original data's units, for example 'nm'
)
...
if __name__ == "__main__":
    ...
    data_files_list = natsorted(
        [i for i in os.listdir(inputpath_data) if "ClusterShapes.pkl" in i]
    )
    if np.shape(data_files_list)[0] > 0:
        total_files_novel = np.shape(data_files_list)[0]
    else:
        raise ValueError(
            "No files to process! Check you have a) given the correct folder and b) this folder contains '...ClusterShapes.pkl' files from the 'Data preparation' processing step."
        )
    ...
    if doUseROIs:
        roi_files_novel = natsorted(
            [i for i in os.listdir(inputpath_rois) if ".json" in i]
        )
        ...
        if total_rois > 0:
            ...
            for test_idx, test_file in enumerate(data_files_list):
                test_file_base = os.path.splitext(test_file)[0].replace(
                    "_ClusterShapes", ""
                )
                test_rois_fnames = [
                    roi
                    for roi in roi_files_novel
                    if test_file_base == roi.split("_roi")[0]
                    or test_file_base == roi.split("_ROI")[0]
                ]
                if len(test_rois_fnames) > 0:
                    keep_files[test_idx] = True
            data_files_list = [
                i for (i, v) in zip(data_files_list, keep_files) if v
            ]
            ...
            if total_files_novel == 0:
                raise ValueError(
                    "Found "
                    + str(total_rois)
                    + " ROI(s) but could not match any ROI to a data file! Check your ROI names."
                )
            else:
                fn_etc.info_msg(
                    "Found "
                    ...
                    + " files (of "
                    + str(total_files_novel_original)
                    + " files overall)."
                )

        else:
            raise ValueError(
                "There are no ROIs (files with .json extension) in the specified folder!"
            )
    else:
    ...
    if doPrismImportableFile:
        ...
        if prism_split_replicants == 0:
            doPrismImportableFile = False
    ...
    outputpath_summary = fn_etc.askforinput(
        ...
        defaultval=os.path.abspath(
            os.path.join(
                inputpath_data, "..", "5_ROIs_" + eval_model + ROI_root
            )
        ),
    ...
    if (
        starting_index > total_files_novel
        or finishing_index > total_files_novel
        or finishing_index < starting_index
    ):
        fn_etc.err_msg(
            "The starting_index (set to "
            + str(starting_index)
            + ") should be less than the finishing_index (set to "
            + str(finishing_index)
            + " and both should be within the range of files being processed ("
            + str(total_files_novel)
            + " files matched with ROIs)"
        )
        raise ValueError(
            "Check that your starting and finishing points take account of the number of files to process (after excluding those without ROIs!)"
        )
    ...
    if DoAreaVsPointsPlot and not os.path.exists(
        os.path.join(outputpath_summary, AreaVsPointsPlot_folder)
    ):
        os.makedirs(os.path.join(outputpath_summary, AreaVsPointsPlot_folder))

    if doPrismImportableFile and not os.path.exists(
        os.path.join(outputpath_summary, ForPrismFolder)
    ):
    ...
    with open(summary_stats_fname, "a") as fstats:
        ...
        writer.writerow(
            [
                ...
                "Points per um2",
                "Clustered Points",
                "Clustered Points per um2",
                "Percent Clustered",
                "Clusters",
                "Clusters per um2",
                "Median Points per Cluster",
                "Median Cluster Area (nm2)",
                "Median Point Density",
                "File Name",
            ]
        )

    stats_collector = np.zeros((total_files_novel * total_rois, 10)).astype(
        float
    )
    #        0 = fileIdx
    #        1 = ROI
    ...
    for fileIdx in range(starting_index, finishing_index):
        ...
        if doUseROIs:
            ...
            current_file_base = os.path.splitext(current_file)[0].replace(
                "_ClusterShapes", ""
            )
            my_rois_fnames = [
                roi
                for roi in roi_files_novel
                if current_file_base == roi.split("_roi")[0]
                or current_file_base == roi.split("_ROI")[0]
            ]
        ...
        stats_fname = os.path.join(
            outputpath_summary, ps_current["FilePrefix"] + "_DataCalled.tsv"
        )

        fn_etc.progress_msg(
            str(fileIdx + 1) + " of " + str(total_files_novel),
        ...
        if doUseROIs:
            for roi_fname in my_rois_fnames:
                with open(
                    os.path.join(inputpath_rois, roi_fname), "r"
                ) as file:
        ...
        if ps_current["SaveMyImages"]:
            ...
            ps_temp = ps_current
            if (
                ps_temp["ImageSize"][0] / 50 < 300
            ):  # TODO: this needs to adapt to the actual ROI area rather than the original ImageSize
                ps_temp["OutputScaling"] = 1
                ps_temp["PointSize"] = 10
            else:
                ps_temp["OutputScaling"] = 100
                ps_temp["PointSize"] = 1.0
            ps_temp["PointsMinValue"] = (
                np.min(cluclub_cIDs) + 1
        ...
        for current_roi in range(my_total_rois):
            ...
            # points within ROI
            pts_in_roi = current_roi_path.contains_points(
                data_essentials[:, :ClusterIDCol]
            )
            ...
            clusterIDs_in_roi = np.unique(
                data_essentials[pts_in_roi, ClusterIDCol]
            ).astype(int)
            clusterIDs_in_roi = clusterIDs_in_roi[
                clusterIDs_in_roi > 0
            ]  # exclude the zero IDs (NC points)
            cluster_idx_in_roi = (
                clusterIDs_in_roi - 1
            )  # convert ID into array indices
            roi_total_clusters = cluster_idx_in_roi.shape[0]

            # All clusters arising from points-witin-ROI
            clusterPaths_in_roi = [
                cluclub_path[patch_id] for patch_id in cluster_idx_in_roi[1:]
            ]

            # Only those clusters which are entirely within the ROI
            clusterPaths_wholly_within_roi = [
                roi_patch
                for roi_patch in clusterPaths_in_roi
                if current_roi_path.contains_path(roi_patch)
            ]

            # And those clusters which cross the ROI
            clusterPaths_on_edge_roi = [
                roi_patch
                for roi_patch in clusterPaths_in_roi
                if not current_roi_path.contains_path(roi_patch)
            ]
            ...
            if len(clusterPaths_on_edge_roi) > 0:
                print(
                    "***\t"
                    + str(len(clusterPaths_on_edge_roi))
                    + " clusters found crossing the ROI boundary line"
                )
            ...
            TotalClusteredPointsInRoi = np.sum(
                np.fromiter(
                    (
                        idx > 0
                        for idx in data_essentials[pts_in_roi, ClusterIDCol]
                    ),
                    dtype=int,
                )
            )
            PercentClustered = (
                TotalClusteredPointsInRoi / TotalPointsInRoi
            ) * 100
            print(
                "\t"
                + str(TotalPointsInRoi)
                + " points, "
                + str(roi_total_clusters)
                + " clusters, "
            ...
            per_cluster_stats = np.vstack(
                (
                    clusterIDs_in_roi,
                    cluclub_popn[cluster_idx_in_roi],
                    cluclub_area[cluster_idx_in_roi],
                    cluclub_density[cluster_idx_in_roi],
                )
            ).T
            ...
            fileidx_clusstats_fname = os.path.join(
                outputpath_summary,
                ps_current["FilePrefix"] + roi_suffix + "_ClusterStats.csv",
            )
            np.savetxt(
                fileidx_clusstats_fname,
                per_cluster_stats,
                "%i, %i, %.6f, %.6f",
                delimiter=",",
                header="ClusterID,Points,Area ("
                + AreaUnits
                + "),Density (pts/"
                + AreaUnits
                + ")",
            ...
            Median_PtsPerCluster = np.median(
                per_cluster_stats[:, 1]
            )  # Median Points per Cluster
            Median_AreaPerCluster = np.median(
                per_cluster_stats[:, 2]
            )  # Median Cluster Area
            Median_PtDensityPerCluster = np.median(
                per_cluster_stats[:, 3]
            )  # Median Density per Cluster
            ...
            if ps_current["SaveMyImages"] and not block_images:
                ...
                if doUseROIs:
                    ...
                    ps_temp["xMin"] = (
                        np.floor(
                            ROI_bounding_box[0] / ps_temp["AutoAxesNearest"]
                        )
                        * ps_temp["AutoAxesNearest"]
                    )
                    ps_temp["xMax"] = (
                        np.ceil(
                            (ROI_bounding_box[2] + ROI_bounding_box[0])
                            / ps_temp["AutoAxesNearest"]
                        )
                        * ps_temp["AutoAxesNearest"]
                    )
                    ps_temp["yMin"] = (
                        np.floor(
                            ROI_bounding_box[1] / ps_temp["AutoAxesNearest"]
                        )
                        * ps_temp["AutoAxesNearest"]
                    )
                    ps_temp["yMax"] = (
                        np.ceil(
                            (ROI_bounding_box[3] + ROI_bounding_box[1])
                            / ps_temp["AutoAxesNearest"]
                        )
                        * ps_temp["AutoAxesNearest"]
                    )

                    ps_temp["ImageSize"][0] = np.min(
                        (
                            ps_temp["xMax"] - ps_temp["xMin"],
                            ps_temp["yMax"] - ps_temp["yMin"],
                        )
                    )

                    test_dpi = ps_temp["ImageSize"][0] / 50
                    if test_dpi < 300:
                        ps_temp["OutputScaling"] = (
                            ps_temp["ImageSize"][0] / 300
                        )
                        ps_temp["PointSize"] = 5
                    else:
                        ps_temp["OutputScaling"] = 100
                    ...
                    patch = patches.PathPatch(
                        current_roi_path, facecolor="none", edgecolor="red"
                    )
                    ax.add_patch(patch)

                    # draw in-ROI cluster shapes
                    for in_roi_patch_idx, in_roi_patch_p in enumerate(
                        clusterPaths_in_roi
                    ):
                        in_roi_patch = patches.PathPatch(
                            in_roi_patch_p,
                            facecolor=jumble_cmap(in_roi_patch_idx),
                            edgecolor=None,
                            alpha=0.5,
                            lw=0.25,
                            zorder=-1,
                    ...
                    # draw in-ROI points
                    c_pts_in_roi = np.logical_and(
                        data_essentials[:, ClusterIDCol] > 0, pts_in_roi
                    )
                    nc_pts_in_roi = np.logical_and(
                        data_essentials[:, ClusterIDCol] == 0, pts_in_roi
                    )
                    plt.scatter(
                        data_essentials[c_pts_in_roi, xCol],
                        data_essentials[c_pts_in_roi, yCol],
                        s=1.0,
                        c="k",
                        zorder=2,
                        marker=".",
                    ...
                    plt.scatter(
                        data_essentials[nc_pts_in_roi, xCol],
                        data_essentials[nc_pts_in_roi, yCol],
                        s=1.0,
                        c="grey",
                        zorder=1,
                        marker=".",
                    ...
                    cluster_outlines_fname = os.path.join(
                        outputpath_summary,
                        ps_current["FilePrefix"] + " - Cluster Outlines.",
                    )
            ...
            if DoAreaVsPointsPlot and not block_images:
                print(
                    "\tSaving Cluster-Area vs Points-per-Cluster histogram...",
                    end="",
                    flush=True,
                )

                if doUseROIs:
                    dual_plot_fname = os.path.join(
                        outputpath_summary,
                        AreaVsPointsPlot_folder,
                        ps_current["FilePrefix"]
                        + roi_suffix
                    ...
                    dual_plot_fname = os.path.join(
                        outputpath_summary,
                        AreaVsPointsPlot_folder,
                        ps_current["FilePrefix"] + " - Points vs Area",
                    )
                ...
                g.plot_joint(
                    plt.scatter, color="black", edgecolor=None, marker=".", s=1
                )
                g.plot_joint(
                    sns.kdeplot,
                    n_levels=20,
    ...
    if export_pool_columns:
        ...
        PtsPerCluster_pzf = np.full_like(
            np.empty((max_length, max_file_rois)), np.nan
        )
        AreaPerCluster_pzf = np.full_like(
            np.empty((max_length, max_file_rois)), np.nan
        )
        DensityPerCluster_pzf = np.full_like(
            np.empty((max_length, max_file_rois)), np.nan
        )
        ...
        for i in range(max_file_rois):
            PtsPerCluster_pzf[
                : len(pts_per_cluster_pool[i]), i
            ] = pts_per_cluster_pool[i]
        ...
        # points per cluster
        pool_fname_save = os.path.join(
            outputpath_summary,
            ForPrismFolder,
            "points-per-cluster_pool_by_cols.csv",
        )
        np.savetxt(
            pool_fname_save,
            PtsPerCluster_pzf,
        ...
        pool_fname_save = os.path.join(
            outputpath_summary,
            ForPrismFolder,
            "area-per-cluster_pool_by_cols.csv",
        )
        np.savetxt(
            pool_fname_save,
            AreaPerCluster_pzf,
        ...
        pool_fname_save = os.path.join(
            outputpath_summary,
            ForPrismFolder,
            "density-per-cluster_pool_by_cols.csv",
        )
        np.savetxt(
            pool_fname_save,
            DensityPerCluster_pzf,
            "%.6f",
            delimiter=",",
            header=pool_header,
            comments="",
        ...
        PtsPerCluster_pzf = np.full_like(
            np.empty((max_file_rois, max_length)), np.nan
        )
        AreaPerCluster_pzf = np.full_like(
            np.empty((max_file_rois, max_length)), np.nan
        )
        ...
        pool_fname_save = os.path.join(
            outputpath_summary,
            ForPrismFolder,
            "points-per-cluster_pool_by_rows.csv",
        )
        str_data = np.char.mod("%.0f", PtsPerCluster_pzf)
        np.savetxt(
            pool_fname_save,
            np.hstack((row_headers, str_data)),
            delimiter=", ",
            fmt="%s",
        )
        ...
        pool_fname_save = os.path.join(
            outputpath_summary,
            ForPrismFolder,
            "area-per-cluster_pool_by_rows.csv",
        )
        ...
        pool_fname_save = os.path.join(
            outputpath_summary,
            ForPrismFolder,
            "density-per-cluster_pool_by_rows.csv",
        )
    ...
    OverAllDump_headers = (
        "Points per Cluster,Area per Cluster, Density per Cluster"
    )
    overalldump_fname = os.path.join(
        outputpath_summary, ForPrismFolder, "ALL_clusters_pooled.csv"
    )
    np.savetxt(
        overalldump_fname,
        OverAllDump,
    ...
    if doPrismImportableFile:
        ...
        column_titles = [
            ...
            "RoI",
            "TotalPoints",
            "TotalClustered Points",
            "PercentPointsClustered",
            "TotalClusters",
            "MedianPointsPerCluster",
            "MedianAreaPerCluster",
            "MedianPointDensityPerCluster",
            "ROI Area",
        ]

        repeats = np.tile(
            np.array(range(prism_split_replicants)),
            (prism_split_conditions, 1),
        ).reshape((prism_split_rows, 1), order="F")
        ...
        for prism_col in range(prism_split_columns):
            ...
            tmp_data = np.zeros(
                (prism_split_replicants, prism_split_conditions)
            )

            insert_row = 0
            for start_rep in range(prism_split_replicants):
                keep_repeats = repeats == start_rep
                tmp_data[insert_row, :] = stats_collector[
                    keep_repeats[:, 0], prism_col
                ].T
                insert_row += 1
    ...
    if EmailUponCompletion:
        CompletionMsg = (
            "Extract cluster information from ROIs\r\n\r\nData folder:\r\n"
            + inputpath_data
            + "\r\n\r\nROI folder:\r\n"
        )
        if doUseROIs:
