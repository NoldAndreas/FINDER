...
mdl_decay_rate = mdl_learning_rate / (
    mdl_epoch_factor * mdl_epochs
)  # zero = no change in learning rate; it's always mdl_learning_rate for all epochs.
...
if __name__ == "__main__":
    ...
    model_json_fname = os.path.splitext(
        os.path.basename(os.path.normpath(modeljson))
    )[0]
    ...
    if os.path.exists(modeljson):
        ...
        outputfolder = os.path.join(
            model_home_folder,
            timestamp + " - " + str(CVSplits) + "-fold Cross-Validation",
        )
    ...
    if "TrainValTest_data_fname" in locals() or "inputpkl" in locals():
        ...
        if "recall_data" in locals():
            recycleRecallData = fn_etc.askforinput(
                message="Do you want to re-use the raw (distances) training, validation, and testing data (loaded from "
                + previousdata
                + ")?",
            ...
            if recycleRecallData.lower() in ["y", "yes"]:
                fn_etc.ok_msg(
                    "Using the existing training, validation, and testing data."
                )
                Load_PKL = False

    if Load_PKL:
        # We are loading a file but we can still suggest the last-used location as default.
        if "TrainValTest_data_fname" in locals() and os.path.exists(
    ...
    if (
        "X_training_data"
        and "X_validation_data"
        and "X_testing_data" in locals()
        and not Load_PKL
    ):
        recyclePreparedData = fn_etc.askforinput(
            message="Existing prepared data (by "
            + PreparationType
            + " method) for training, validation, and testing exists. Do you want to re-use it?",
            errormessage="Please enter y or n",
            defaultval=default_input_pkl,
            isvalid=lambda v: v.lower() in ["y", "n", "yes", "no"],
        )
        ...
        if recyclePreparedData.lower() in ["n", "no"]:
            ...
            RedoDataPrep = False
            fn_etc.ok_msg(
                "Training, validation, and testing data will be re-used. NB: These data were prepared as '"
                + PreparationType
                + "'"
            )
    ...
    if RedoDataPrep:
        ...
        load_order = pickle.load(
            pickle_open_file
        )  # retrieve the first item, which is the list in which everything was saved
        for item in load_order:
            locals()[item] = pickle.load(
                pickle_open_file
            )  # load each item and convert it into a local variable named according to 'item'
        pickle_open_file.close()

        # The following variables will already have been loaded.
        loaded_pickle = (
            True  # this is to avoid dropping into the if statement below
        )
        if not loaded_pickle:
            ...
            X_validation_distances_raw = (
                None  # In case you are trying to search
            )
        ...
        fn_etc.ok_msg(
            "Training, validation, and testing data were unpickled from "
            + inputpkl
        )
        ...
        if CVDataFrom == "training":
            ...
            fn_etc.err_msg(
                "Can't work out which data to use for Cross Validation!"
            )
            raise ValueError(
                "Cross Validation data must be from either the Training, Validation, or Testing dataset. Check 'CVDataFrom' variable..."
            )

        fn_etc.ok_msg(
            "Cross Validation data (using the "
            + CVDataFrom
            + " dataset) was unpickled from "
            + inputpkl
        )

        # Prepare the CrossValidation data for the model
        X_CrossValidation = fn_normalize.normalize_dists(
            X_CrossValidation_raw,
            total_CrossValidation_count,
    ...
    kfold = StratifiedKFold(
        n_splits=CVSplits, shuffle=True, random_state=todays_seed
    )
    cvscores = []
    ...
    for train, validate in kfold.split(X_CrossValidation, Y_CrossValidation):
        ...
        fn_etc.progress_msg(
            str(CV_iteration_tracker + 1) + " of " + str(CVSplits),
            modelID
            + " "
            + str(CVSplits)
        ...
        modelCV = (
            None  # Wipe out any existing (trained) versions of this model.
        )
        modelCV = keras.models.model_from_json(
            loaded_model_json
        )  # Build model fresh from JSON
        ...
        CV_train_accu[
            "Round " + str(CV_iteration_tracker + 1)
        ] = CV_history.history["acc"]
        CV_train_loss[
            "Round " + str(CV_iteration_tracker + 1)
        ] = CV_history.history["loss"]

        scores = modelCV.evaluate(validate_X, validate_Y, verbose=3)
        cvscores.append(scores[1] * 100)

        print(
            "Finished round %i with Accuracy: %.2f%% and Loss: %.4f"
            % (CV_iteration_tracker + 1, scores[1] * 100, scores[0])
        )
    ...
    ax_CVTrnVal_Accu = fig_CVTrnVal_Accu.add_subplot(
        111, facecolor=plotbgcolor
    )
    ...
    ax_CVTrnVal_Loss = fig_CVTrnVal_Loss.add_subplot(
        111, facecolor=plotbgcolor
    )
