...
if __name__ == "__main__":
    ...
    for fileIdx in range(starting_index, finishing_index):
        ...
        if ps_current["doSetUpAxes"] == True:
            ...
            ps_current["xMin"] = (
                np.floor(xmin / ps_current["AutoAxesNearest"])
                * ps_current["AutoAxesNearest"]
            )
            ps_current["xMax"] = (
                np.ceil(xmax / ps_current["AutoAxesNearest"])
                * ps_current["AutoAxesNearest"]
            )
            ps_current["yMin"] = (
                np.floor(ymin / ps_current["AutoAxesNearest"])
                * ps_current["AutoAxesNearest"]
            )
            ps_current["yMax"] = (
                np.ceil(ymax / ps_current["AutoAxesNearest"])
                * ps_current["AutoAxesNearest"]
            )

            ps_current["ImageSize"] = np.max(
                (
                    ps_current["xMax"] - ps_current["xMin"],
                    ps_current["yMax"] - ps_current["yMin"],
                )
            )
        ...
        if doSanityCheck:
            print(
                "SanityCheck: \t X data in column " + str(ps_current["xCol"])
            )
            print(
                "SanityCheck: \t Y data in column " + str(ps_current["yCol"])
            )
            xy_range = [
                np.floor(min(datatable[:, ps_current["xCol"]])),
            ...
            print(
                "SanityCheck: \t Checking the data makes sense ... ",
                end="",
                flush=True,
            )
            try:
            ...
            if aspect_ratio < 0.1 or aspect_ratio > 10:
                fn_etc.err_msg(
                    "Extreme xy-range aspect ratio ("
                    + str(round(aspect_ratio, 5))
                    + ") - check xy-column input..."
                )
                raise ValueError(
        ...
        if ps_current["labels_col"] == ps_current["clustID_col"]:
            ...
            datatable = np.concatenate(
                (datatable, ClusteredLabels[:, None]), axis=1
            )
        ...
        fn_etc.make_image(
            ...
            os.path.join(
                output_dir,
                ps_current["FilePrefix"] + "_PointsOnly." + PreviewImageFormat,
            ),
            ps_temp,
            None,
        )
        ...
        if ps_current["clustID_col"] > 0:
            ...
            print(
                "Saving image of points colours by cluster membership... ",
                end="",
                flush=True,
            )
            fn_etc.make_image(
