...
from keras.callbacks import (
    ModelCheckpoint,  # save partially trained models after each epoch
)
...
mdl_features_length = (
    100  # how many samples (near-neighbours) do we have for our input data?
)
...
mdl_labels = (
    1  # how many labels do we need to find, i.e. size of the final output?
)
...
def build_model():
    ...
    classifier_model.add(
        Dense(32, activation="relu", input_shape=input_data_shape)
    )
...
delete_autosaved_models = (
    True  # Delete partial models once all epochs complete.
)
...
ExportImageFormat = [
    "eps",
    "png",
]  # file extension(s) of the saved images. Use 'eps', 'png', 'jpg', 'svg'.
...
if __name__ == "__main__":
    ...
    if cmapfile == "jet":
        fn_etc.warn_msg(
            "You have specified the 'jet' colormap, with its infamous perceptual ambiguity problems.\r\n"
    ...
    elif "TrainValTest_data_fname" in locals() and os.path.exists(
    ...
    load_order = pickle.load(
        pickle_open_file
    )  # load first item: the list of names
    for item in load_order:
        locals()[item] = pickle.load(
            pickle_open_file
        )  # load each and name them
    ...
    if X_training_distances_raw.shape[1] != mdl_features_length:
        ...
        fn_etc.err_msg(
            "You specified "
            + str(mdl_features_length)
            + " near-neighbours (variable 'mdl_features_length' in this script) but the imported training data is for "
            + str(X_training_distances_raw.shape[1])
            + " neighbours..."
        )
        ...
        if recheck_features_len in ["1"]:
            ...
            raise ValueError(
                "Please check that the size of your training data matches the 'mdl_features_length' setting in this script (or change either, as desired)."
            )
            ...
            input_data_shape = (
                mdl_features_length,
                mdl_features_types,
            )  # this is used when building the model so it needs updating too.
            fn_etc.info_msg(
                "Changed variable 'mdl_features_length' to "
                + str(mdl_features_length)
                + " near-neighbours"
            )
            ...
            fn_etc.info_msg(
                "Trimming the original input data pool from "
                + str(X_training_distances_raw.shape[1])
                + " near-neighbours to match 'mdl_features_length' ("
                + str(mdl_features_length)
                + " near-neighbours)"
            )

            X_training_distances_raw2 = X_training_distances_raw[
                :, :mdl_features_length, :
            ]
            X_validation_distances_raw = X_validation_distances_raw[
                :, :mdl_features_length, :
            ]
            X_testing_distances_raw = X_testing_distances_raw[
                :, :mdl_features_length, :
            ]

    if (
        len(X_training_distances_raw.shape) > 2
        and X_training_distances_raw.shape[2] != mdl_features_types
    ):

        fn_etc.err_msg(
            "You specified "
            + str(mdl_features_types)
            + " feature(s) but the training data was done with "
            + str(X_training_distances_raw.shape[2])
            + " feature(s)..."
        )
        ...
        if recheck_features_type in ["1"]:
            ...
            raise ValueError(
                "Please give training data which matches the settings in this script."
            )
            ...
            fn_etc.info_msg(
                "Changed variable 'mdl_features_types' to support "
                + str(mdl_features_types)
                + " feature types"
            )
    ...
    if RedoDataPrep:
        checkPrepType = fn_etc.askforinput(
            message="This script has Preparation Type set to "
            + PreparationType
        ...
        X_training_data = fn_normalize.normalize_dists(
            X_training_distances_raw, total_training_count, PreparationType
        )
        X_validation_data = fn_normalize.normalize_dists(
            X_validation_distances_raw, total_validation_count, PreparationType
        )
        X_testing_data = fn_normalize.normalize_dists(
            X_testing_distances_raw, total_testing_count, PreparationType
        )
        ...
        if X_validation_data.ndim != 3:
            X_validation_data = X_validation_data.reshape(
                (
        ...
        if X_testing_data.ndim != 3:
            X_testing_data = X_testing_data.reshape(
                (total_testing_count, ps["FurthestFriend"], mdl_features_types)
            )
    ...
    if new_load_model in ["1"]:
        ...
        modelID_full = (
            ...
            + PreparationType
            + " - Train("
            + str((round(total_training_count / 100) / 10))
            + "k) - Val("
            + str((round(total_validation_count / 100) / 10))
            + "k)"
        )
        ...
        this_model_notes = fn_etc.askforinput(
            message="Enter any custom quick notes here, to be saved alongside Model "
            + modelID
        ...
        model_notes_file = os.path.join(
            outputpath_model, modelID + " - notes.txt"
        )
        with open(model_notes_file, "w") as text_file:
            ...
            text_file.write(
                "Data preparation method: " + PreparationType + "\n"
            )
        ...
        with open(model_summary_fname + " - Model Summary.txt", "a") as fh:
            ...
            print(
                "Learning rate decay epoch factor:\t" + str(mdl_epoch_factor),
                file=fh,
            )
            print(
                "Learning rate decay function value:\t" + str(mdl_decay_rate),
                file=fh,
            )
        ...
        callbacks_list = (
            []
        )  # we can pass several callbacks, our list starts out empty.

        if auto_save_model:
            model_partial_base = "_Partial_Epoch-{epoch:02d}_ValAcc-{val_acc:.2f}_ValLoss-{val_loss:.2f}"
            model_partial_fname = os.path.join(
                outputpath_model, modelID + model_partial_base + ".h5"
            )
        ...
        if log_to_csv:
            csv_logger = keras.callbacks.CSVLogger(
                os.path.join(outputpath_model, modelID + "_training_log.csv")
            )
        ...
        model.save(
            model_fname, include_optimizer=False
        ...
        ax_TrnVal_Accu = fig_TrnVal_Accu.add_subplot(
        ...
        # add labels etc
        plt.title(
            "Model " + modelID + " training & validation accuracy",
            color=plottxtcolor,
        )
        ...
        ax_TrnVal_Loss = fig_TrnVal_Loss.add_subplot(
            111, facecolor=plotbgcolor
        )
        ...
        # add labels etc
        plt.title(
            "Model " + modelID + " training & validation loss",
            color=plottxtcolor,
        )
        ...
        for ExportImage in ExportImageFormat:
            ...
            fname_out = os.path.join(
                outputpath_model,
                model_summary_fname
                + " - Training & Validation Loss Evolution."
                + ExportImage,
            )
            fig_TrnVal_Loss.savefig(
                fname_out,
                dpi=300,
                bbox_inches=0,
                facecolor=plotbgcolor,
        ...
        plot_model(
            model,
            to_file=os.path.join(outputpath_model, modelID_full + ".png"),
            show_shapes=True,
        )  # Save the model diagram as png file
        ...
        if os.path.exists(model_fname):
            ...
            outputpath_model = os.path.join(
                model_home_folder, timestamp + " - testing"
            )

            # extract the model ID
            modelID_full = os.path.splitext(
                os.path.basename(os.path.normpath(model_fname))
            )[0]
            modelID = modelID_full.split()[0]
            model_summary_fname = os.path.join(
                outputpath_model, modelID + " - " + timestamp
            )

            # get some showstopping parameters from the model
            model_config = model.get_config()
            RequiredInputSize = model_config[0]["config"]["batch_input_shape"][
                1
            ]
            ...
            if X_training_data.shape[1] != RequiredInputSize:
                raise ValueError(
                    "Please make sure your model and input data sizes are matched.\r\n"
                    + "Model "
                    + modelID
                    + " expects "
    ...
    print("Training Data:")
    loss_training, accuracy_training = model.evaluate(
        X_training_data, Y_training_data, verbose=1
    )
    print(
        "\tAccuracy:\t%.2f%%\n\tLoss:\t\t%.2f"
        % (accuracy_training * 100, loss_training)
    )

    print("Validation Data:")
    loss_validation, accuracy_validation = model.evaluate(
        X_validation_data, Y_validation_data, verbose=1
    )
    print(
        "\tAccuracy:\t%.2f%%\n\tLoss:\t\t%.2f"
        % (accuracy_validation * 100, loss_validation)
    )

    print("Testing Data:")
    loss_testing, accuracy_testing = model.evaluate(
        X_testing_data, Y_testing_data, verbose=1
    )
    print(
        "\tAccuracy:\t%.2f%%\n\tLoss:\t\t%.2f"
        % (accuracy_testing * 100, loss_testing)
    )

    ### Scoring by F1, precision, and recall metrics
    X_testing_output = model.predict(
        X_testing_data
    )  # spits out probabilities for classification
    ...
    if len(label_names) > 2:
        ...
        X_testing_predictions = [
            float(np.round(x - (pred_threshold - 0.5)))
            for x in X_testing_output
        ]
    ...
    np.savetxt(
        model_summary_fname + " - Testing Probabilities.tsv",
        X_truth_vs_preds,
        delimiter="\t",
    )
    ...
    for label in range(len(label_names)):
        ...
        true_label_idx = np.where(X_truth_vs_preds[:, 0] == label)[
            0
        ]  # find the points which are truly this label
        if mdl_labels == 1:
            label_hist = X_testing_output[true_label_idx][
                :, 0
            ]  # find the scores for those points, binary it's all in one column
        else:
            label_hist = X_testing_output[true_label_idx][
                :, label
            ]  # find the scores for those points, multilabel have scores in multicolumns
    ...
    plt.title(
        "Testing Model " + modelID + " - Score Distribution",
        color=plottxtcolor,
    )
    ...
    fmeasure3 = f1_score(
        Y_testing, X_testing_predictions, average="weighted"
    )  # calculates metric for each label and finds unweighted mean. Does not account for label imbalance
    precision = precision_score(
        Y_testing, X_testing_predictions, average="macro"
    )
    ...
    if len(label_names) > 2:
        ...
        pred_thr_formatted = (
            "threshold = " + str(pred_threshold) + " (binary classification)"
        )
    ...
    training_ratio_formatted = [
        "%.2f" % elem for elem in training_label_fraction
    ]
    validation_ratio_formatted = [
        "%.2f" % elem for elem in validation_label_fraction
    ]
    testing_ratio_formatted = [
        "%.2f" % elem for elem in testing_label_fraction
    ]

    with open(
        model_summary_fname + " - Performance Summary.txt", "a"
    ) as fstats:
        ...
        writer.writerow(
            ["", "Loss: ", "", "", round(loss_training, ndigits=3)]
        )
        writer.writerow(
            ["------------------------------------------------------"]
        )
        ...
        writer.writerow(
            ["", "Loss: ", "", "", round(loss_validation, ndigits=3)]
        )
        writer.writerow(
            ["------------------------------------------------------"]
        )
    ...
    confmat_trn_probabilities = model.predict(
        X_training_data, batch_size=64
    ...
    if len(label_names) > 2:
        ...
        confusionmatrix_trn = confusion_matrix(
            Y_training_data.argmax(axis=-1), confmat_trn_predictions
        )
    else:
        confmat_trn_predictions = [
            float(np.round(x)) for x in confmat_trn_probabilities
        ]  # convert probability into boolean
        confusionmatrix_trn = confusion_matrix(
            Y_training_data, confmat_trn_predictions
        )

    # Validation data confusion matrix
    confmat_val_probabilities = model.predict(
        X_validation_data, batch_size=64
    )  # probability assessment of event being in cluster(~1) or not (~0)
    if len(label_names) > 2:
        confmat_val_predictions = confmat_val_probabilities.argmax(axis=-1)
        confusionmatrix_val = confusion_matrix(
            Y_validation_data.argmax(axis=-1), confmat_val_predictions
        )
    else:
        confmat_val_predictions = [
            float(np.round(x)) for x in confmat_val_probabilities
        ]  # convert probability into boolean
        confusionmatrix_val = confusion_matrix(
            Y_validation_data, confmat_val_predictions
        )

    # Testing data confusion matrix
    confmat_test_probabilities = model.predict(
        X_testing_data, batch_size=64
    )  # probability assessment of event being in cluster(~1) or not (~0)
    if len(label_names) > 2:
        confmat_test_predictions = confmat_test_probabilities.argmax(axis=-1)
        confusionmatrix_test = confusion_matrix(
            Y_testing_data.argmax(axis=-1), confmat_test_predictions
        )
    else:
        confmat_test_predictions = [
            float(np.round(x)) for x in confmat_test_probabilities
        ]  # convert probability into boolean
        confusionmatrix_test = confusion_matrix(
            Y_testing_data, confmat_test_predictions
        )

    # Plot absolute values confusion matrix
    fig_AbsCM_trn = fn_etc.plot_confusion_matrix(
        confusionmatrix_trn,
        classes=["NC", "C"],
        cmap=cmap,
    ...
    for ExportImage in ExportImageFormat:
        fname_out = os.path.join(
            outputpath_model,
            model_summary_fname
            + " - Abs Confusion Matrices (training)."
            + ExportImage,
        )
        fig_AbsCM_trn.savefig(
            fname_out,
            dpi=300,
            bbox_inches=0,
            facecolor=plotbgcolor,
    ...
    fig_NormCM_trn = fn_etc.plot_confusion_matrix(
        confusionmatrix_trn,
        classes=["NC", "C"],
        cmap=cmap,
        plottxtcolor=plottxtcolor,
    ...
    for ExportImage in ExportImageFormat:
        fname_out = os.path.join(
            outputpath_model,
            model_summary_fname
            + " - Norm Confusion Matrices (training)."
            + ExportImage,
        )
        fig_NormCM_trn.savefig(
            fname_out,
            dpi=300,
            bbox_inches=0,
            facecolor=plotbgcolor,
    ...
    # Plot absolute values confusion matrix
    fig_AbsCM_val = fn_etc.plot_confusion_matrix(
        confusionmatrix_val,
        classes=["NC", "C"],
        cmap=cmap,
        plottxtcolor=plottxtcolor,
    ...
    for ExportImage in ExportImageFormat:
        fname_out = os.path.join(
            outputpath_model,
            model_summary_fname
            + " - Abs Confusion Matrices (validation)."
            + ExportImage,
        )
        fig_AbsCM_val.savefig(
            fname_out,
            dpi=300,
            bbox_inches=0,
            facecolor=plotbgcolor,
    ...
    # Plot normalized confusion matrix
    fig_NormCM_val = fn_etc.plot_confusion_matrix(
        confusionmatrix_val,
        classes=["NC", "C"],
        cmap=cmap,
        plottxtcolor=plottxtcolor,
    ...
    for ExportImage in ExportImageFormat:
        fname_out = os.path.join(
            outputpath_model,
            model_summary_fname
            + " - Norm Confusion Matrices (validation)."
            + ExportImage,
        )
        fig_NormCM_val.savefig(
            fname_out,
            dpi=300,
            bbox_inches=0,
            facecolor=plotbgcolor,
    ...
    fig_AbsCM_test = fn_etc.plot_confusion_matrix(
    ...
    for ExportImage in ExportImageFormat:
        fname_out = os.path.join(
            outputpath_model,
            model_summary_fname
            + " - Abs Confusion Matrices (testing)."
            + ExportImage,
        )
        fig_AbsCM_test.savefig(
            fname_out,
            dpi=300,
            bbox_inches=0,
            facecolor=plotbgcolor,
    ...
    fig_NormCM_test = fn_etc.plot_confusion_matrix(
        confusionmatrix_test,
        classes=["NC", "C"],
        cmap=cmap,
        plottxtcolor=plottxtcolor,
    ...
    for ExportImage in ExportImageFormat:
        fname_out = os.path.join(
            outputpath_model,
            model_summary_fname
            + " - Norm Confusion Matrices (testing)."
            + ExportImage,
        )
        fig_NormCM_test.savefig(
            fname_out,
            dpi=300,
            bbox_inches=0,
            facecolor=plotbgcolor,
