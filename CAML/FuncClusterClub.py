...
def cluster_shaper(
    ...
    if (
        PtsThisCluster >= MinPtsForACluster
    ):  # we need 3 or more points to make a cluster
        ...
        if BufferEps == "dims":
            buffer_estimate = (
                sum(
                    all_points[:, 0:2].max(axis=0)
                    - all_points[:, 0:2].min(axis=0)
                )
                / all_points.shape[0]
            ) * 5
        elif BufferEps == "dens":
            ...
            if boundingarea == 0:
                # points in a line
                boundingarea = xyrange.max(
                    axis=0
                )  # If this is still zero then all the points have the same coordinates.
            buffer_estimate = np.sqrt(
                (boundingarea / all_points.shape[0]) / np.pi
            )
        elif BufferEps == "nnd":
            NNDs_this_cluster = NNDs[PtsIdx]
            buffer_estimate = 1.2 * np.mean(
                NNDs_this_cluster
            )  # the nearest-neighbour distance for this point
        else:
            buffer_estimate = BufferEps

        # convert points to Shapely multipoint
        shpPoints = MultiPoint(
            np.array([all_points[:, 0], all_points[:, 1]]).T
        )
        ...
        if shpPatches.area == 0:
            ...
            raise ValueError(
                "Cluster has an area of zero but contains "
                + str(PtsThisCluster)
                + " points. There are "
                + str(all_points.shape[0] - uniq_idx.shape[0])
                + " duplicate xy points."
            )
        ...
        if shpPatches.type == "MultiPolygon":
            ...
            for polygon in shpPatches:
                ...
                p = Path(
                    poly_coords
                )  # make a matplotlib polygon to check how many points are inside it
                if sum(p.contains_points(shpPoints)) > 1:
                    # remove small holes from this polygon, if specified
                    if DropSmallHoles:
                        polygon = drop_small_holes(
                            polygon, 0.5 * buffer_estimate
                        )
                    rebuild_shpPatches.append(polygon)
                else:  # only 1 point, demote it from clustering
                    demoted_ptID = all_points[p.contains_points(shpPoints), 2][
                        0
                    ].astype(int)
                    if VerboseOut:
                        print(
                            "Demoting isolated point "
                            + str(demoted_ptID)
                            + " from cluster "
                            + str(ThisClusterUID)
                        )
            ...
            if len(rebuild_shpPatches) == 1:
                ...
                if VerboseOut:
                    print(
                        "Have managed to exclude all points in cluster; demoting "
                        + str(ThisClusterUID)
                    )
            ...
            if sum(p.contains_points(shpPoints)) > 1:
                ...
                if DropSmallHoles:
                    shpPatches = drop_small_holes(
                        shpPatches, 0.5 * buffer_estimate
                    )
            else:
                demoted_clusters.append(ThisClusterUID)
                demoted_flag = True
                if VerboseOut:
                    print(
                        "I don't think you should be able to get into this position regarding Cluster"
                        + str(ThisClusterUID)
                    )
        ...
        if shpPatches.type == "MultiPolygon" and not demoted_flag:
            ...
            for polygon in polygons:
                ...
                if total_points_contained > MinPtsForACluster:
                    ...
                    cluster_buffer2 = (
                        len(str(total_new_polygons)) + 1
                    )  # e.g. 3

                    NewCluUID = int(
                        str(maxCluUIDs)
                        + str(ThisClusterUID).zfill(cluster_buffer1)
                        + str(valid_sub_cluster).zfill(cluster_buffer2)
                    )

                    # record the pointIDs and their new clusterID
                    reassignment_cluID = np.array(NewCluUID).repeat(
                        total_points_contained
                    )
                    reassignment_tmp = np.hstack(
                        (
                            contained_ptIDs[..., np.newaxis],
                            reassignment_cluID[..., np.newaxis],
                        )
                    )
                    ...
                    if VerboseOut:
                        print(
                            "\tNot enough points in split cluster. Demoting those points"
                        )
                    for demoted_pt in contained_ptIDs:
                        if VerboseOut:
                            print(
                                "Demoting isolated point UID_"
                                + str(demoted_pt)
                                + " from cluster "
                                + str(ThisClusterUID)
                            )
            ...
            if valid_sub_cluster == 0:
                if VerboseOut:
                    print(
                        "Splitting this cluster creates pathetic clusters. Demoting cluster "
                        + str(ThisClusterUID)
                        + " for insufficient points"
                    )
                demoted_clusters.append(ThisClusterUID)
