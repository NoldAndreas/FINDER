...
if __name__ == "__main__":
    ...
    inputpath = fn_etc.askforinput(
        message="Enter the path of the folder containing model-classified "
        + InputFileExt
        + " data tables",
    ...
    files = natsorted(
        [
            ...
            if os.path.isfile(os.path.join(inputpath, i)) and InputFileExt in i
        ]
    )
    ...
    confusion_pool = np.zeros(
        (1, 2)
    )  # ground-truth classification, model-classification ... init with a dummy row
    ...
    for fileIdx in range(current_index, total_files):
        ...
        confusionmatrix_data = confusion_matrix(
            datatable[:, 0], datatable[:, 1], labels=[0, 1]
        )
        ...
        if total_true_label_0 > 0:
            ...
            confusion_box[fileIdx, 6] = (
                confusionmatrix_data[0, 0] / total_true_label_0
            )
        else:
            confusion_box[fileIdx, 6] = np.nan

        if total_true_label_1 > 0:
            confusion_box[fileIdx, 7] = (
                confusionmatrix_data[1, 1] / total_true_label_1
            )
        else:
            confusion_box[fileIdx, 7] = np.nan

        confusion_box[fileIdx, 8] = (
            confusionmatrix_data[0, 0] + confusionmatrix_data[1, 1]
        ) / datatable.shape[0]
    ...
    if SimuCellChecker:
        confusion_out = np.concatenate(
            (confusion_box, simucell_tracker), axis=1
        )
    ...
    confusion_pool = np.delete(
        confusion_pool, (0), axis=0
    )  # remove the first row

    # Training data confusion matrix
    confusionmatrix_trn = confusion_matrix(
        confusion_pool[:, 0], confusion_pool[:, 1]
    )

    # Plot non-normalized confusion matrix
    fig_AbsCM = fn_etc.plot_confusion_matrix(
        confusionmatrix_trn,
        classes=["NC", "C"],
        cmap=cmap,
        plottxtcolor=plottxtcolor,
    ...
    fig_AbsCM.savefig(
        os.path.join(inputpath, "AAA Abs Confusion Matrices.png"),
        dpi=300,
        bbox_inches=0,
        facecolor=plotbgcolor,
        edgecolor="none",
    ...
    # Plot normalized confusion matrix
    fig_NormCM = fn_etc.plot_confusion_matrix(
        confusionmatrix_trn,
        classes=["NC", "C"],
        cmap=cmap,
        plottxtcolor=plottxtcolor,
    ...
    fig_NormCM.savefig(
        os.path.join(inputpath, "AAA Norm Confusion Matrices.png"),
        dpi=300,
        bbox_inches=0,
        facecolor=plotbgcolor,
        edgecolor="none",
