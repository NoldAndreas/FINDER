...
#    clusterInfo_2 = clusterInfo.loc[clusterInfo['index']==j,:]; 
    ...
    for i1 in np.unique(labels_1):
        ...
        for i2 in np.unique(labels_2):
            ...
            if(__checkNoOverlapClusters(i1,i2,centers_1,centers_2,radii_1,radii_2) and 
                 __checkNoOverlap_SimilarityClusters(i1,i2,labels_1,labels_2)):
...
def __checkNoOverlapClusters(i1,i2,centers_1,centers_2,radii_1,radii_2):
    c1,c2 = centers_1[i1], centers_2[i2];
    r1,r2 = radii_1[i1], radii_2[i2];
    return (np.linalg.norm(c2-c1) > r1 + r2);    

def __checkNoOverlap_SimilarityClusters(i1,i2,labels_1,labels_2):
    ...
    for i, ps in PS.iterrows():
        ...
        for j in np.arange(i+1):                  
            ...
            if(not (i==j)):
                ...
                similarityScoreMatrix[i,j] = number_labels_unique;#np.max(ps["labels"]) + 1;
