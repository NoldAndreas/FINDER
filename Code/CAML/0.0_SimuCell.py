...
PointsPerCluster = np.array([10,20])                    # Each cluster will contain this many points.
Radii_min = np.array([0])                               # Nearest a point in the cluster can be to the cluster centre. 0 forms solid clusters
Radii_max = np.array([20,40])                           # Furthest a pointin the cluster can be from cluster centre. Effectively cluster radius.
PercentPointsInClusters = np.array([0, 75, 50, 25, 10]) # Percentage of all points which are clustered.
PointsPerMicronSquared = np.array([100])                # Overall density of points within the 'cell'.
...
DensityRatio_min = 1.5  # Scenarios resulting in a density ratio < 1.5 will be removed
DensityRatio_max = 100  # Scenarios resulting in a density ratio > 100 will be removed
...
SMAC_DeleteClustersFromCentral = True  # If False then clustered points will be deleted from cSMAC
SMAC_DeleteClustersFraction = 0.95     # 
SMAC_DeleteNonCluPtsFromCentral = True # If False then NC points deleted from outside cSMAC
...
# Cropped Regions
# ========================================================================
DoCentreCroppedRegions = False # create centre-cropped subregions of each image?
...
if __name__ == '__main__':
    ...
    if doGenerateJSON:
        ...
        FurthestNeighbour = fn_etc.askforinput(
                ...
                errormessage= 'An integer is required and must be greater than ' + NearestNeighbour + ' (Nearest Neighbour)',
                defaultval= '100',
    ...
    if any(PointsPerMicronSquared) <= 0:
        PointsPerMicronSquared = np.array([PtsPerMS for PtsPerMS in PointsPerMicronSquared if PtsPerMS > 0])
        fn_etc.warn_msg('PointsPerMicronSquared values must be greater than zero. Invalid values are being ignored.')
    if np.max(PointsPerMicronSquared) == 0 or PointsPerMicronSquared.shape[0] == 0:
        raise ValueError('You must give at least one positive value for the point density (\'PointsPerMicronSquared\').\n\
    ...
    if any(PercentPointsInClusters > 100):
        PercentPointsInClusters = np.array([PC for PC in PercentPointsInClusters if PC <= 100])
        fn_etc.warn_msg('PercentPointsInClusters values must be less than 100. Invalid values are being ignored.')

    gen_nocluster_hits = np.min(np.hstack((PointsPerCluster, PercentPointsInClusters, Radii_max))) <= 0
    gen_clustered_hits = np.max(np.hstack((PointsPerCluster, PercentPointsInClusters, Radii_max))) != 0
    ...
    if gen_nocluster_hits and gen_clustered_hits:
        ...
        PointsPerCluster = np.array([PpC for PpC in PointsPerCluster if PpC > 0])
        PercentPointsInClusters = np.array([PCPtsC for PCPtsC in PercentPointsInClusters if PCPtsC > 0])
        Radii_max = np.array([RadMax for RadMax in Radii_max if RadMax > 0])
        csr_scenarios = False

    AllCombinations = np.array(np.meshgrid(PointsPerMicronSquared,
                                           PointsPerCluster,
                                           PercentPointsInClusters,
                                           Radii_min,
                                           Radii_max
                                           ), dtype=float).T.reshape(-1,5)
...
#    ReplicateID_col = 6
    ...
    if csr_scenarios:
        fn_etc.info_msg(str(size_before_filtering) + ' non-clustering scenarios can created from combinations of the supplied cluster parameters.')
    else:
        fn_etc.info_msg(str(size_before_filtering) + ' clustering scenarios can created from combinations of all cluster parameters.')

    # remove any scenarios where min radius is larger than max radius
    if np.any(AllCombinations[:,RadiusMin_col] < AllCombinations[:,RadiusMax_col]):
        AllCombinations = AllCombinations[AllCombinations[:,RadiusMin_col] < AllCombinations[:,RadiusMax_col],:]
        size_after_filtering = AllCombinations.shape[0]
        if size_after_filtering < size_before_filtering:
            print('\t\t... ' + str(size_before_filtering - size_after_filtering) + ' scenarios removed because their maximum radius was less than the minimum radius.')
            size_before_filtering = AllCombinations.shape[0]
...
#    ClustersPerMicronSqd = np.full((AllCombinations.shape[0],1), np.nan)
    ...
    for comboIDX, clusteringcombo in enumerate(AllCombinations):
        if clusteringcombo[PercentPtsClustered_col] > 0 and clusteringcombo[PtsPerCluster_col] > 0 :
            ClustersPerMicronSqd[comboIDX] = (clusteringcombo[PtsPerMicronSqd_col] * (0.01 * clusteringcombo[PercentPtsClustered_col])) / clusteringcombo[PtsPerCluster_col]

    AllCombinations = np.concatenate((AllCombinations, ClustersPerMicronSqd), axis=1) #glue CpMS to the list of conditions
    ...
    TotalPointsClusteredPerMicronSqd = (AllCombinations[:, PtsPerMicronSqd_col, None] * (0.01 * AllCombinations[:, PercentPtsClustered_col, None]))
    TotalAreaClusteredPerMicronSqd = AllCombinations[:, RadiusMax_col, None] * AllCombinations[:, RadiusMax_col, None] * np.pi * ClustersPerMicronSqd * 1E-6
    if np.all(TotalAreaClusteredPerMicronSqd == 0):
        DensityInClusters = np.zeros((AllCombinations.shape[0], 1))
    else:
        DensityInClusters = TotalPointsClusteredPerMicronSqd / TotalAreaClusteredPerMicronSqd
    AllCombinations = np.concatenate((AllCombinations, TotalPointsClusteredPerMicronSqd, TotalAreaClusteredPerMicronSqd, DensityInClusters), axis=1) #glue CpMS to the list of conditions
    TotalPtsClustered_col = 6
    TotalAreaClustered_col = 7
    PtDensityInClusters_col = 8
    
    # Calculate density outside of clustered areas
    TotalPointsNotClusteredPerMicronSqd = AllCombinations[:,PtsPerMicronSqd_col, None] - TotalPointsClusteredPerMicronSqd
    TotalAreaNotClusteredPerMicronSqd = 1 - TotalAreaClusteredPerMicronSqd
    DensityOutSideClusters = TotalPointsNotClusteredPerMicronSqd / TotalAreaNotClusteredPerMicronSqd
    AllCombinations = np.concatenate((AllCombinations, TotalPointsNotClusteredPerMicronSqd, TotalAreaNotClusteredPerMicronSqd, DensityOutSideClusters), axis=1)
    TotalPtsNotClustered_col = 9
    TotalAreaNotClustered_col = 10
    PtDensityOutsideClusters_col = 11
    
    DensityOutSideClusters[DensityOutSideClusters == 0] = np.NaN # convert zero outside-cluster-densities to NaN to avoid divide-by-zero errors (e.g. when there's 100% points clustered)
    ...
    del(ClustersPerMicronSqd, TotalPointsClusteredPerMicronSqd, TotalAreaClusteredPerMicronSqd, DensityInClusters)
    del(TotalPointsNotClusteredPerMicronSqd, TotalAreaNotClusteredPerMicronSqd, DensityOutSideClusters)
    ...
    if min(PercentPointsInClusters) > 0 and max(PercentPointsInClusters) != 0:
        ...
        AllCombinations = AllCombinations[AllCombinations[:, TotalAreaClustered_col] < 1, :]
        size_after_filtering = AllCombinations.shape[0]
        if size_after_filtering < size_before_filtering:
            print('\t\t... ' + str(size_before_filtering - size_after_filtering) + ' scenarios removed for describing clusters that would occupy more area than is available.')
            size_before_filtering = AllCombinations.shape[0]
    
    if FilterScenarios and not csr_scenarios:
        # Keep points within the specified range of clusters per micron squared
        AllCombinations = AllCombinations[AllCombinations[:, ClustersPerMicronSqd_col] < CpM_max, :]
        AllCombinations = AllCombinations[AllCombinations[:, ClustersPerMicronSqd_col] > CpM_min, :]
        size_after_filtering = AllCombinations.shape[0]
        if size_after_filtering < size_before_filtering:
            print('\t\t... ' + str(size_before_filtering - size_after_filtering) + ' scenarios removed for failing the cluster density limits ('+ str(CpM_min) +' < CpMS > '+ str(CpM_max) +').')
            size_before_filtering = AllCombinations.shape[0]
        
        # Keep points within the specified range of clusters per micron squared
        AllCombinations = AllCombinations[AllCombinations[:, DensityRatio_Col] < DensityRatio_max, :]
        AllCombinations = AllCombinations[AllCombinations[:, DensityRatio_Col] > DensityRatio_min, :]
        size_after_filtering = AllCombinations.shape[0]
        if size_after_filtering < size_before_filtering:
            print('\t\t... ' + str(size_before_filtering - size_after_filtering) + ' scenarios removed for failing the density ratio limits ('+ str(DensityRatio_min) +' < Density In/Out > '+ str(DensityRatio_max) +').')
            size_before_filtering = AllCombinations.shape[0]
    
    # Add the no-clustering (CSR) conditions
    if not csr_scenarios and AppendNonClustered:
        ZeroCombinations = np.zeros((PointsPerMicronSquared.shape[0], AllCombinations.shape[1]))
        ZeroCombinations[:,PtsPerMicronSqd_col] = PointsPerMicronSquared
    
        ZeroCombinations[:, TotalPtsNotClustered_col] = ZeroCombinations[:,PtsPerMicronSqd_col]
        ZeroCombinations[:, TotalAreaNotClustered_col] = 1
        ZeroCombinations[:, PtDensityOutsideClusters_col] = ZeroCombinations[:,PtsPerMicronSqd_col]
        ZeroCombinations[:, DensityRatio_Col] = np.NaN
    
        AllCombinations = np.concatenate((ZeroCombinations, AllCombinations), axis=0)
        
        size_after_filtering = AllCombinations.shape[0]
        if size_after_filtering > size_before_filtering:
            print('\t\t... ' + str(size_after_filtering - size_before_filtering) + ' non-clustering scenarios were added for each given point-density.')

    if TotalReplicates < 1 or not type(TotalReplicates) == int:
        TotalReplicates = 1
        print('\t\t... Value for replicates must be a positive integer (you gave ' + str(TotalReplicates) + '). Setting it to 1 replicate per scenario.')
        
    TotalScenarios = AllCombinations.shape[0]
    TotalSimulations = TotalScenarios * TotalReplicates
    
    print('\t\t... ' + str(TotalScenarios) + ' viable scenarios passed checks.')
    ...
    if TotalScenarios > 0:
        ...
        fn_etc.info_msg('Generating ' + str(TotalSimulations) + ' datasets in total: ' + str(TotalScenarios) + ' scenarios, ' + str(TotalReplicates) + ' repeats per scenario')
    else:
        raise ValueError('Clustering parameters ultimately combined to create no viable clustering scenarios! Please check the clustering and filtering parameters you have given.')
    
    gnd_truth = np.zeros((TotalSimulations,16))
    
    if starting_scenario != 0:
        reset_starting_scenario = fn_etc.askforinput(
            message = 'The starting scenario is set to' + str(starting_scenario) + '. Is this correct? (Y or N). If NO then processing will begin with the first scenario in the list',
            errormessage= 'Type Y or N',
            defaultval= 'y',
            isvalid = lambda v : v.lower() in ['y','n','yes','no'])
        ...
        if reset_starting_scenario.lower() in ['n','no']:
            starting_scenario = 0
            print('Starting scenario has been reset and processing will begin from the first scenario in the list.')
        else:
            print('Keeping the current scenario. The first scenario generated by this session will be number ' + str(starting_scenario + 1) + ' in the list.')
    ...
    if TotalReplicates > 1:
        ...
        print('\tImages will be saved for repeat number ' + str(lucky_simRepeat + 1) + ' in each scenario.')
        ...
        defaultval= 'n',
        isvalid = lambda v : v.lower() in ['y','n','yes','no'])
        if doImageEachReplicate: lucky_simRepeat = 0
    else:
        lucky_simRepeat = 0
    ...
    for scenario in range(starting_scenario, TotalScenarios):
        ...
        for simRepeat in range(0,TotalReplicates):
            ...
            scenarioUID = str(scenarioUIDint+1).zfill(len(str(TotalSimulations)))
            
            build_str = scenarioUID + '_PpMS(' + str(ptsperum2) + ')_PpC(' + str(ptsperclu) + ')_PC(' + str(percentptsinclu) + ')_r(' + str(rad_min) + '-' + str(rad_max) + ')_CpMS(' + str(round(cluperum2,3)) + ')'

            if simRepeat == 0:
                print(str(scenario + 1) + '/' + str(TotalScenarios) + '\tCreating ' + str(TotalReplicates), '× [' + str(ptsperum2) + ' PpMS, ' + str(ptsperclu) + ' PpC, ' + str(percentptsinclu) + ' %PC, r(' + str(rad_min) + '-' + str(rad_max),'), ' + str(round(cluperum2,4)) + ' CpMS]')
        
            save_file_name = build_str + '_cellID(' + str(simRepeat + 1) + ')'
        
            if simRepeat == lucky_simRepeat:
                cell_bounds, csmac_bounds = fn_gendata.MakeCellOutline(ImageSize, Create_cSMAC, outlines_dir, save_file_name, True)
            else:
                cell_bounds, csmac_bounds = fn_gendata.MakeCellOutline(ImageSize, Create_cSMAC, outlines_dir, save_file_name, False)
        
            # Save outlines as JSON ROIs
            json_fname = os.path.join(outlines_dir, save_file_name + '_ROI_1.json')
        ...
        #    cell_multipoint = MultiPoint(cell_bounds)
            ...
            try:
                shrunk_cell_polygon = cell_polygon.buffer(-2.1*rad_max)[0] # sometimes shrinking creates a multipolygon. We take the first (large) shape.
            except:
            ...
            Ch1_TightCluster_Eventlist, _ = fn_gendata.SprinklePoints(cluperum2,
                                                                ptsperclu,
                                                                rad_min,
                                                                rad_max,
                                                                ptsperum2,
            ...
            _, uniq_idx = np.unique(Ch1_TightCluster_Eventlist[:,:2], axis=0, return_index=True)
            if uniq_idx.shape[0] < Ch1_TightCluster_Eventlist.shape[0]:
                uniq_idx = np.sort(uniq_idx)
                print('\t***\t' + str(Ch1_TightCluster_Eventlist.shape[0] - uniq_idx.shape[0]) + ' points with duplicate coordinates were removed.')
                Ch1_TightCluster_Eventlist = Ch1_TightCluster_Eventlist[uniq_idx,:]
        
            # add ChannelID column
            Ch1_TightCluster_Eventlist = np.append(Ch1_TightCluster_Eventlist, np.ones((Ch1_TightCluster_Eventlist.shape[0], 1)), axis=1)
                       
            # save the data table
            if ImageSize[2] == 0:
                data_headers = 'x (nm)' + OutDelimiter + 'y (nm)' + OutDelimiter + 'Cluster UID' + OutDelimiter + 'Class Label' + OutDelimiter + 'Channel UID'
                Ch1_TightCluster_Eventlist = np.delete(Ch1_TightCluster_Eventlist, 2, 1)
                ClusterUID_col = 2
                LabelID_col = 3

            else:
                data_headers = 'x (nm)' + OutDelimiter + 'y (nm)' + OutDelimiter + 'z (nm)' + OutDelimiter + 'Cluster UID' + OutDelimiter + 'Class Label' + OutDelimiter + 'Channel UID'
                ClusterUID_col = 3
                LabelID_col = 4
...
#                    data_headers = data_headers + OutDelimiter + 'Uncertainty'
            ...
            np.savetxt(data_fname, Ch1_TightCluster_Eventlist, delimiter=OutDelimiter, fmt='%10.3f', header=data_headers, comments='')
            ...
            if simRepeat == lucky_simRepeat:
                ...
                if any(Idx_Clu):
                    ...
                    plt.scatter(Ch1_TightCluster_Eventlist[Idx_Nonclu,0], Ch1_TightCluster_Eventlist[Idx_Nonclu,1], s=1, color='grey', zorder=1)
                    plt.scatter(Ch1_TightCluster_Eventlist[Idx_Clu,0], Ch1_TightCluster_Eventlist[Idx_Clu,1], s=1, c=Ch1_TightCluster_Eventlist[Idx_Clu,ClusterUID_col])
                    ...
                    for ExportImage in ExportImageFormat:
                        lucky_fig_fname = os.path.join(lucky_fig_dir, save_file_name + ' - Highlight Preview.' + ExportImage)
                        plt.savefig(lucky_fig_fname,
                                    dpi=300,
                                    bbox_inches=0,
                                    facecolor='none',
                                    edgecolor='none',
                ...
                fig = plt.figure(figsize=(30, 30))
                ax = fig.add_subplot(111, facecolor='none')
                plt.scatter(Ch1_TightCluster_Eventlist[:,0], Ch1_TightCluster_Eventlist[:,1], s=1, color='black')
                ax.set_xlim(0, ImageSize[0])
                ax.set_ylim(0, ImageSize[1])
                ax.set_aspect('equal')
                plt.tight_layout()
                ...
                for ExportImage in ExportImageFormat:
                    lucky_fig_fname = os.path.join(lucky_fig_dir, save_file_name + ' - Flat Preview.' + ExportImage)
                    plt.savefig(lucky_fig_fname,
                                dpi=300,
                                bbox_inches=0,
                                facecolor='none',
            ...
            ActualClusterCount = np.unique(Ch1_TightCluster_Eventlist[:,ClusterUID_col]).shape[0] - 1 # can't use max as some clusters could be deleted from within the csmac (if that option is used) or from outside the cell ROI.
            ActualClustersPerMicron = ActualClusterCount / SimuCellAreaMicronsSqd
            TotalClusteredPoints = np.sum(Ch1_TightCluster_Eventlist[:,ClusterUID_col] > 0)
            ActualPercentClustered = TotalClusteredPoints / TotalPoints * 100
            ActualInCellPointDensity =  np.sum(Ch1_TightCluster_Eventlist[:,ClusterUID_col] > -1) / SimuCellAreaMicronsSqd
            ...
            gnd_truth_tmp[0,3] = percentptsinclu                # Percentage points clustered
            ...
            gnd_truth_tmp[0,8] = ActualClusterCount             # Total number of clusters
            gnd_truth_tmp[0,9] = ActualClustersPerMicron        # Actual cluster density in this image (clusters/um2)
            gnd_truth_tmp[0,10] = ActualPercentClustered        # Actual percent-points-clustered in this image
            gnd_truth_tmp[0,12] = TotalPoints                   # Total points in this image
            gnd_truth_tmp[0,13] = TotalClusteredPoints          # Total number of points in clusters (Cluster UID > 0)
            gnd_truth_tmp[0,13] = SimuCellAreaMicronsSqd        # Area of the simulated cell (in um2)
            gnd_truth_tmp[0,14] = ActualInCellPointDensity      # Actual point-density inside the cell-shape (label > -1)
            if ImageSize[2] == 0:
                gnd_truth_tmp[0,15] = TotalPoints / ((ImageSize[0] * ImageSize[1]) / 1E6)                   # Overall point density in the image
            else:
                gnd_truth_tmp[0,15] = TotalPoints / ((ImageSize[0] * ImageSize[1] * ImageSize[2]) / 1E9)    # Overall point density in the image
            ...
            if DoCentreCroppedRegions:
                ...
                Cropped_Ch1_TightCluster_Eventlist = Ch1_TightCluster_Eventlist[xycrop, :]

                if ResetOrigin:
                    Cropped_Ch1_TightCluster_Eventlist[:,0] = Cropped_Ch1_TightCluster_Eventlist[:,0] - min_x
                    Cropped_Ch1_TightCluster_Eventlist[:,1] = Cropped_Ch1_TightCluster_Eventlist[:,1] - min_y
                ...
                if simRepeat == lucky_simRepeat:
                    ...
                    if any(Idx_Clu):
                        ...
                        ax = fig.add_subplot(111, facecolor='none')
                        plt.scatter(Cropped_Ch1_TightCluster_Eventlist[Idx_Nonclu,0], Cropped_Ch1_TightCluster_Eventlist[Idx_Nonclu,1], s=1, color='grey', zorder=1)
                        plt.scatter(Cropped_Ch1_TightCluster_Eventlist[Idx_Clu,0], Cropped_Ch1_TightCluster_Eventlist[Idx_Clu,1], s=1, c=Cropped_Ch1_TightCluster_Eventlist[Idx_Clu,ClusterUID_col])
                        ...
                        for ExportImage in ExportImageFormat:
                            lucky_fig_fname = os.path.join(cropped_dir, save_file_name + ' - Centre-Cropped Highlight Preview.' + ExportImage)
                            plt.savefig(lucky_fig_fname,
                                        dpi=600,
                                        bbox_inches=0,
                                        facecolor='none',
                    ...
                    fig = plt.figure(figsize=(6, 6))
                    ax = fig.add_subplot(111, facecolor='none')
                    plt.scatter(Cropped_Ch1_TightCluster_Eventlist[:,0], Cropped_Ch1_TightCluster_Eventlist[:,1], s=1, color='black')
                    ax.set_xlim(min_x, max_x)
                    ax.set_ylim(min_y, max_y)
                    ax.set_aspect('equal')
                    plt.tight_layout()
                    ...
                    for ExportImage in ExportImageFormat:
                        lucky_fig_fname = os.path.join(cropped_dir, save_file_name + ' - Centre-Cropped Flat Preview.' + ExportImage)
                        plt.savefig(lucky_fig_fname,
                                    dpi=600,
                                    bbox_inches=0,
                                    facecolor='none',
                ...
                data_fname = os.path.join(cropped_dir, save_file_name + ' - Cropped' + OutFileExt)
                np.savetxt(data_fname, Cropped_Ch1_TightCluster_Eventlist, delimiter=OutDelimiter, fmt='%10.3f', header=data_headers, comments='')
        #
        # End of per-scenario processing

    # Save the Ground Truth Data as CSV to avoid colliding with TSV generated data
    #
    gnd_truth_headers = 'FileUID,' + \
                        'Points per um2,' + \
                        'Points per cluster,' + \
    ...
    np.savetxt(os.path.join(outputpath, 'AAA Ground Truth.csv'),
               gnd_truth,
               delimiter=',',
               fmt='%10.5f',
