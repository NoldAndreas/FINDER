...
ij_roi_files = natsorted([i for i in os.listdir(inputpath_IJROIs) if '.txt' in i])
...
for fileIdx in range(0, total_ij_rois):
    ...
    roi_data = pd.read_csv(current_file, sep="\t" ,lineterminator='\n', usecols=[0,1], header=None)
    roi_data = roi_data.rename(columns={0: 'ROIx', 1: 'ROIy'}) # not really necessary here, but just so we know what we have!
    ...
    outputfile = os.path.join(inputpath_IJROIs, current_file.split('.txt')[0] + '.json')
