...
class CellSegmentation:
    ...
    def __get_parameter_filename(self,dir='Input'):
        return os.path.join(self.basefolder,dir,'PARAMETERS_splitInOutCell.json')
    ...
    def save_parameters(self):
        ...
        with open(self.__get_parameter_filename(dir='Output'), 'w') as json_file:
    ...
    def create_image(self):
        ...
        self.image_full, self.x, self.y = self.get_image_from_localizations(self.xc, self.parameters['N_x'], 'heatmap_XC')    
    ...
    def get_cutoff_image_old(self):
        ...
        cutoff_h = np.quantile(h[h > 0], self.parameters['intensity_quantile_cutoff'])
    ...
    def segmentation(self):
        ...
        g_img = filters.gaussian(self.cutoff_image, sigma=self.parameters['sigma_gaussian_filter'])
        ...
        self.im_outcell = select_outcell(labels, self.parameters['pad_cell_border'])
    ...
    def save_split(self):
        ...
        xc_incell = get_masked_localization(self.x, self.y, self.xc, self.im_incell)
        xc_outcell = get_masked_localization(self.x, self.y, self.xc, self.im_outcell)

        np.savetxt(self.__get_outputfiles("X_incell.txt"), xc_incell, fmt="%f")
        np.savetxt(self.__get_outputfiles("X_outcell.txt"), xc_outcell, fmt="%f")

        self.save_parameters()

        if self.output_mode >= 1:
            self.get_image_from_localizations(xc_incell, self.parameters['N_x'], 'heatmap_XCincell')
            self.get_image_from_localizations(xc_outcell, self.parameters['N_x'], 'heatmap_XCoutcell')
    ...
    def segmentation_old(self):
        ...
        xc_outcell = get_masked_localization(self.x, self.y, self.xc, im_outcell)
