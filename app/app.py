...
app.layout = html.Div([
    ...
    html.Div(id="name-of-file",
             children="no file selected"
             ),

    html.Div([
        html.Div(["points per dimension",
                  dcc.Input(value=15,
                            id='points-per-dimension')]),
        html.Div(["algorithm",
                  dcc.Dropdown(
                      ["DbscanLoop", "dbscan"],
                      value="DbscanLoop",
                      id='algorithm')]),
        html.Div(["minimun threshold",
                  dcc.Input(value=5,
                            id='minimum-threshold')]),
        html.Div(["maximum threshold",
                  dcc.Input(value=20,
                            id='maximum-threshold')]),

        html.Div(["threshold values",
                  dcc.RadioItems(
                      ['Linear', 'Log'],
                      value='Linear',
                      id='threshold-values')]),

        html.Div(["sigma values",
                  dcc.RadioItems(
                      ['Linear', 'Log'],
                      value='Log',
                      id='sigma-values')]),

        html.Button('Run FINDER and download labels', id='run-FINDER', n_clicks=0),
...
def run_FINDER(n_clicks,contents, filename, points_per_dimension, algorithm, min_th, max_th, th_vals, sigma_vals):
    if n_clicks:
